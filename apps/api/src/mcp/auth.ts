...
export const authenticateMcpRequest = async (headers: IncomingHttpHeaders): Promise<McpAuthContext> => {
    ...
    if (!auth.ok)
        throw unauthorized('invalid_mcp_signature');
    const conversation = await loadConversation(conversationId);
    if (!conversation)
        throw unauthorized('conversation_not_found');
