...
const [liveRow] = (await db.execute<{
  ...
  `)).rows;
if (!liveRow)
    throw new Error('fixture: no live session — run npm run db:seed');
...
const postMcp = (body: unknown, headers: Record<string, string>): Promise<Response> => fetch(`${base}/mcp`, { method: 'POST', headers, body: JSON.stringify(body) });
console.log('\n=== MCP auth ===');
const badSig = await postMcp({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'get_cart', arguments: {} } }, signedHeaders(conversationId, expires, 'not-a-valid-signature'));
...
ok('context payload mentions surface or session', toolBody.includes('live') || toolBody.includes('session'), toolBody.slice(0, 200));
...
ok('missing required args surfaces validation error', invalidArgs.status === 200 &&
    (invalidBody.includes('invalid_arguments') ||
        invalidBody.includes('Input validation error') ||
        invalidBody.includes('isError')), invalidBody.slice(0, 200));
