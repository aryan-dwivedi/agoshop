...
export const executeNamedTool = async (conversation: ConversationRecord, turnId: number, name: ToolName, args: Record<string, unknown>): Promise<Record<string, unknown>> => {
    const surfaced = new SurfacedProducts();
    const [executed] = await executeToolCalls(conversation, turnId, [{ id: `mcp_${randomUUID()}`, name, arguments: JSON.stringify(args) }], surfaced, { path: 'direct' });
