...
const mcpLoopbackUrl = (): string => {
  ...
  if (configured.length > 0 && configured.includes('127.0.0.1')) return `${configured}/mcp`;
  if (configured.length > 0 && configured.includes('localhost')) return `${configured}/mcp`;
...
/**
 * Loopback MCP `tools/call` — same wire path Agora uses in MCP voice mode.
 * Throws on transport failure; tool-level errors are returned in the payload.
 */
export const callMcpTool = async (
  ...
  if (!response.ok) {
    logger.warn(
      { conversationId: conversation.id, tool: name, status: response.status, body: body.slice(0, 200) },
      'mcp loopback rejected tool call',
    );
