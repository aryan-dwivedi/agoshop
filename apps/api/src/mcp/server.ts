...
import { toolSchemas, type ToolName } from '@shop/shared';
import { logger } from '../lib/logger.js';
import { authenticateMcpRequest } from './auth.js';
import { executeNamedTool } from './executeTool.js';
const TOOL_DESCRIPTIONS: Record<ToolName, string> = {
    get_conversation_context: 'Live commerce context: session, offers, product in context, promotion rules.',
...
const createShopMcpServer = (conversationId: string, turnId: number): McpServer => {
    ...
    for (const name of Object.keys(toolSchemas) as ToolName[]) {
        ...
        server.registerTool(name, {
            ...
            const { loadConversation } = await import('../ai/conversations.js');
            const conversation = await loadConversation(conversationId);
            if (!conversation) {
                return {
                    content: [{ type: 'text' as const, text: JSON.stringify({ error: { code: 'not_found' } }) }],
                    isError: true,
                };
            }
                    ...
                    content: [
                        {
                            type: 'text' as const,
                            text: JSON.stringify({
                                error: { code: 'invalid_arguments', message: validated.error.message },
                            }),
                        },
                    ],
                    isError: true,
