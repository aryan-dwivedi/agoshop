...
import {
  invalidateCheckoutPolicyCache,
  toPolicy,
  type PolicyRow,
} from '../domain/checkoutPolicy.js';
import { badRequest, conflict, notFound } from '../lib/errors.js';
import { publishGlobal } from '../lib/sse.js';
import { requireRole } from '../middleware/session.js';

/**
 * Global rules are admin-owned (decision 14): the live discount and the checkout
 * policy are rows, and changing either is an API call plus a cache invalidation —
 * no redeploy. Every edit publishes on `events:global` so clients on every replica
 * drop their cart/checkout queries and refetch authoritative snapshots.
 */
