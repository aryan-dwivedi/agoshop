...
import { sellers } from '@shop/db/schema.js';
import { listSessionsForSellerSlug } from '@shop/domain-live/sessions.js';
...
router.get('/api/sellers/:slug', storefront, async (req, res, next) => {
    try {
        ...
        res.json({
            seller,
            sessions: await listSessionsForSellerSlug(seller.slug),
        });
    } catch (err) {
        next(err);
    }
