...
/**
 * Identity. Passwords are bcrypt (cost 10); the session itself is an opaque 32-byte id
 * in a signed httpOnly cookie with the record in Redis, so replicas stay stateless.
 *
 * An unauthenticated shopper is not turned away: `POST /api/auth/guest` provisions a
 * guest row and the same kind of session, and signing in afterwards carries that
 * guest's cart onto the account.
 */
...
/**
 * The guest a caller is currently signed in as, if any — the cart to carry over on
 * sign-in. Anything other than a guest is left alone: switching between two real
 * accounts must never move one shopper's cart into another's.
 */
const currentGuestId = async (req: { session?: { userId: string } }): Promise<string | null> => {
...
router.post('/api/auth/register', async (req, res, next) => {
  try {
    ...
    if (await findByEmail(email)) throw conflict('email_taken', 'that email is already registered');
...
router.post('/api/auth/login', rateLimit('login', BUDGETS.login), async (req, res, next) => {
  try {
    ...
    if (!parsed.success) throw unauthorized('invalid_credentials');

    const row = await findByEmail(parsed.data.email);
    // Compare against a dummy hash when the user is unknown so timing is uniform.
    const hash =
...
/**
 * Bootstraps a guest identity so browsing, watching and buying never need an account.
 * Idempotent by construction: `ensureIdentity` reuses an existing session, so a
 * repeat call returns the same shopper rather than stranding the first cart.
 */
router.post(
  '/api/auth/guest',
  rateLimit('login', BUDGETS.login),
  ensureIdentity,
  async (req, res, next) => {
    try {
