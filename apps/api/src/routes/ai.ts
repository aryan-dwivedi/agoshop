...
const parse = <T extends z.ZodTypeAny>(schema: T, body: unknown): z.infer<T> => {
    ...
    if (!result.success) {
        throw badRequest('invalid_body', result.error.issues.map((i) => `${i.path.join('.') || 'body'}: ${i.message}`).join('; '));
    }
    return result.data;
};
const conversationIdFrom = (value: unknown): string => {
    ...
    if (!parsed.success)
        throw badRequest('invalid_conversation_id');
    return parsed.data;
};
router.post('/api/ai/conversations', ensureIdentity, rateLimit('aiConversations', BUDGETS.aiConversations), async (req, res, next) => {
    try {
...
router.post('/api/ai/conversations/:id/start', ensureIdentity, rateLimit('aiConversations', BUDGETS.aiConversations), async (req, res, next) => {
    try {
        const input = parse(startSchema, req.body);
        const conversation = await loadOwnedConversation(conversationIdFrom(req.params.id), req.session!.userId);
...
router.post('/api/ai/convo/:conversationId/message', ensureIdentity, rateLimit('aiConversations', BUDGETS.aiConversations), async (req, res, next) => {
    try {
        const { text, history } = parse(messageSchema, req.body);
        const conversation = await loadOwnedConversation(conversationIdFrom(req.params.conversationId), req.session!.userId);
        ...
        if (!transport.sendUserTurn)
            throw conflict('transport_has_no_text_channel');
        if (conversation.transport !== 'text')
            await transport.start(conversation);
        const result = await transport.sendUserTurn({ ...conversation, transport: 'text' }, text, history);
        res.json(result);
    }
    catch (err) {
        next(err);
