...
/**
 * Every mutation publishes `cart.updated` to the user's Redis channel, so the drawer
 * updates on whichever replica the browser's SSE connection landed on.
 */
...
/** Express types a path param as `string | string[]`; a cart line id must be one uuid. */
