    ...
    router.put('/api/sessions/:id/cohost', requireSessionHost, async (req, res, next) => {
        try {
            ...
            const [candidate] = await db
                ...
                .from(users)
                .where(and(sql `lower(${users.email}) = lower(${parsed.data.email})`, isNull(users.deletedAt)));
            if (!candidate)
                throw notFound('user_not_found');
            if (isGuestEmail(candidate.email)) {
                throw badRequest('cohost_must_be_registered', 'a co-host needs a registered account');
            }
            const [row] = await db
