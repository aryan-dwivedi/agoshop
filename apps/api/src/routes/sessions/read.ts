...
export const registerReadRoutes = (router: Router): void => {
    router.get('/api/sessions', async (req, res, next) => {
        try {
            ...
            if (seller !== undefined) {
                ...
                if (!parsed.success)
                    throw badRequest('invalid_seller_slug');
    ...
    router.get('/api/sessions/:slug/products', async (req, res, next) => {
        try {
            ...
            if (!parsed.success)
                throw badRequest('invalid_session_slug');
    ...
    router.get('/api/sessions/:slug/transcript', async (req, res, next) => {
        try {
            ...
            const rows = await db
                ...
                .where(q.length > 0
                ? and(eq(sessionTranscripts.sessionId, session.id), sql `to_tsvector('simple', ${sessionTranscripts.text}) @@ plainto_tsquery('simple', ${q})`)
