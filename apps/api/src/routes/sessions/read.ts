...
import { sessionTranscripts } from '@shop/db/schema.js';
import { listChatMessages } from '@shop/domain-live/chat.js';
import { listPolls } from '@shop/domain-live/polls.js';
import { reactionCounts } from '@shop/domain-live/reactions.js';
import {
...
import { STATUSES, slugParam } from './schemas.js';
...
export const registerReadRoutes = (router: Router): void => {
    router.get('/api/sessions', async (req, res, next) => {
        try {
            ...
            res.json({
                sessions: await listSessions({ statuses: requested, sellerSlug }),
            });
        } catch (err) {
            next(err);
        }
    ...
    router.get('/api/sessions/:slug/polls', async (req, res, next) => {
        try {
            ...
            if (!session) throw notFound('session_not_found');
            res.json({
                polls: await listPolls(session.id, req.session?.userId ?? null),
            });
        } catch (err) {
            next(err);
        }
    ...
    router.get('/api/sessions/:slug/transcript', async (req, res, next) => {
        try {
            ...
            const lines: TranscriptLine[] = rows.map((row) => ({
                id: String(row.id),
                speaker:
                    row.speaker === 'assistant' || row.speaker === 'user' ? row.speaker : 'host',
