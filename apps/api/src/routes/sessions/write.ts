...
import {
    ...
    requireRole,
    requireSessionHost,
} from '@shop/platform/middleware/session.js';
...
export const registerWriteRoutes = (router: Router): void => {
    ...
    router.post('/api/sessions/:id/products', requireSessionHost, async (req, res, next) => {
        try {
            ...
            if (!parsed.success) throw badRequest('invalid_body', parsed.error.issues[0]?.message);
            res.json({
                products: await setSessionProducts(id.data, parsed.data.items),
            });
        } catch (err) {
            next(err);
        }
    ...
    router.post('/api/sessions/:id/pin', requireSessionHost, async (req, res, next) => {
        try {
            ...
            if (!id.success) throw badRequest('invalid_session_id');
            const parsed = z
                .object({ productId: z.string().uuid().nullable() })
                .safeParse(req.body);
            if (!parsed.success) throw badRequest('invalid_body', parsed.error.issues[0]?.message);
            res.json({
                products: await pinProduct(id.data, parsed.data.productId),
            });
        } catch (err) {
            next(err);
        }
    ...
    router.patch('/api/sessions/:id/pricing', requireSessionHost, async (req, res, next) => {
        try {
            ...
            const parsed = z
                .object({
                    discountPercent: z.number().int().min(0).max(90).nullable(),
                })
                .safeParse(req.body);
            if (!parsed.success) throw badRequest('invalid_body', parsed.error.issues[0]?.message);
            res.json({
                session: await setSessionPricing(id.data, parsed.data.discountPercent),
            });
        } catch (err) {
            next(err);
        }
