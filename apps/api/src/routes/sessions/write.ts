...
import { createSession, endSession, heartbeatSession, joinSession, pinProduct, setSessionPricing, setSessionProducts, startSession, updateSession, } from '../../domain/sessions.js';
...
export const registerWriteRoutes = (router: Router): void => {
    ...
    router.post('/api/sessions/:id/start', requireSessionHost, async (req, res, next) => {
        try {
            ...
            const consent = z
                ...
                .safeParse(req.body ?? {});
            res.json(await startSession(id.data, {
    ...
    router.post('/api/sessions/:id/consent', ensureIdentity, async (req, res, next) => {
        try {
            ...
            if (!parsed.success)
                throw badRequest('invalid_body', 'acknowledged must be true');
            track({
