...
export const registerMediaRoutes = (router: Router): void => {
    router.post('/api/sessions/:id/recording', requireSessionHost, upload.single('recording'), async (req, res, next) => {
        try {
            const id = idParam.safeParse(req.params.id);
            if (!id.success)
            ...
            const durationMs = Number((req.body as Record<string, unknown> | undefined)?.['durationMs'] ?? 0);
            const mirror = await mirrorRecording(req.file.path, `sessions/${sessionId}/${req.file.filename}`, req.file.mimetype || 'video/webm');
    ...
    router.post('/api/sessions/:id/source-video', requireSessionHost, sourceVideoUpload.single('video'), async (req, res, next) => {
        try {
            const id = idParam.safeParse(req.params.id);
            if (!id.success)
                throw badRequest('invalid_session_id');
