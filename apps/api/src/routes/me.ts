...
/**
 * Subject erasure, executed as the schema declares it (data model, decision 15) in ONE
 * transaction: personal artefacts are deleted, analytics rows are de-identified, and the
 * `users` row is tombstoned rather than removed — `orders`, `orderItems` and
 * `promotionRedemptions` must survive for financial and tax reasons, and they reference
 * the user row.
 */
...
router.delete('/api/me/data', requireAuth, async (req, res, next) => {
  try {
    ...
    const removed = await db.transaction(async (tx) => {
      ...
      await run(
        'aiToolCalls',
        sql`delete from ai_tool_calls where conversation_id in
              (select id from ai_conversations where user_id = ${uid})`,
      );
      await run(
        'aiMessages',
        sql`delete from ai_messages where conversation_id in
              (select id from ai_conversations where user_id = ${uid})`,
      );
      ...
      await run(
        'chatModeration',
        sql`delete from chat_moderation where target_user_id = ${uid} or actor_user_id = ${uid}`,
      );
      await run(
        'analyticsEvents',
        sql`update analytics_events set user_id = null where user_id = ${uid}`,
      );

      // Tombstone: orders and redemptions keep pointing at a scrubbed row.
      await run(
        'users',
        sql`update users
            set deleted_at = now(),
