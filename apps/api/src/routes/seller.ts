...
import { categories, liveSessions, liveSessionProducts, products, productVariants, sellers, } from '../db/schema.js';
...
const resolveScope = async (req: Request): Promise<Scope> => {
    ...
    const visible = session.role === 'admin' ? rows : rows.filter((s) => s.ownerUserId === session.userId);
    if (visible.length === 0)
        throw notFound('no_seller_profile');
...
const requireScopedSession = async (req: Request, scope: Scope): Promise<string> => {
    ...
    if (!scope.visible.some((s) => s.id === row.sellerId))
        throw forbidden('session_not_owned');
    return sessionId;
};
...
router.get('/api/seller/orders', ...guards, async (req, res, next) => {
    try {
        ...
        const scope = await resolveScope(req);
        const orders = await listSellerOrders(scope.sellerIds, parsed.data.limit ?? SELLER_ORDERS_DEFAULT_LIMIT);
...
const toSellerProduct = (row: SellerProductRow, variants: SellerVariantRow[], inShows: ProductShow[] = []): SellerProduct => {
    ...
    return {
        ...
        priceMinorUnits: variants.reduce((min, v) => (min === 0 ? v.priceMinorUnits : Math.min(min, v.priceMinorUnits)), 0),
        totalStock,
...
router.get('/api/seller/products', ...guards, async (req, res, next) => {
    try {
        ...
        const showRows = productIds.length === 0
            ? []
            : await db
                .select({
                ...
                .where(and(inArray(liveSessionProducts.productId, productIds), inArray(liveSessions.status, ['scheduled', 'live'])))
        ...
        res.json({
            ...
            products: rows.map((row) => toSellerProduct(row, variantsByProduct.get(row.productId) ?? [], showsByProduct.get(row.productId) ?? [])),
        });
    }
    catch (err) {
        next(err);
...
const variantPatchBody = z
    ...
    .partial();
router.patch('/api/seller/products/:productId/variants/:variantId', ...guards, async (req, res, next) => {
    try {
        const productId = z.string().uuid().safeParse(req.params.productId);
        if (!productId.success)
            throw badRequest('invalid_product_id');
        const variantId = z.string().uuid().safeParse(req.params.variantId);
        if (!variantId.success)
            throw badRequest('invalid_variant_id');
        ...
        const [current] = await db
            ...
            .from(productVariants)
            .where(and(eq(productVariants.id, variantId.data), eq(productVariants.productId, productId.data)));
        if (!current)
            throw notFound('variant_not_found');
        const priceMinorUnits = parsed.data.priceMinorUnits ?? current.priceMinorUnits;
        const mrpMinorUnits = parsed.data.mrpMinorUnits === undefined ? current.mrpMinorUnits : parsed.data.mrpMinorUnits;
...
router.post('/api/seller/products', ...guards, async (req, res, next) => {
    try {
        ...
        if (!scope.visible.some((s) => s.id === sellerId))
            throw forbidden('seller_not_owned');
        ...
        if (!category)
            throw badRequest('unknown_category', `No category ${body.categorySlug}`);
        ...
        const product = await db
            ...
            const variants = await tx
                ...
                .values(body.variants.map((variant, index) => ({
                productId: row.productId,
                sku: skus[index]!,
                label: variant.label,
