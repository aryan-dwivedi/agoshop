...
import { LOW_STOCK_THRESHOLD, sellerOverview, sessionAnalytics } from '../domain/analytics.js';
import { invalidateCatalogCache } from '../domain/catalog.js';
import { enqueueSearchIndex } from '../lib/searchIndex.js';
import { listModeration } from '../domain/moderation.js';
import { SELLER_ORDERS_DEFAULT_LIMIT, listSellerOrders } from '../domain/orders.js';
import { viewerCount } from '../domain/sessions.js';
import { badRequest, forbidden, notFound } from '../lib/errors.js';
import { publishGlobal } from '../lib/sse.js';
import { requireAuth, requireRole } from '../middleware/session.js';

/**
 * The seller dashboard's read API. Decision 14: sellers own their sessions, products,
 * moderation history and analytics; global promotions and checkout policy live behind
 * `requireRole('admin')` in routes/admin.ts. Nothing here would reject a seller.
 *
 * Scoping: a seller sees the sellers they own (`sellers.ownerUserId`); an admin sees
 * every seller. `?sellerId=` narrows to one and is refused when a seller asks for a
...
type Scope = {
  sellerIds: string[];
  primaryId: string;
  visible: { id: string; slug: string; displayName: string }[];
};
...
const resolveScope = async (req: Request): Promise<Scope> => {
  ...
  if (requested) {
    ...
    return {
      sellerIds: [match.id],
      primaryId: match.id,
      visible: visible.map((s) => ({ id: s.id, slug: s.slug, displayName: s.displayName })),
    };
  }
...
/**
 * Validates and scopes `:id` in one place. `@types/express-serve-static-core@5` types
 * params as `string | string[]`, so the id is parsed rather than cast.
 */
...
router.get('/api/seller/overview', ...guards, async (req, res, next) => {
  try {
    ...
    res.json({ ...overview, sellerId: scope.primaryId, sellers: scope.visible });
  } catch (err) {
    next(err);
  }
});
...
/**
 * Recent orders containing this seller's lines. Not paginated: `limit` (default 100,
 * max 500) bounds the read and the response is exactly that window, newest first —
 * there is no cursor and the console must not pretend otherwise.
 */
router.get('/api/seller/orders', ...guards, async (req, res, next) => {
  try {
    const parsed = ordersQuery.safeParse(req.query);
    if (!parsed.success) {
      throw badRequest('validation_failed', 'invalid order query', { issues: parsed.error.issues });
    }
    const scope = await resolveScope(req);
...
router.get('/api/seller/sessions', ...guards, async (req, res, next) => {
  try {
    ...
    res.json({ sessions: enriched, sellerId: scope.primaryId, sellers: scope.visible });
  } catch (err) {
    next(err);
  }
});
...
/** The Moderation-log data source. `listModeration` is owned by domain/moderation.ts. */
...
/** One show a product is committed to. Ended shows are history, not a commitment. */
type ProductShow = { id: string; slug: string; title: string; status: 'scheduled' | 'live' };
...
type SellerProduct = {
  ...
  lowStock: boolean;
  /**
   * The shows this product is on the rail of, scheduled and live only: the console's
   * "In shows" column exists to answer "may I take this off sale", and an ended show
   * has no claim on stock.
   */
  inShows: ProductShow[];
...
/**
 * The dashboard's product row, assembled in exactly one place. The listing and the
 * create response are the SAME object, so a field added for one cannot quietly go
 * missing from the other and leave the Inventory table rendering a hole the moment a
...
router.get('/api/seller/products', ...guards, async (req, res, next) => {
  try {
    ...
    // One `in (...)` for the whole page, not one per row.
    const showRows =
      productIds.length === 0
        ? []
        : await db
            ...
            .orderBy(sql`coalesce(${liveSessions.scheduledFor}, ${liveSessions.createdAt}) asc`);
    ...
    for (const show of showRows) {
      ...
      list.push({ id: show.id, slug: show.slug, title: show.title, status: show.status });
...
const variantPatchBody = z
  .object({
    ...
    /** `null` retracts a published MRP: the product simply stops advertising a markdown. */
...
/**
 * The dashboard's only write path: a seller moves one variant's price, MRP or stock.
 *
 * Ownership is the same `resolveScope` question every read here asks — the product's
 * seller must be one this account owns (or the caller must be an admin). The MRP/price
 * ordering is enforced against the RESULT of the patch, not against the body, so a
 * price-only edit that would leave the stored MRP at or below the new price is refused
 * rather than silently producing a ladder with a meaningless top tier.
 */
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const mrpMinorUnits =
        parsed.data.mrpMinorUnits === undefined ? current.mrpMinorUnits : parsed.data.mrpMinorUnits;
      ...
      // Order matters: drop the cached listings BEFORE announcing the change, so a client
      // that refetches the instant it hears the event cannot be served the old price.
...
const productCreateBody = z.object({
  /** Optional: an owner running several storefronts says which one gets the listing. */
  ...
  variants: z
    .array(
      z.object({
        ...
        /** Omitted: derived from the slug, so a one-variant listing needs no SKU scheme. */
        sku: z.string().trim().min(3).max(64).optional(),
        priceMinorUnits: z.number().int().positive(),
        /** `null` and omitted both mean "no published MRP", as in the variant PATCH. */
...
/** Folds diacritics, drops punctuation and collapses separator runs into one dash. */
...
/**
 * Probes `products.slug` and appends `-2`, `-3`, … until one is free.
 *
 * A counter rather than a random suffix because this slug IS the shopper-facing URL
 * (`/p/<slug>`): a seller relisting a colourway wants a link they can read out on a
 * live stream, not `aurora-buds-pro-7f3a91`. The probe runs inside the insert's own
 * transaction, so the window between "free" and "taken" is as narrow as it can be.
 */
const claimProductSlug = async (tx: Tx, title: string): Promise<string> => {
  // A title made only of punctuation kebab-cases to nothing, and it still needs a URL.
...
/** Seeded SKUs are upper-case and dash-separated (`AUR-BP-BLK`); generated ones match. */
const deriveSku = (slug: string, index: number): string => `${slug}-${index + 1}`.toUpperCase();

/** `pg` reports a unique-index violation as this string on the thrown error's `code`. */
...
/**
 * The console's only creating write path. Everything else in the dashboard reads, or
 * moves one number on a row that already exists.
 *
 * The product and its variants land in ONE transaction. A variant rejected halfway
 * through — duplicate SKU, MRP at or below the price — must not leave a product with
 * no buyable variant behind: the storefront would happily list it, and no shopper
 * could add it to a cart.
 */
router.post('/api/seller/products', ...guards, async (req, res, next) => {
  try {
    ...
    // Flagging nothing is the common case — a single-variant listing — so the first
    // variant becomes the default instead of making the client state the obvious.
    const defaultIndex = flagged === 1 ? body.variants.findIndex((v) => v.isDefault === true) : 0;

    // Same rule and same message as the variant PATCH: an MRP at or below the selling
    // price advertises a markdown that does not exist.
    ...
    const product = await db
      .transaction(async (tx) => {
        ...
        // Two body variants can collide with each other before Postgres ever sees them.
        ...
        const [row] = await tx
          ...
          .values({
            ...
            specs: body.specs,
            images: body.images,
            // The column defaults to 4 for the seeded catalog; a listing created here has
            // been rated by nobody, and 4 stars from zero ratings is a fabricated claim.
            rating: 0,
            ratingCount: 0,
            // The listing price a shopper sorts and filters on is the cheapest variant.
            basePriceMinorUnits: Math.min(...body.variants.map((v) => v.priceMinorUnits)),
          })
          .returning({
            productId: products.id,
            slug: products.slug,
      ...
      .catch((err: unknown) => {
        // The pre-check closes the ordinary case; two sellers racing on the same SKU
        // still reach the unique index, and that is the seller's 400, not our 500.
    ...
    // Same order as the variant PATCH: drop the cache BEFORE announcing. The event is
    // reused rather than invented because a NEW listing makes the cached listing pages
    // stale in exactly the way a price move does — same keys, same readers — and every
    // client already refetches on it.
    await invalidateCatalogCache();
