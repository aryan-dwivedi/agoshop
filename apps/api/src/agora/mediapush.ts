...
const converterBase = () => `${AGORA_REST_BASE}/${env.MEDIA_PUSH_REGION}/v1/projects/${env.AGORA_APP_ID}/rtmp-converters`;
type Json = Record<string, unknown>;
const call = async (method: 'POST' | 'GET' | 'DELETE', suffix: string, body?: Json): Promise<{
    ok: boolean;
    status: number;
    json: Json | null;
}> => {
...
export const hlsOriginReady = (session: {
    mediaPushStatus: string;
}): boolean => env.MEDIA_PUSH_ENABLED ? session.mediaPushStatus === 'running' : true;
export const createConverter = async (session: {
    id: string;
    slug: string;
    rtcChannel: string;
}): Promise<void> => {
    if (!env.MEDIA_PUSH_ENABLED)
        return;
    if (!env.MEDIA_PUSH_RTMP_URL || !env.MEDIA_PUSH_HLS_URL) {
        ...
        logger.warn({ sessionId: session.id }, 'media push enabled but MEDIA_PUSH_RTMP_URL/MEDIA_PUSH_HLS_URL are empty');
        return;
    }
    await db
        .update(liveSessions)
    ...
    if (!created.ok || typeof converterId !== 'string') {
        await db
            ...
            .set({ mediaPushStatus: 'failed' })
            .where(eq(liveSessions.id, session.id));
        logger.warn({ sessionId: session.id, status: created.status, body: created.json }, 'media push converter creation failed');
        return;
    }
    await db
        .update(liveSessions)
...
export const getConverter = async (sessionId: string): Promise<{
    converterId: string;
    state: string;
} | null> => {
    const [row] = await db
...
export const deleteConverter = async (sessionId: string): Promise<void> => {
    const [row] = await db
        ...
        .from(liveSessions)
        .where(eq(liveSessions.id, sessionId));
    if (!row?.converterId || row.status === 'off')
        return;
