...
import { getConversationLlmMode, setConversationLlmMode, CONVOAI_FAILURE_MESSAGE, type LlmMode, } from '../ai/llmMode.js';
...
export const buildConvoAiTtsBlock = (language: string): Record<string, unknown> => ({
    ...
    params: {
        url: env.CONVOAI_TTS_URL.replace(/\/$/, '') ||
...
export const buildConvoAiJoinBody = (input: ConvoAiJoinInput, llmMode: ConvoAiLlmMode): Record<string, unknown> => {
    const mcpEndpoint = env.MCP_ENDPOINT_URL.replace(/\/$/, '') || `${env.PUBLIC_API_URL.replace(/\/$/, '')}/mcp`;
    const authHeaders = {
...
const call = async (req: ConvoAiRequest): Promise<{
    status: number;
    json: unknown;
}> => {
    const auth = agoraBasicAuth();
    if (!auth)
        throw new AppError(503, 'convoai_unconfigured');
    ...
    if (response.status === 404 && req.tolerateNotFound)
...
const recoverConflictingJoin = async (err: unknown, llmMode: ConvoAiLlmMode): Promise<{
    agentId: string;
    llmMode: ConvoAiLlmMode;
} | null> => {
    if (!(err instanceof AppError) || err.code !== 'convoai_join_failed')
        return null;
    const agora = err.details?.agora as {
        agent_id?: unknown;
        reason?: unknown;
    } | null | undefined;
    if (agora?.reason !== 'TaskConflict' ||
    ...
    for (const delayMs of JOIN_CONFLICT_LOOKUP_DELAYS_MS) {
        ...
        const agentId = typeof agent?.agent_id === 'string' && agent.agent_id.length > 0
            ? agent.agent_id
            : agora.agent_id;
        logger.warn({ agentId, agentStatus: agent?.status, llmMode }, 'recovered existing convoai agent after join conflict');
        return { agentId, llmMode };
    }
    return null;
};
const joinWithMode = async (input: ConvoAiJoinInput, llmMode: ConvoAiLlmMode): Promise<{
    agentId: string;
    llmMode: ConvoAiLlmMode;
}> => {
    let json: unknown;
...
export const joinConvoAiAgent = async (input: ConvoAiJoinInput, opts?: {
    forceMode?: ConvoAiLlmMode;
}): Promise<{
    agentId: string;
    llmMode: ConvoAiLlmMode;
}> => {
    ...
    try {
        ...
        logger.info({ conversationId: input.conversationId, llmMode: 'mcp' }, 'convoai agent joined with mcp llm');
        return joined;
    }
    catch (err) {
        if (err instanceof AppError && err.code === 'ai_capacity')
            throw err;
        logger.warn({ err, conversationId: input.conversationId }, 'convoai mcp join failed; falling back to custom llm');
        await setConversationLlmMode(input.conversationId, 'custom');
        const joined = await joinWithMode(input, 'custom');
        logger.info({ conversationId: input.conversationId, llmMode: 'custom' }, 'convoai agent joined with custom llm after mcp fallback');
        return joined;
    }
};
...
export const speakConvoAiAgent = async (agentId: string, a: {
    text: string;
...
export const getConvoAiAgent = async (agentId: string): Promise<{
    agentId: string;
    status: string;
    startTs?: number;
} | null> => {
