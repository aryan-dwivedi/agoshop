...
const streamKeyBase = () => `${AGORA_REST_BASE}/${env.MEDIA_GATEWAY_REGION}/v1/projects/${env.AGORA_APP_ID}/rtls/ingress/streamkeys`;
...
const call = async (method: 'POST' | 'DELETE', suffix: string, body?: Json): Promise<{
    ok: boolean;
    status: number;
    json: Json | null;
}> => {
...
export const ensureGatewayUid = async (sessionId: string): Promise<number> => {
    ...
    if (claimed.length > 0 && claimed[0]!.uid !== null)
        return claimed[0]!.uid;
    const [winner] = await db
        .select({ uid: liveSessions.mediaGatewayUid })
        .from(liveSessions)
        .where(eq(liveSessions.id, sessionId));
    if (!winner || winner.uid === null)
        throw new Error('gateway_uid_alloc_failed');
    return winner.uid;
};
export const mintObsIngest = async (session: {
    ...
    if (!created.ok || typeof streamKey !== 'string') {
        ...
        logger.warn({ sessionId: session.id, status: created.status, body: created.json }, 'media gateway stream key creation failed');
