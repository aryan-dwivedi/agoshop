...
const run = async (): Promise<void> => {
    check('TRANSCRIPTION_PROVIDER is agora (otherwise startRtt is a documented no-op)', env.TRANSCRIPTION_PROVIDER === 'agora', env.TRANSCRIPTION_PROVIDER);
    ...
    try {
        ...
        check('rttStatus reflects a live worker rather than a failure', after?.status === 'running' || after?.status === 'connecting', after?.status ?? 'unknown');
        if (after?.taskId) {
        ...
        try {
            ...
            check('RTT leave released the worker', ended?.status !== 'running', ended?.status ?? 'unknown');
        }
        catch (err) {
            check('RTT leave released the worker', false, err instanceof Error ? err.message : String(err));
        }
    }
};
run()
