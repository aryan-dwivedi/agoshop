...
export const startRtt = async (session: {
    ...
    if (!joined.ok || typeof taskId !== 'string') {
        await db
            ...
            .set({ rttStatus: 'failed' })
            .where(eq(liveSessions.id, session.id));
        logger.warn({ sessionId: session.id, status: joined.status, body: joined.json }, 'rtt join failed');
        return;
    }
    await db
        .update(liveSessions)
...
export const queryRtt = async (sessionId: string): Promise<{
    taskId: string;
    status: string;
} | null> => {
    const [row] = await db
