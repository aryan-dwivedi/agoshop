...
const CHECKS = [
    {
        ...
        script: 'packages/domain-commerce/src/__checks__/commerce.check.ts',
    },
    {
        name: 'live — conditional transitions, frozen shards, one-way tier flip, moderation',
        script: 'packages/domain-live/src/__checks__/live.check.ts',
    },
    {
        name: 'ai — signed callback, tool-call ordering, replay dedupe, disconnect abort',
        script: 'packages/ai/src/__checks__/ai.check.ts',
    },
    {
        name: 'seller — product creation scoping, SKU collisions, catalog visibility',
        script: 'packages/domain-commerce/src/__checks__/seller.check.ts',
    },
    {
        name: 'support — ticket reservation, RTC activation, authorization, close',
        script: 'packages/domain-commerce/src/__checks__/support.check.ts',
    },
] as const;
const run = (
    script: string,
