...
/**
 * The domain checks assert observable contracts against real Postgres and Redis
 * through the app factory. They are executed as child processes so each keeps its own
 * `--env-file=.env`, exactly as a developer runs them.
 *
 * Prerequisites (the suite says so instead of silently passing): `npm run stack:up`,
...
const run = (script: string): Promise<{ code: number; output: string }> => {
  const { promise, resolve: settle } = Promise.withResolvers<{ code: number; output: string }>();
...
describe('integration checks', () => {
  it('has an .env to run against', () => {
    expect(
      existsSync(envFile),
      'copy .env.example to .env before running the integration checks',
    ).toBe(true);
  });
  ...
  for (const check of CHECKS) {
    it(
      ...
      async () => {
        ...
        if (code !== 0) {
          // Surface the child's own assertion output; a bare exit code is useless.
