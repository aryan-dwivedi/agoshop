...
describe('resolveRtcTokenRole', () => {
    ...
    it('downgrades a random viewer requesting publisher to subscriber', () => {
        expect(resolveRtcTokenRole('publisher', {
            userId: 'viewer-1',
            userRole: 'customer',
            ...session,
        })).toBe('subscriber');
    ...
    it('grants publisher to the session host', () => {
        expect(resolveRtcTokenRole('host', {
            userId: 'host-1',
            userRole: 'seller',
            ...session,
        })).toBe('publisher');
    ...
    it('maps audience requests to subscriber', () => {
        expect(resolveRtcTokenRole('audience', {
            userId: 'host-1',
            userRole: 'seller',
            ...session,
        })).toBe('subscriber');
...
describe('media gateway RTMP URL shape', () => {
    it('uses the Agora unified ingress domain pattern', () => {
        expect(rtmpServerUrl('ap')).toMatch(/^rtmp:\/\/rtls-ingress-prod-[a-z]{2}\.agoramdn\.com\/live$/);
    });
});
