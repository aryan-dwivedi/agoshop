...
const { buildConvoAiJoinBody, buildConvoAiTtsBlock, joinConvoAiAgent } = await import('../agora/convoai.js');
const input = {
...
describe('ConvoAI speech handling', () => {
    it.each<ConvoAiLlmMode>(['mcp', 'custom'])('buffers startup speech and accepts short utterances in %s mode', (mode) => {
    ...
    it('recovers the agent created before a retried join conflicts', async () => {
        ...
        const fetchMock = vi
            ...
            .mockResolvedValueOnce(new Response(JSON.stringify({ agent_id: agentId, status: 'RUNNING' }), {
            status: 200,
