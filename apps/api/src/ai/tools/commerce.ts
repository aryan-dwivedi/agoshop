...
/** Commerce tools — cart, wishlist, delivery, payment, offers, live offer. */
export const runCommerceTool = async (
  ...
  switch (call.name) {
    ...
    case 'get_payment_options': {
      ...
      // The same function the checkout page calls, so the two cannot disagree.
    ...
    case 'add_to_cart': {
      ...
      // The web client raises its cart toast from this event alone, so the payload
      // carries everything the toast renders — no refetch race against `cart.updated`.
