...
/**
 * Catalog read tools — search, details, compare, recommend. Every price-bearing answer
 * is recomputed here at call time; the assistant is never allowed to be the cache.
 */
export const runCatalogTool = async (
  ...
  switch (call.name) {
    ...
    case 'compare_products': {
      // Both reads hit the same per-product cache, so the cards cost no extra query.
