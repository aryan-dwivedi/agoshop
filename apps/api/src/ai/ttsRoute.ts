...
const synthesizeOpenAi = async (body: z.infer<typeof speechBody>, format: NonNullable<z.infer<typeof speechBody>['response_format']>, signal: AbortSignal): Promise<Buffer> => {
...
router.post('/api/ai/tts/speech', async (req, res, next) => {
    ...
    try {
        ...
        if (!engine) {
            throw new AppError(503, 'tts_unconfigured', 'voice TTS requires ELEVENLABS_API_KEY or OPENAI_API_KEY');
        }
        ...
        logger.info({ chars: parsed.data.input.length, bytes: audio.byteLength, engine, format }, 'tts synthesized');
    }
    catch (err) {
        if (!abort.signal.aborted)
            next(err);
