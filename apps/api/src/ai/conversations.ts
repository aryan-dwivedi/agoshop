...
const GREETINGS: Record<string, string> = {
    'en-US': 'Hi! I can help you compare these products, check delivery, and add anything to your cart. What are you looking for?',
    'en-IN': 'Hi! I can help you compare these products, check delivery, and add anything to your cart. What are you looking for?',
    'hi-IN': 'नमस्ते! मैं इन प्रोडक्ट्स की तुलना कर सकता हूँ, डिलीवरी देख सकता हूँ और कार्ट में जोड़ सकता हूँ। आप क्या ढूंढ रहे हैं?',
    'es-ES': '¡Hola! Puedo comparar estos productos, comprobar la entrega y añadir lo que quieras al carrito. ¿Qué estás buscando?',
};
...
export const loadOwnedConversation = async (id: string, userId: string): Promise<ConversationRecord> => {
    const conversation = await loadConversation(id);
    if (!conversation)
        throw notFound('conversation_not_found');
    if (conversation.userId !== userId)
        throw forbidden('not_your_conversation');
    return conversation;
};
...
export const createConversation = async (input: CreateConversationInput): Promise<CreateConversationDto> => {
    ...
    if (language !== LANGUAGE_AUTO && !env.CONVOAI_SUPPORTED_LANGUAGES.includes(language)) {
        throw badRequest('unsupported_language', `language must be one of ${env.CONVOAI_SUPPORTED_LANGUAGES.join(', ')} or ${LANGUAGE_AUTO}`);
    }
    ...
    if (input.liveSessionId) {
        const [session] = await db
            ...
            .limit(1);
        if (!session)
            throw notFound('live_session_not_found');
    }
...
const buildConvoAiJoinParams = async (conversation: ConversationRecord): Promise<Parameters<typeof joinConvoAiAgent>[0]> => {
    const expires = Math.floor(conversation.callbackExpiresAt.getTime() / 1000);
    const spokenLanguage = resolveSpokenLanguage(conversation.language, env.CONVOAI_SUPPORTED_LANGUAGES, {
        text: conversation.language === LANGUAGE_AUTO
...
export const startConversation = async (conversation: ConversationRecord): Promise<{
    agentId: string;
}> => {
    if (!features.convoai) {
...
export const fallbackConversationToCustomLlm = async (conversation: ConversationRecord): Promise<{
    agentId: string;
    llmMode: 'custom';
}> => {
    if (!features.convoai) {
    ...
    if (conversation.agoraAgentId) {
        await leaveConvoAiAgent(conversation.agoraAgentId).catch((err: unknown) => logger.warn({ err, conversationId: conversation.id, agoraAgentId: conversation.agoraAgentId }, 'convoai leave before llm fallback failed; rejoining anyway'));
    }
    ...
    logger.info({ conversationId: conversation.id, agoraAgentId: agentId }, 'convoai agent rejoined with custom llm after runtime fallback');
    return { agentId, llmMode: 'custom' };
};
export const stopConversation = async (conversationId: string, opts: {
    ...
    if (conversation.agoraAgentId) {
        await leaveConvoAiAgent(conversation.agoraAgentId).catch((err: unknown) => logger.warn({ err, conversationId, agoraAgentId: conversation.agoraAgentId }, 'convoai leave failed; releasing the slot anyway'));
    }
    await releaseSlot(conversationId);
...
export const heartbeatConversation = async (conversation: ConversationRecord): Promise<{
    refreshed: boolean;
