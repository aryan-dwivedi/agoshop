...
export type PendingToolCall = { id: string; name: string; arguments: string };
...
export type ToolExecutionOptions = {
  /** `direct` skips MCP loopback (used by the MCP HTTP server to avoid recursion). */
  path?: 'auto' | 'direct';
};

/** Stable key order at every depth, so equal arguments always hash equal. */
...
const persistToolMessage = async (
  ...
  // PRIVACY_MODE=strict keeps the tool name and turn so analytics still work, but
  // never the argument or result bodies.
...
const executeOne = async (
  ...
  // zod's per-tool schema map cannot correlate its parse output with the narrowed
  // name, so the correlation is asserted once, here, on already-validated data.
  ...
  const runDirect = async (): Promise<Record<string, unknown>> => {
    try {
      ...
      if ('error' in direct) outcome = 'error';
      return direct;
    } catch (err) {
      outcome = 'error';
  ...
  const useMcpLoopback =
    opts.path !== 'direct' && (await getConversationLlmMode(conversation.id)) === 'mcp';

  if (useMcpLoopback) {
    try {
      result = await callMcpTool(conversation, turnId, name, args);
      if ('error' in result) outcome = 'error';
    } catch (err) {
      logger.warn(
        { err, conversationId: conversation.id, tool: name },
        'mcp tool loopback failed; falling back to custom tool path',
      );
  ...
  if (mutating && outcome === 'ok') {
    // Written in the same request that mutated, so the next retry of this turn
    // short-circuits above instead of adding to the cart twice.
    await db
      .insert(aiToolCalls)
      .values({
        conversationId: conversation.id,
  ...
  track({
    ...
    sessionId: conversation.liveSessionId,
    productId:
      typeof args.product_id === 'string' ? args.product_id : conversation.contextProductId,
  ...
  await persistToolMessage(conversation, turnId, name, args, result).catch((err: unknown) =>
    logger.warn({ err, conversationId: conversation.id }, 'ai tool message persist failed'),
  );
...
/**
 * Executes one round of tool calls and returns results in MODEL ORDER, which is the
 * order the `{role:'tool'}` messages must be appended in.
 */
export const executeToolCalls = async (
  ...
  for (const [index, call] of calls.entries()) {
    if (MUTATING_TOOLS[call.name as ToolName] === true) mutating.push(index);
    else readOnly.push(index);
  }

  // Read-only calls have no ordering constraint; mutations do.
  const parallel = Promise.all(
    readOnly.map(async (index) => {
