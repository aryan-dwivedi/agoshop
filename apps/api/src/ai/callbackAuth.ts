...
export const signCallback = (conversationId: string, expiresUnixSeconds: number): string => createHmac('sha256', env.CONVO_LLM_SHARED_SECRET)
    .update(`${conversationId}.${expiresUnixSeconds}`)
    .digest('base64url');
export type CallbackAuthFailure = 'malformed_conversation_id' | 'missing_headers' | 'malformed_expires' | 'expired' | 'expires_too_far_future' | 'bad_signature';
...
export const verifyCallback = (conversationId: string, expiresHeader: string | undefined, signatureHeader: string | undefined, nowSeconds: number = Math.floor(Date.now() / 1000)): CallbackAuthResult => {
    if (!expiresHeader || !signatureHeader)
...
export const requireCallbackAuth = (req: Request, _res: Response, next: NextFunction): void => {
    ...
    const result = conversationId
        ? verifyCallback(conversationId, req.header('X-Convo-Expires') ?? undefined, req.header('X-Convo-Signature') ?? undefined)
