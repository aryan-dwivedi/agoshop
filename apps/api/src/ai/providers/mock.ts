...
/**
 * The deterministic provider. It is a real registry member, not a stub: the automated
 * tests and the k6 `ai-proxy` scenario select it with `LLM_PROVIDER=mock` so proxy and
 * tool-execution overhead can be measured without model latency, and so the
 * fragmented-tool-call and callback-replay paths are reproducible.
 *
 *   `[[mock:text]]`                     — four content deltas, then `stop` (default)
 *   `[[mock:streaming]]`                — first delta, pause, then remaining deltas
 *   `[[mock:deferred]]`                 — a wait-only draft, then a repaired answer
 *   `[[mock:references]]`               — echoes canonical product-reference context
 *   `[[mock:tool:add_to_cart:<uuid>]]`  — content + a FRAGMENTED `add_to_cart` call
 *   `[[mock:tool:details:<uuid>]]`      — content + a fragmented `get_product_details` call
 *   `[[mock:text_tool]]`                — a textual `<tool_call>` protocol leak
 *   `[[mock:slow]]`                     — one delta, then blocks until the signal aborts
 *   `[[mock:error]]`                    — fails mid-stream like a provider outage
 *
 * Once the transcript already contains a `role:'tool'` message the script always
 * switches to the closing text response, which is what terminates the proxy's
 * multi-round loop exactly as a real model does.
 */

/** In-process counters the tests assert on; a real provider has metrics instead. */
export const mockStats = { streams: 0, aborted: 0, completed: 0 };

/**
 * Tool-call ids are FRESH on every run, deliberately: a retried Agora callback re-runs
 * the model and gets a new `call_…` id, which is exactly why `aiToolCalls` is keyed on
 * the argument hash instead.
 */
let callSeq = 0;
...
type Script =
  ...
  | { kind: 'error' }
  | { kind: 'text_tool' }
  | { kind: 'tool'; tool: 'add_to_cart' | 'get_product_details'; productId: string };
...
/** Resolves never; rejects with an AbortError the moment the client disconnects. */
...
async function* run(req: LlmRequest): AsyncGenerator<LlmChunk> {
  ...
  try {
    ...
    // A fragmented tool call: the first fragment carries id + name, later fragments
    // only append `arguments`, so a consumer that does not accumulate by `index`
    ...
    // A disconnect surfaces here because the generator is awaiting, not suspended
    // at a `yield`, when the abort fires.
