...
/**
 * The OpenAI-compatible streaming provider — OpenAI direct, Groq, Together, Azure or a
 * self-hosted vLLM, selected purely by `LLM_BASE_URL`.
 *
 * The SSE reader here is the one shared by every HTTP provider in the registry
 * (OpenRouter configures it differently rather than reimplementing it).
 */
...
export type SseStreamOptions = {
  ...
  signal: AbortSignal;
  /**
   * OpenRouter repeats `finish_reason` on the trailing usage-only chunk (OpenAI sends
   * an empty `choices` array instead). Forwarding it would end the round twice.
   */
...
const errorText = (error: NonNullable<RawChunk['error']>): string =>
  typeof error === 'string' ? error : (error.message ?? String(error.code ?? 'provider error'));

/**
 * Frames an SSE byte stream into `data:` payloads. Comment lines (`:`-prefixed,
 * OpenRouter's `: OPENROUTER PROCESSING` keepalive) are skipped, and a payload is
 * only emitted once its terminating blank line has arrived, so a chunk split across
 * two TCP reads is never parsed as truncated JSON.
 */
async function* sseData(body: ReadableStream<Uint8Array>): AsyncGenerator<string> {
  ...
  try {
    ...
    // A provider that closes without the final blank line still gets its last frame read.
    const tail = buffer
      .split(/\r?\n/)
      .filter((line) => line.startsWith('data:'))
      .map((line) => line.slice(5).trim())
...
const toolCallDeltas = (raw: RawToolCall[]): LlmToolCallDelta[] =>
  raw.map((call, position) => ({
    // `index` is the accumulation key; fall back to array position only if absent.
...
export async function* streamOpenAiSse(opts: SseStreamOptions): AsyncGenerator<LlmChunk> {
  ...
  if (!response.ok || !response.body) {
    ...
    throw new LlmProviderError(
      opts.providerId,
      `HTTP ${response.status} ${response.statusText}: ${detail.slice(0, 500)}`,
    );
  }
  ...
  for await (const data of sseData(response.body)) {
    ...
    // A mid-stream failure arrives as an HTTP-200 data event with a top-level `error`.
    if (chunk.error) throw new LlmProviderError(opts.providerId, errorText(chunk.error));
    ...
    const isUsageTail =
      opts.ignoreTrailingUsageFinish &&
      chunk.usage != null &&
      contentDelta === undefined &&
      toolCalls === undefined;

    const finishReason =
      rawFinish && !(finished && isUsageTail) ? (FINISH_REASONS[rawFinish] ?? 'stop') : undefined;
