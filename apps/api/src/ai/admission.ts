...
export const acquireSlot = async (conversationId: string): Promise<void> => {
    ...
    const [granted, liveCount, earliest] = (await redis.eval(ACQUIRE, 1, keys.aiAgents, conversationId, String(now), String(LEASE_TTL_SECONDS), String(env.CONVOAI_MAX_CONCURRENT_AGENTS))) as [
        number,
        number,
        number
    ];
    ...
    if (granted !== 1) {
        ...
        logger.warn({ conversationId, liveCount, max: env.CONVOAI_MAX_CONCURRENT_AGENTS }, 'convoai admission refused: capacity exhausted');
        throw new AppError(503, 'ai_capacity', 'all voice agents are busy; the text assistant is available', { retryAfterSeconds });
    }
};
export const refreshSlot = async (conversationId: string): Promise<boolean> => {
    const now = Math.floor(Date.now() / 1000);
    const refreshed = (await redis.eval(REFRESH, 1, keys.aiAgents, conversationId, String(now), String(LEASE_TTL_SECONDS))) as number;
