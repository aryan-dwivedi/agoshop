/**
 * Stage-3 AI slice check — a tsx script, not a vitest suite, run against the live
 * Postgres and Redis with `LLM_PROVIDER=mock`:
 *
 *   node --env-file=.env --import tsx apps/api/src/ai/__checks__/ai.check.ts
 *
 * It asserts the properties the custom-LLM proxy exists to guarantee:
 *   1. a valid signed callback streams `chat.completion.chunk` frames ending in
 *      `data: [DONE]`, and `Authorization` is never consulted;
 *   2. a tampered signature, an expired `expires`, a signature minted for a different
 *      conversation, and a missing header each return 401, and an unknown conversation
 *      returns 404;
 *   3. a FRAGMENTED tool call yields the assistant `tool_calls` message BEFORE the
 *      tool result messages, with arguments correctly accumulated by `index`;
 *   4. replaying the WHOLE callback for the same `turn_id` (which re-runs the model and
 *      mints a fresh `call_…` id) replays the stored `aiToolCalls` result without a
 *      second cart mutation;
 *   5. a client disconnect aborts the provider stream and clears the keepalive timer.
 *   6. a stalled provider is aborted on deadline and receives a spoken fallback.
 *
 * Environment is set before any import so `env.ts` parses the intended config.
 */
...
// Dynamic on purpose: `env.ts` parses `process.env` at module evaluation, so a static
// import would bind the real provider before the two assignments above take effect.
...
const [liveRow] = (
  await db.execute<{ session_id: string; product_id: string }>(sql`
...
const cartQuantity = async (): Promise<number> => {
  const { rows } = await db.execute<{ q: number }>(sql`
...
/**
 * The check owns this cart line and this shopper's mock conversations for its
 * duration, so a previous run that was killed before its cleanup cannot skew the
 * quantity assertions. Both are removed again at the end.
 */
...
// `Promise.withResolvers` is Node-24 runtime but ES2024 lib; this workspace targets
// ES2023, so the executor form is what type-checks here.
...
const postCallback = (options: CallbackOptions): Promise<Response> => {
  ...
  const headers: Record<string, string> = {
    ...
    // Deliberately bogus: authorization must never be consulted.
...
const cleanup = async (): Promise<void> => {
  // Cascades ai_messages and ai_tool_calls.
...
try {
  ...
  console.log('\n1. a valid signed callback streams SSE and ends with [DONE]');
  {
    ...
    ok(
      ...
      chunks.every((c) => c.object === 'chat.completion.chunk'),
      chunks.length,
    );
    ...
    ok(
      ...
      chunks.at(-1)?.choices?.[0]?.finish_reason === 'stop',
      chunks.at(-1),
    );
  ...
  console.log('\n1b. the first model delta reaches speech before generation finishes');
  {
    ...
    ok(
      'first content delta arrived while the provider was still generating',
      firstDeltaMs < 500 &&
    ...
    body += decoder.decode();
    ok(
      'remaining content completed after the early delta',
      body.includes('while the rest is still generating') && body.includes('[DONE]'),
    );
  }
  ...
  console.log('\n2. signature, expiry and identity failures are all 401');
  {
    ...
    ok(
      'expired expires -> 401 (even correctly signed)',
    ...
    ok(
      'signature minted for another conversation -> 401',
      wrongConversationResponse.status === 401,
      wrongConversationResponse.status,
    );
    ...
    ok(
      'unknown conversation with a valid signature -> 404',
    ...
    ok(
      'absurdly-future expires -> 401',
      farFutureResponse.status === 401,
      farFutureResponse.status,
    );
  }

  // ---------------------------- 3. fragmented tool call, message ordering ----
  console.log('\n3. a fragmented tool call orders assistant tool_calls before tool results');
  {
    ...
    ok(
      'assistant tool_calls message precedes every tool result',
      assistantIndex !== -1 && firstToolIndex !== -1 && assistantIndex < firstToolIndex,
      { assistantIndex, firstToolIndex },
    );

    const assistant = messages[assistantIndex]!;
    ok(
      'assistant message kept its content',
    ...
    ok(
      'tool name survived fragmentation',
      call.function.name === 'get_product_details',
      call.function.name,
    );
    ...
    ok(
      ...
      result.executed[0]?.outcome === 'ok',
      result.executed[0]?.outcome,
    );
  }

  // -------------------------- 3b. wait-only replies are repaired in-turn ----
  console.log('\n3b. a wait-only assistant draft is repaired before the turn returns');
  {
    ...
    ok(
      'wait-only draft was not returned',
      !/one second|checking that/i.test(result.text),
      result.text,
    );
  ...
  // -------------------- 3c. textual tool protocol is recovered, never spoken ----
  console.log('\n3c. textual tool markup executes without leaking into voice output');
  {
    ...
    ok(
      'textual protocol was converted into a real delivery tool call',
      result.executed[0]?.name === 'check_delivery' && result.executed[0].outcome === 'ok',
      result.executed[0],
    );
    ok(
      'tool markup and arguments were not sent to speech',
      !spoken.includes('<tool_call>') &&
        !spoken.includes('check_delivery') &&
        !spoken.includes('product_id'),
      spoken,
    );
    ok(
      'the shopper received the delivery result in the same turn',
      spoken.includes('delivery is available to 560001') &&
        result.text.includes('delivery is available to 560001'),
      { spoken, text: result.text },
    );
  }

  // -------------------------------- 4. callback replay, no second mutation ----
  console.log('\n4. replaying the whole callback for one turn_id does not mutate twice');
  {
    ...
    ok(
      'first callback streamed',
      first.status === 200 && firstBody.includes('[DONE]'),
      first.status,
    );
    const afterFirst = await cartQuantity();
    ok('the cart was mutated exactly once', afterFirst === before + 1, { before, afterFirst });

    const toolCallRows = await db.execute<{ n: number; tool_call_id: string }>(sql`
    ...
    `);
    ok(
      'one aiToolCalls dedupe row was written',
      toolCallRows.rows[0]?.n === 1,
      toolCallRows.rows[0],
    );
    ...
    ok(
      'replayed callback streamed',
      second.status === 200 && secondBody.includes('[DONE]'),
      second.status,
    );
    ...
    const afterRows = await db.execute<{ n: number; tool_call_id: string }>(sql`
      select count(*)::int as n, min(tool_call_id) as tool_call_id
      from ai_tool_calls
      where conversation_id = cast(${conversationId} as uuid) and turn_id = ${turnId}
    `);
    ...
    ok(
      'the stored row kept the ORIGINAL tool_call_id (a fresh id did not create a new row)',
      afterRows.rows[0]?.tool_call_id === firstToolCallId,
      { firstToolCallId, now: afterRows.rows[0]?.tool_call_id },
    );
  }

  // --------------------------- 5. disconnect aborts provider and keepalive ----
  console.log('\n5. a client disconnect aborts the provider stream and clears the keepalive');
  {
    ...
    // The turn is parked inside the provider, so the only thing that can reach the
    // wire is the 5 s keepalive comment. Observing one proves the timer is armed —
    // a global timer count is confounded by the HTTP client's own request timers.
    ...
    ok(
      'a keepalive comment reached the wire while the turn was in flight',
      seen.includes(': keepalive'),
      seen.slice(0, 160),
    );
    ...
    // A surviving interval would raise ERR_STREAM_WRITE_AFTER_END on its next tick,
    // which the uncaught-exception path below would report.
    await sleep(6_000);
    ok('no write-after-end one full keepalive period after the disconnect', true);
  }

  // -------------------- 6. a provider failure is spoken, never an HTTP error ----
  console.log('\n6. a provider failure streams a graceful sentence instead of an HTTP error');
  {
    ...
    const errors = await db.execute<{ n: number }>(sql`
    ...
    ok(
      'a structured ai_error event was recorded before the fallback sentence',
      inStream || (errors.rows[0]?.n ?? 0) > 0,
      { inStream, drained: errors.rows[0]?.n },
    );
  }

  // ----------------------- 7. stalled provider has a bounded spoken fallback ----
  console.log('\n7. a stalled provider is aborted on deadline and receives a spoken fallback');
  {
    ...
    ok(
      'stalled turn receives the graceful spoken fallback',
      spoken.includes("couldn't reach that"),
      spoken,
    );
    ...
    ok(
      'spoken fallback arrived on the configured deadline',
      elapsedMs >= 5_000 && elapsedMs < 7_500,
      {
        elapsedMs,
      },
    );
  ...
  // ------------------------------------------- 8. admission control (leases) ----
  console.log('\n8. admission control refuses a slot past the configured maximum');
  const held: string[] = [];
  try {
    const max = env.CONVOAI_MAX_CONCURRENT_AGENTS;
    // Deterministic capacity needs an empty semaphore. This is a development check
    // against the local stack; it drops any lease held by a running agent, which is
    // exactly what the background sweeper would reconcile anyway.
  ...
  // ---------------------------------- 9. the labelled degraded text transport ----
  ...
  // -------------------- 9b. live state remains useful without captions/chat ----
  console.log('\n9b. live context identifies the host and line-up without claiming video vision');
  {
    ...
    ok(
      ...
      policy.includes('never more than two short sentences') &&
        ...
        policy.includes('short replies such as "yes", "no", "okay" and "thanks" as complete turns'),
      policy,
    );
  }

  // ------------------------------ 10. OpenRouter/OpenAI SSE reader hardening ----
  console.log('\n10. the HTTP providers survive OpenRouter\u2019s documented SSE quirks');
  {
    const serveOnce = async (
      ...
      const stub = createServer((_req, res) => {
        ...
        // Written in two halves so a frame is split across TCP reads.
    ...
    const stream =
      ...
      chunk({}, 'tool_calls') +
      // The trailing usage chunk repeats finish_reason with an empty delta.
