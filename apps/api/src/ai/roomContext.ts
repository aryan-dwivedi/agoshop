...
const recentTranscript = async (sessionId: string): Promise<string[]> => {
    ...
    return rows
        .reverse()
        .map((row) => `[${stamp(row.startMs)}] ${row.speaker === 'host' ? 'Host' : row.speaker}: ${clamp(row.text)}`);
};
const recentChat = async (sessionId: string): Promise<string[]> => {
    ...
    return rows
        .reverse()
        .map((row) => `${row.displayName}${row.role === 'seller' ? ' (the host)' : row.role === 'admin' ? ' (staff)' : ''}: ${clamp(row.text)}`);
};
export const buildRoomContextLines = async (sessionId: string): Promise<string[]> => {
    ...
    if (transcript.length > 0) {
        lines.push('What the host has said most recently, from live speech-to-text (auto-generated, ' +
            'so names and numbers may be misheard — never quote a price from here, look it up):', ...transcript);
    }
    if (chat.length > 0) {
        lines.push('The most recent public chat in this room. The shopper can see all of it, so ' +
            '"what did they ask?" or "answer the question in chat" refers to these lines:', ...chat);
    }
    return lines;
};
