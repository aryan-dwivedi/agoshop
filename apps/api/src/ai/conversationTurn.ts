...
const persistAssistantText = async (conversation: ConversationRecord, turnId: number, text: string): Promise<void> => {
...
const persistUserText = async (conversation: ConversationRecord, turnId: number, messages: ChatMessage[]): Promise<void> => {
    ...
    const [existing] = await db
        ...
        .from(aiMessages)
        .where(and(eq(aiMessages.conversationId, conversation.id), eq(aiMessages.turnId, turnId), eq(aiMessages.role, 'user')))
        .limit(1);
    if (existing)
        return;
    await db.insert(aiMessages).values({
...
const isDeferredOnlyReply = (text: string): boolean => DEFERRED_ONLY_REPLY.some((pattern) => pattern.test(text));
const parseTextualToolCalls = (text: string, idPrefix: string): PendingToolCall[] | null => {
    const trimmed = text.trim();
    if (!trimmed.toLowerCase().startsWith('<tool_call>'))
        return null;
    const bodies = [...trimmed.matchAll(/<tool_call>\s*([\s\S]*?)\s*<\/tool_call>/giu)].map((match) => match[1]?.trim() ?? '');
    if (bodies.length === 0 ||
    ...
    for (const [index, body] of bodies.entries()) {
        ...
        if (name === null) {
            ...
            const nested = envelope.function !== null &&
...
const DEFERRED_REPAIR_INSTRUCTION: ChatMessage = {
    role: 'system',
    content: 'Your prior draft only asked the shopper to wait and did not answer them. Replace it now ' +
...
const CLOSING_INSTRUCTION: ChatMessage = {
    role: 'system',
    content: 'Tool calls are disabled for this reply. Answer the shopper now, in their language, ' +
...
const forceClosingAnswer = async (provider: LlmProvider, input: ConversationTurnInput): Promise<string> => {
    ...
    try {
        for await (const chunk of provider.streamChat({
            ...
            if (chunk.contentDelta)
                closing += chunk.contentDelta;
        }
    }
    catch (err) {
        logger.warn({ err, conversationId: conversation.id, turnId }, 'forced closing round failed; falling back to the canned sentence');
    }
...
export const runConversationTurn = async (input: ConversationTurnInput): Promise<ConversationTurnResult> => {
    const { conversation, messages, turnId, signal, onText } = input;
    const provider = getProvider(conversation.provider);
    await persistUserText(conversation, turnId, messages).catch((err: unknown) => logger.warn({ err, conversationId: conversation.id }, 'user turn persist failed'));
    ...
    for (let round = 0; round < MAX_ROUNDS; round += 1) {
        ...
        const textualCalls = accumulator.size === 0 ? parseTextualToolCalls(roundText, `${turnId}_${round}`) : null;
        const malformedProtocol = textualCalls === null &&
            withholdingProtocol &&
            roundText.trimStart().toLowerCase().startsWith('<tool_call');
        const answerText = textualCalls !== null || malformedProtocol ? '' : roundText;
        const terminalWithoutTools = textualCalls === null && (finishReason !== 'tool_calls' || accumulator.size === 0);
