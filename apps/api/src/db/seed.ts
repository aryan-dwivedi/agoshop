...
/**
 * `npm run db:seed` — the demo fixture. Idempotent: it truncates every table, clears
 * the app's Redis keyspace, and reseeds, so re-running is always safe.
 *
 * The first 53 products preserve the title, brand, price, MRP, rating, spec sheet,
 * and images from their source listings. Another ~1,000 rows from the Luminati Walmart
 * sample CSV expand the catalog with real marketplace titles, images, specs, and prices
 * converted to INR paise (`MinorUnits`). Amazon, Lazada, and Shopee rows are filtered
 * to electronics and phones.
 *
 * Five storefronts, five separate seller LOGINS — `sellers.ownerUserId` differs per
 * storefront rather than pointing every one at a single demo account. That is what makes
 * the console's scoping observable: each seller signs in and sees only their own
 * sessions, products and analytics, and `?sellerId=` on a storefront they do not own is
 * refused instead of silently working because one account owned everything.
 *
 * What it deliberately produces, and why:
 *  - curated demo products plus Walmart, Amazon, Lazada, and Shopee online listings
 *  - `744101` (non-serviceable) and `682001`/`190001` (COD-ineligible), so the delivery
 *    and payment-option tools have real negative cases. Phones list above the COD
 *    ceiling, so `get_payment_options` also has a value-based refusal to report.
 *  - a wishlist row for shopper@demo.test on the `ready-to-go-live` featured product, so
 *    `WISHLIST5` is ELIGIBLE and its `not_stackable` suppression under `LIVE20` is a
 *    demonstrable outcome rather than a claim.
 *  - three paid orders for loyal@demo.test, so the `loyalty_3plus` segment is reachable.
 *  - eight live-session records: two scheduled shows and six completed shows with
 *    ready recordings. The five category videos are seeded as replays rather than
 *    pretending that a host is broadcasting prerecorded fixture media.
 *
 * Covers vs playback. Five category replays each use their own recorded video, and
 * four use a still cut from that same video by `scripts/make-sample-video.sh`.
 * `glow-live` keeps `coverImageUrl: null` on purpose, proving the replay tile's
 * video-frame fallback without starting a decoder for every other tile.
 *
 * Session slugs preserve the fixture-media names — `headphones-live`, `phones-live`,
 * `decor-live`, `gym-live`, `glow-live`, plus `scheduled`, `ready-to-go-live` and
 * `ended` — while status and recording fields determine whether each is live or replay.
 */
...
/** clamp(ceil(expectedPeakViewers / RTM_CHAT_SHARD_TARGET), 1, 49) — decision 12. */
...
/**
 * The same formula `domain/chat.ts` exports as `shardIndexFor`, inlined here on purpose:
 * a data-seeding script must not drag the Agora token/signaling module graph in just to
 * hash a user id. domain/chat.ts remains the runtime authority.
 */
...
type SeedVariant = {
  ...
  /** The listed MRP, not a derived markup: it is struck through in the UI. */
...
/**
 * One owner login per storefront. `seller@demo.test` keeps the first storefront so the
 * documented demo flow still signs in as it; the other four exist so console scoping is
...
/** Catalog-only marketplace sellers; operator logins remain limited to the five above. */
...
/** 56 real products: 20 electronics, 11 activewear, 10 home, 6 fitness, 6 beauty, 3 jewellery. */
const PRODUCTS: SeedProduct[] = [
  // ---------------- Pulse Audio — the under-Rs 5,000 shortlist the headphones room walks through ----------------
  {
  ...
  // ---------------- Cellverse — the under-Rs 45,000 line-up the phones room ranks ----------------
  {
  ...
  // ---------------- Casa Nido — the haul the decor room unpacks ----------------
  {
  ...
  // ---------------- FlexFit — the outfits and training kit the gym room builds ----------------
  {
  ...
  // ---------------- Glow Atelier — the routine the glow room builds ----------------
  {
...
const PINCODES: {
  ...
  // Serviceable but COD-ineligible — proves get_payment_options is pincode-aware.
  {
    pincode: '682001',
    city: 'Kochi',
    state: 'Kerala',
  ...
  // Not serviceable at all — the negative case check_delivery must report.
  {
    pincode: '744101',
    city: 'Port Blair',
...
/** The 20% live-session discount from the assignment, expressed as data (decision 5). */
const PROMOTIONS = [
  {
    code: 'LIVE20',
  ...
  {
    /**
     * The threshold is ₹500 because that is where this catalog's beauty lines actually
     * sit (₹220–₹711). A ₹999 floor would have made the rule unreachable, which reads
     * as a working promotion in the admin list and applies to nothing on the storefront.
     */
    code: 'BEAUTY15',
  ...
  {
    /**
     * A flat rule alongside the percent ones, so the evaluator's priority ordering and
     * its two `kind`s are both exercised by seeded data rather than only by tests.
     */
    code: 'AUDIO300',
...
/**
 * The replay's transcript. It is the audio show the `ended` session recorded, so the
 * prices quoted in it are the prices the catalog actually lists — a summary that cites a
 * figure the storefront contradicts is worse than no summary.
 */
...
const CHAT_FIXTURE: { user: 'shopper' | 'loyal' | 'admin'; text: string }[] = [
...
/** Clears only this app's Redis keyspace, so a shared local Redis is left intact. */
...
/**
 * Regenerates the media fixtures. It remembers the per-room source map an operator
 * installed, so this call reproduces the five recorded videos and their covers without
 * db:seed having to know any source paths.
 */
const runSampleVideoScript = (): boolean => {
  const script = join(repoRoot, 'scripts/make-sample-video.sh');
  try {
    execFileSync('bash', [script], {
      stdio: 'inherit',
      env: { ...process.env, RECORDING_LOCAL_DIR: env.RECORDING_LOCAL_DIR },
    });
  } catch (err) {
    console.warn(`seed: make-sample-video.sh failed (${(err as Error).message})`);
    return false;
  }
  return existsSync(join(env.RECORDING_LOCAL_DIR, 'sample-session.mp4'));
};
...
const seed = async (): Promise<void> => {
  ...
  const userRows = await db
    ...
    .values([
      ...
      // One login per storefront, so console scoping is observable rather than asserted.
      ...SELLERS.map((s) => ({
        email: s.ownerEmail,
        passwordHash,
  ...
  const promotionRows = await db
    ...
    .values(
      PROMOTIONS.map((p) => ({
        ...
        // `sellerSlugs` is seed vocabulary; the evaluator only knows `sellerIds`, so it
        // is resolved here. AUDIO300 says "audio" on the label, and scoping it to the
        // audio storefront is what stops it also discounting a Rs 38,999 phone.
        conditions: {
  ...
  await db.insert(t.checkoutPolicies).values({
    ...
    // Rs 5,000: every phone in the catalog lists above it, so the COD refusal
    // `get_payment_options` reports is a real order-value case, not only a pincode one.
  ...
  // ---------------- personalization ----------------
  // The `ready-to-go-live` featured product. This single row is what makes WISHLIST5
  // eligible, and therefore what makes its `not_stackable` suppression under LIVE20 an
  // observable outcome rather than a claim.
  ...
  const sessionSeeds = [
    {
      // Oldest of the category replays.
      slug: 'gym-live',
      sellerSlug: 'flexfit',
    ...
    {
      /**
       * The one replay seeded with NO cover art. Its tile therefore has nothing to
       * show but a frame decoded from the recording, making the fallback visible on
       * the replay shelf instead of only in a comment.
       */
      slug: 'glow-live',
    ...
    {
      /**
       * This completed room retains the historical host markdown, but replay pricing
       * correctly ignores it because the session is no longer live.
       */
    ...
    {
      // Most recent of the category replays.
      slug: 'decor-live',
    ...
    },
    {
      ...
      // No footage exists yet, so the cover is the line-up's own product shot.
    ...
    {
      // The demo starts this one (M5): scheduledFor is already in the past.
    ...
    .values(
      sessionSeeds.map((s) => {
        ...
        return {
          ...
          // Seeded recordings are playback-only. HLS delivery is attached only when a
          // host starts a scheduled show and the live tier changes.
          hlsUrl: null,
          hlsOriginKind: null,
  ...
  await db.insert(t.liveSessionProducts).values(
    sessionSeeds.flatMap((s) =>
      s.products.map((slug, index) => ({
        ...
        pinnedAt:
          s.featured === slug && s.startedAt ? new Date(s.startedAt.getTime() + 4 * MINUTE) : null,
      })),
    ),
  );

  /**
   * Seeds are addressed by slug, never by position: new sessions get appended over time
   * and a positional lookup would silently repoint the ended-session fixtures at them.
   */
  ...
  await db.insert(t.sessionTranscripts).values(
    TRANSCRIPT_FIXTURE.map((line, index) => ({
      ...
      startMs: index * 135_000,
      createdAt: new Date(endedStart + index * 135_000),
    })),
  );
  ...
  // ---------------- order history ----------------
  // Three paid orders for loyal@demo.test make the `loyalty_3plus` segment reachable.
  // The third carries a per-line liveSessionId, which is the only place live attribution
  // exists — there is deliberately no order-level session column. Every figure below is
  // the catalog's own listed price, so the totals reconcile against the product pages.
  ...
  // ---------------- analytics fixtures for the ended session ----------------
  // domain/analytics.ts reads these types. background.ts is the only emitter of
  // `viewer_sample`, which powers the per-minute viewer series.
  const analyticsRows: {
    occurredAt: Date;
    userId: string | null;
  ...
  for (const [index, uid] of [shopperId, loyalId, adminId, endedHostId].entries()) {
    analyticsRows.push({
      occurredAt: new Date(endedStart + index * 90_000),
      userId: uid,
      sessionId: endedId,
      productId: null,
      type: 'session_join',
  ...
  // A 45-minute session: one viewer sample a minute, peaking at 5.
  const viewerCurve = [
  ...
  // Pin windows: the featured Noise, then the boAt, then the Philips.
  ...
  // ---------------- Redis state ----------------
  // Status is cached for every seeded session. Recorded fixtures stay ended and do
  // not receive a delivery-tier key; only a show started through the host flow does.
  for (const s of sessionSeeds) {
    await redis.set(keys.sessionStatus(sessionId(s.slug)), s.status);
  }
  // Reactions are counted in Redis (decision 13); this is the ended session's total.
  await redis.hset(keys.sessionReactions(endedId), { '❤️': 128, '🔥': 64, '👏': 32, '😮': 11 });

  // ---------------- recorded playback and live-broadcast fixtures ----------------
  const mp4Present = runSampleVideoScript();
  if (!mp4Present) {
    console.warn(
  ...
  // ---------------- report ----------------
  const counts = await pool.query<{ table: string; rows: number }>(`
