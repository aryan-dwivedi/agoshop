...
export const withIdempotency = async <T>(userId: string, key: string | undefined, res: Response, handler: () => Promise<{
    status: number;
    body: T;
}>): Promise<void> => {
    if (!key) {
    ...
    try {
        const result = await handler();
        await redis.set(redisKey, JSON.stringify({ status: result.status, body: result.body }), 'EX', 86400);
        res.status(result.status).json(result.body);
    }
    catch (err) {
