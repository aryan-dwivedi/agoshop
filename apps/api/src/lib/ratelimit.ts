...
/**
 * Redis token bucket (Lua, so refill+take is atomic).
 *
 * Authenticated routes are limited PER USER. The IP bucket exists for
 * unauthenticated routes and is skipped for RATE_LIMIT_TRUSTED_CIDRS — the docker
 * network, health probes and the k6 generator all share one address, and an IP bucket
 * would measure the limiter instead of the API. The per-user bucket is never skipped.
 */
const TAKE = `
local key = KEYS[1]
...
type Budget = { perMinute: number; ipPerMinute?: number };
...
/** Budgets are sized for the documented k6 load profile — see docs/scale-and-capacity.md. */
