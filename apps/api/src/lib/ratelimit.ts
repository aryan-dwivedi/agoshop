...
const cidrMatches = (ip: string, cidr: string): boolean => {
    ...
    if (normalized === '::1')
        return cidr.startsWith('127.');
    if (!/^\d+\.\d+\.\d+\.\d+$/.test(normalized))
        return false;
    const mask = bits === 0 ? 0 : (0xffffffff << (32 - bits)) >>> 0;
    return (toInt(normalized) & mask) === (toInt(range) & mask);
};
export const isTrustedIp = (ip: string): boolean => env.RATE_LIMIT_TRUSTED_CIDRS.some((cidr) => cidrMatches(ip, cidr));
type Budget = {
    perMinute: number;
    ipPerMinute?: number;
};
const take = async (key: string, perMinute: number): Promise<number> => {
    const retry = (await redis.eval(TAKE, 1, key, String(perMinute), String(perMinute / 60), String(Date.now() / 1000))) as number;
    return retry;
};
export const rateLimit = (name: string, budget: Budget) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
