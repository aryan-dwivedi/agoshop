...
const recordingCutoff = (): Date => new Date(Date.now() - env.RECORDING_RETENTION_DAYS * 24 * 60 * 60 * 1000);
export const purgeExpiredRecordingFiles = async (): Promise<number> => {
    const expiredRecordings = await db
        ...
        .from(liveSessions)
        .where(and(isNotNull(liveSessions.recordingUrl), isNotNull(liveSessions.endedAt), lt(liveSessions.endedAt, recordingCutoff())));
    let removed = 0;
    ...
    if (removed > 0)
        logger.info({ removed }, 'recording files purged');
    return removed;
};
