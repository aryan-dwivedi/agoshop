...
import { evaluatePromotions, type CartDto, type CartLineDto, type CartNotice, type Promotion, type Surface, } from '@shop/shared';
...
const ensureCart = async (userId: string, exec: Executor = db): Promise<string> => {
  ...
  `);
    const id = rows[0]?.id;
    if (!id)
        throw notFound('cart_not_found');
    return id;
};
export const loadCartItems = async (cartId: string, exec: Executor = db): Promise<CartItemRow[]> => {
...
export const priceCartItem = async (userId: string, item: CartItemRow, orderSubtotalMinorUnits: number, now: Date, exec: Executor = db, promotionsOrUndefined?: Promotion[]): Promise<PricedLine> => {
    const promotions = promotionsOrUndefined ?? (await loadActivePromotions(exec === db ? undefined : exec));
    const surface: Surface = item.live_session_id ? 'live' : 'browse';
    const resolved = await resolveLineContext(userId, {
    ...
    return {
        ...
        liveDiscountActive: resolved.ctx.liveEligible && evaluation.applied.some((ap) => liveCodes.includes(ap.code)),
...
export const getCart = async (userId: string, exec: Executor = db): Promise<CartDto> => {
    ...
    const totals = priced.reduce((acc, p) => ({
    ...
    if (priced.some((p) => p.liveDiscountActive))
        notices.push('live_discount_active');
    if (priced.some((p) => p.liveSessionEnded))
        notices.push('live_discount_expired');
    if (totals.discountMinorUnits > 0)
        notices.push('promotion_applied');
...
export const addItem = async (userId: string, input: AddItemInput): Promise<CartDto> => {
    ...
    if (resolved.productId !== input.productId) {
        throw badRequest(requestedSession ? 'invalid_live_session_product' : 'invalid_variant', 'variant does not belong to the requested product');
    }
    if (requestedSession && (!resolved.sessionExists || !resolved.productAttachedToSession)) {
        throw badRequest('invalid_live_session_product', 'product is not featured in that live session');
    }
...
export const setQuantity = async (userId: string, itemId: string, quantity: number): Promise<CartDto> => {
...
export const clearCartWithin = async (tx: Tx, userId: string): Promise<void> => {
  ...
  `);
    await tx.execute(sql `update carts set updated_at = now() where user_id = cast(${userId} as uuid)`);
};
