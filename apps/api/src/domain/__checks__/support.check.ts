...
const ticketStatus = async (): Promise<{
    status: string;
    assignedAgentId: string | null;
}> => {
...
const run = async (): Promise<void> => {
  ...
  `);
    ...
    try {
        ...
        const accepted = await support.request<AcceptBody>('POST', `/api/support/tickets/${ticketId}/accept`);
        ...
        const shopperCannotActivate = await shopper.request('POST', `/api/support/tickets/${ticketId}/connected`);
        ...
        const duplicateConnected = await support.request('POST', `/api/support/tickets/${ticketId}/connected`);
