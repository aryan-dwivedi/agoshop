...
import { LOW_STOCK_THRESHOLD } from '../analytics.js';
...
/**
 * Stage-2 acceptance check for the seller console's create path. A tsx script, not
 * vitest: it boots a real app against the real Postgres and Redis, signs in as the
 * DEMO SELLER (the seed owns the personas; this check owns nothing but the rows it
 * creates), exercises POST /api/seller/products over HTTP, and cleans up after itself.
 *
 *   node --env-file=.env --import tsx apps/api/src/domain/__checks__/seller.check.ts
 *
 * Prerequisites: `npm run stack:up`, `npm run db:push`, `npm run db:seed`.
 */
...
/**
 * A single alphanumeric run so the Postgres text parser emits ONE token: the search
 * assertion below leans on it to prove the FTS expression index covers rows this run
 * inserted. It also makes every title, slug and SKU unique per run, so the check stays
 * green on a re-run without a reseed.
 */
const MARK = `zephyrine${randomUUID().replace(/-/g, '').slice(0, 10)}`;
const TITLE = `Zephyrine ${MARK} Stovetop Kettle`;

const PRICE_SMALL = 249_900; // ₹2,499.00
const PRICE_LARGE = 329_900; // ₹3,299.00
const STOCK_SMALL = 12;
const STOCK_LARGE = 5;

type Fixtures = {
  /** A seller owned by nobody, so the demo seller provably cannot list under it. */
...
const cleanup = async (): Promise<void> => {
  ...
  if (fixtures.foreignSellerId) {
    await db.execute(sql`delete from sellers where id = cast(${fixtures.foreignSellerId} as uuid)`);
  }
  // The listings just deleted are cached under the same keys the endpoint invalidates.
...
type CreatedBody = { product: SellerProductDto };
type InventoryBody = {
  lowStockThreshold: number;
  sellerId: string;
  sellers: { id: string; slug: string; displayName: string }[];
...
/** The body the happy path posts; every negative case is this with one field bent. */
...
const run = async (): Promise<void> => {
  ...
  try {
    ...
    // --- the seller creates a two-variant listing --------------------------------
    const seller = makeClient(base);
    await login(seller, SELLER_EMAIL);
    pass(`POST /api/auth/login signs in ${SELLER_EMAIL}`);

    const created = await seller.request<CreatedBody>('POST', '/api/seller/products', createBody());
    ...
    const basePrice = await one<{ base_price_minor_units: number }>(sql`
    ...
    // --- the console lists it, and so does the storefront ------------------------
    ...
    const pdp = await seller.request<{ product: { id: string; slug: string } }>(
      'GET',
    ...
    const search = await seller.request<{ items: { id: string }[]; total: number }>(
      'GET',
      `/api/products?q=${MARK}&sort=relevance`,
    );
    ...
    // --- a second listing with the same title gets a readable -2 slug ------------
    const twin = await seller.request<CreatedBody>(
      'POST',
      '/api/seller/products',
      createBody({
        variants: [
          { label: '1.5 litre', sku: `${MARK}-twin-a`, priceMinorUnits: PRICE_SMALL, stock: 3 },
        ],
      }),
    );
    ...
    const duplicate = await seller.request<ErrorBody>(
      ...
      createBody({
        ...
        variants: [{ label: 'Clash', sku: takenSku, priceMinorUnits: PRICE_SMALL, stock: 1 }],
      }),
    );
    ...
    const orphans = await one<{ n: number }>(sql`
