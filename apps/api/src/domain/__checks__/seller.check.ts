...
const run = async (): Promise<void> => {
    ...
    try {
        ...
        assert.equal(product.variants.filter((v) => v.isDefault).length, 1, 'exactly one variant is the default');
        ...
        assert.equal(product.priceMinorUnits, PRICE_SMALL, 'the listed price is the CHEAPEST variant, not the default one');
        ...
        assert.equal(new Set(product.variants.map((v) => v.sku)).size, 2, 'derived SKUs must differ per variant');
    ...
    `);
        assert.equal(basePrice.base_price_minor_units, PRICE_SMALL, 'products.basePriceMinorUnits is the minimum variant price');
        ...
        assert.ok(search.body.items.some((i) => i.id === product.productId), 'the FTS expression index must cover rows inserted after the index was built');
        pass('GET /api/products?q= matches the new listing through the FTS index');
        const twin = await seller.request<CreatedBody>('POST', '/api/seller/products', createBody({
            variants: [
        ...
        assert.equal(twin.body.product.variants[0]?.isDefault, true, 'a lone unflagged variant becomes the default');
        ...
        const refusedShopper = await shopper.request<ErrorBody>('POST', '/api/seller/products', createBody({ title: `${TITLE} Shopper Attempt` }));
        ...
        const refusedAnonymous = await anonymous.request<ErrorBody>('POST', '/api/seller/products', createBody({ title: `${TITLE} Anonymous Attempt` }));
        ...
        const foreign = await seller.request<ErrorBody>('POST', '/api/seller/products', createBody({ sellerId: fixtures.foreignSellerId, title: `${TITLE} Foreign Attempt` }));
        ...
        assert.ok(takenSku);
        const duplicate = await seller.request<ErrorBody>('POST', '/api/seller/products', createBody({
    ...
    `);
        ...
        const badMrp = await seller.request<ErrorBody>('POST', '/api/seller/products', createBody({
            title: `${TITLE} Bad MRP`,
            variants: [
                {
        ...
        const badCategory = await seller.request<ErrorBody>('POST', '/api/seller/products', createBody({ title: `${TITLE} Bad Category`, categorySlug: `${MARK}-nope` }));
        ...
        const twoDefaults = await seller.request<ErrorBody>('POST', '/api/seller/products', createBody({
