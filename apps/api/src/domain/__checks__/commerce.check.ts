...
const cleanup = async (): Promise<void> => {
    ...
    if (users.length > 0) {
        const ids = sql.join(users.map((u) => sql `cast(${u} as uuid)`), sql `, `);
        await db.execute(sql `delete from promotion_redemptions where user_id in (${ids})`);
        await db.execute(sql `delete from order_items where order_id in (select id from orders where user_id in (${ids}))`);
        await db.execute(sql `delete from orders where user_id in (${ids})`);
        await db.execute(sql `delete from cart_items where cart_id in (select id from carts where user_id in (${ids}))`);
    ...
    if (fixtures.policyId) {
        await db.execute(sql `delete from checkout_policies where id = cast(${fixtures.policyId} as uuid)`);
    }
    if (fixtures.pincodeInserted) {
        await db.execute(sql `delete from pincodes where pincode = '560001'`);
    }
    if (fixtures.liveSessionId) {
        await db.execute(sql `delete from live_session_products where session_id = cast(${fixtures.liveSessionId} as uuid)`);
        await db.execute(sql `delete from live_sessions where id = cast(${fixtures.liveSessionId} as uuid)`);
    }
    for (const productId of [fixtures.productId, fixtures.otherProductId]) {
        ...
        await db.execute(sql `delete from product_variants where product_id = cast(${productId} as uuid)`);
...
const makeClient = (base: string): Client => {
    const client: Client = {
        cookie: null,
        request: async <T>(method: string, path: string, body?: unknown, headers: Record<string, string> = {}): Promise<ApiResponse<T>> => {
...
const run = async (): Promise<void> => {
    ...
    `);
        ...
        assert.ok(search.body.items.some((i) => i.id === fixtures.productId), 'full-text search must find the fixture product');
        ...
        for (const [term, source] of [
            ...
            assert.ok(discovery.body.items.some((i) => i.id === fixtures.productId), `catalog search must find the fixture by ${source}`);
        }
        pass('GET /api/products?q= also matches seller, category, and variant SKU');
        const first = await api.request<CartDto>('POST', '/api/cart/items', { productId: fixtures.productId, variantId: fixtures.variantId, quantity: 1 }, { 'Idempotency-Key': `${TAG}-add-1` });
        assert.equal(first.status, 200, JSON.stringify(first.body));
        const second = await api.request<CartDto>('POST', '/api/cart/items', { productId: fixtures.productId, variantId: fixtures.variantId, quantity: 1 }, { 'Idempotency-Key': `${TAG}-add-2` });
    ...
    `);
        ...
        const extra = await api.request<CartDto>('POST', '/api/cart/items', { productId: fixtures.otherProductId, variantId: fixtures.otherVariantId, quantity: 1 }, { 'Idempotency-Key': `${TAG}-add-other` });
        ...
        const mismatch = await api.request<ErrorBody>('POST', '/api/cart/items', {
        ...
        const liveAdd = await api.request<CartDto>('POST', '/api/cart/items', {
        ...
        const declined = await api.request<ErrorBody>('POST', '/api/orders', {
            paymentMethod: 'card',
            pincode: '560001',
        ...
        const orderCount = await countRows(sql `select count(*)::int as n from orders where user_id = cast(${userId} as uuid)`);
        ...
        const declineRowsFor = async (): Promise<number> => countRows(sql `
      ...
      where type = 'payment_declined' and user_id = cast(${userId} as uuid)
    `);
        assert.ok(!JSON.stringify(declineRow.payload).includes('4111'), 'decline telemetry must never carry card data');
        ...
        const paid = await api.request<OrderBody>('POST', '/api/orders', {
            paymentMethod: 'card',
            pincode: '560001',
        ...
        const liveAttributed = paid.body.order.items.filter((i) => i.liveSessionId === fixtures.liveSessionId);
        ...
        const replay = await api.request<OrderBody>('POST', '/api/orders', { paymentMethod: 'card', pincode: '560001', card: { number: '4111111111111111' } }, { 'Idempotency-Key': `${TAG}-order-paid` });
        ...
        assert.ok(new TextDecoder().decode(chunk?.value).includes(': connected'), 'the stream must open with a comment frame');
        sseAbort.abort();
        ...
        await db.execute(sql `update users set role = 'admin' where id = cast(${adminUser.body.user.id} as uuid)`);
        ...
        }>('PATCH', `/api/admin/promotions/${created.body.promotion.id}`, { active: false, value: 20000 });
        ...
        }>('PATCH', '/api/admin/checkout-policy', { minOrderMinorUnits: policy.body.policy.minOrderMinorUnits });
        assert.equal(policyPatched.status, 200, JSON.stringify(policyPatched.body));
        assert.equal(policyPatched.body.policy.minOrderMinorUnits, policy.body.policy.minOrderMinorUnits);
        ...
        assert.ok(recommended.every((p) => p.id !== fixtures.productId), 'recommendations never include the product they are based on');
        ...
        const leftovers = await countRows(sql `select count(*)::int as n from wishlist_items where user_id = cast(${erasedId} as uuid)`);
