...
import { drainAnalyticsOnce } from '../../background.js';
...
import { router as eventsRouter } from '../../routes/eventsStub.js';
import { router as meRouter } from '../../routes/me.js';
import { router as ordersRouter } from '../../routes/orders.js';
import { router as wishlistRouter } from '../../routes/wishlist.js';

/**
 * Stage-2 acceptance check. A tsx script, not vitest: it boots a real app against the
 * real Postgres and Redis, inserts its own minimal fixtures (the demo seed is owned
 * elsewhere), exercises the commerce slice over HTTP, and cleans up after itself.
 *
 *   node --env-file=.env --import tsx apps/api/src/domain/__checks__/commerce.check.ts
 */

const TAG = `chk-${randomUUID().slice(0, 8)}`;
const PRICE = 250_000; // ₹2,500.00
const STOCK = 7;
...
const one = async <T extends Record<string, unknown>>(statement: SQL): Promise<T> => {
  ...
  // drizzle widens the row type through `Assume`; this script owns both sides of the query.
...
const insertFixtures = async (): Promise<void> => {
  ...
  const variant = await one<{ id: string }>(sql`
  ...
  // A second product deliberately NOT attached to the live session below.
  const otherProduct = await one<{ id: string }>(sql`
    insert into products (slug, category_id, seller_id, title, brand, description,
  ...
  const otherVariant = await one<{ id: string }>(sql`
    insert into product_variants (product_id, sku, label, price_minor_units, stock, is_default)
  ...
  const promotion = await one<{ id: string }>(sql`
  ...
  // Checkout needs an active policy row; create one only if the real seed has not.
  const { rows: policies } = await db.execute<{ id: string }>(
    sql`select id from checkout_policies where active = true limit 1`,
  );
  if (policies.length === 0) {
    const policy = await one<{ id: string }>(sql`
  ...
  // The order path needs a serviceable pincode; add 560001 only if it is missing.
...
const cleanup = async (): Promise<void> => {
  ...
  if (fixtures.promotionId) {
    await db.execute(sql`delete from promotions where id = cast(${fixtures.promotionId} as uuid)`);
  }
  ...
  for (const productId of [fixtures.productId, fixtures.otherProductId]) {
    ...
    await db.execute(sql`delete from product_views where product_id = cast(${productId} as uuid)`);
    await db.execute(
  ...
  if (fixtures.categoryId) {
    await db.execute(sql`delete from categories where id = cast(${fixtures.categoryId} as uuid)`);
  }
  await redis.del(keys.promotionsCache, keys.checkoutPolicyCache);
};
...
type HealthBody = { status: string; db: string; redis: string; version: string };
type UserBody = { user: PublicUser };
type OrderBody = { order: OrderDto };
...
const makeClient = (base: string): Client => {
  const client: Client = {
    ...
    request: async <T>(
      ...
      // The check asserts against its own server's documented shapes.
...
const countRows = async (statement: SQL): Promise<number> => {
  const row = await one<{ n: number }>(statement);
  return row.n;
};
...
const run = async (): Promise<void> => {
  ...
  try {
    ...
    // --- catalog: PDP records a view, search finds the fixture ------------------
    const pdp = await api.request<{ product: { id: string; variants: { id: string }[] } }>(
      'GET',
      `/api/products/${TAG}-product`,
    );
    ...
    const search = await api.request<{ items: { id: string }[]; total: number }>(
      'GET',
      '/api/products?q=CheckBrand%20Widget&sort=relevance',
    );
    ...
    for (const [term, source] of [
      ...
      const discovery = await api.request<{ items: { id: string }[] }>(
        'GET',
        `/api/products?q=${encodeURIComponent(term)}&sort=relevance`,
      );
    ...
    // --- two browse adds coalesce into ONE line (UNIQUE NULLS NOT DISTINCT) -----
    ...
    // --- PATCH and DELETE on a cart line ----------------------------------------
    ...
    const extraLineId = extra.body.items.find((i) => i.productId === fixtures.otherProductId)?.id;
    ...
    // --- mismatched (liveSessionId, productId) is rejected 400 ------------------
    ...
    // --- the attached product IS live-eligible and gets the live discount -------
    ...
    const options = await api.request<{ methods: string[]; blockedReason: string | null }>(
      'GET',
      '/api/checkout/options?pincode=560001',
    );
    ...
    // --- a failing card writes no order and leaves stock unchanged --------------
    ...
    /**
     * A decline is the one checkout outcome with no row of its own, so the telemetry
     * write IS the contract: without it a seller cannot tell a declined card from a
     * shopper who changed their mind. Ingestion is a Redis stream, so this drains it
     * itself rather than assuming a background process is running — the first drain
     * may only create the consumer group, and a live background process may win the
     * race, so the loop tolerates both.
     */
    ...
    const declineRow = await one<{ payload: Record<string, unknown> }>(sql`
    ...
    // --- a good card succeeds, decrements stock and clears the cart -------------
    ...
    if (paid.body.order.status === 'pending') {
      ...
      const settled = await api.request<OrderBody>('GET', `/api/orders/${paid.body.order.id}`);
    ...
    assert.equal(replay.body.order.id, paid.body.order.id, 'the same key replays the same order');
    assert.equal(await stockOf(fixtures.variantId), stockBefore - 3, 'a replay decrements nothing');
    pass('replayed Idempotency-Key returns the original order without touching stock');

    // --- order history and detail -----------------------------------------------
    const history = await api.request<{ orders: OrderDto[] }>('GET', '/api/orders');
    ...
    // --- wishlist ---------------------------------------------------------------
    const wishAdd = await api.request<{ added: boolean }>('POST', '/api/wishlist', {
      productId: fixtures.productId,
    });
    assert.equal(wishAdd.status, 200, JSON.stringify(wishAdd.body));
    assert.equal(wishAdd.body.added, true);
    const wishList = await api.request<{ items: { id: string }[] }>('GET', '/api/wishlist');
    assert.ok(wishList.body.items.some((p) => p.id === fixtures.productId));
    const wishDel = await api.request<{ removed: boolean }>(
      'DELETE',
      `/api/wishlist?productId=${fixtures.productId}`,
    );
    assert.equal(wishDel.body.removed, true);
    pass('wishlist add / list / delete round trip');

    // --- SSE stream opens with the documented headers ---------------------------
    ...
    // The session record caches the role, so re-login after the promotion.
    await adminApi.request<UserBody>('POST', '/api/auth/login', {
      email: adminEmail,
      password: 'check1234',
    });

    const rules = await adminApi.request<{ promotions: { id: string; code: string }[] }>(
      'GET',
    ...
    const created = await adminApi.request<{ promotion: { id: string; active: boolean } }>(
      'POST',
      '/api/admin/promotions',
      {
        code: createdCode,
    ...
    const promoPatched = await adminApi.request<{ promotion: { active: boolean; value: number } }>(
      'PATCH',
      `/api/admin/promotions/${created.body.promotion.id}`,
      { active: false, value: 20_000 },
    );
    ...
    assert.equal(promoPatched.body.promotion.value, 20_000);

    const policy = await adminApi.request<{ policy: { id: string; minOrderMinorUnits: number } }>(
      'GET',
      '/api/admin/checkout-policy',
    );
    assert.equal(policy.status, 200, JSON.stringify(policy.body));
    // Re-assert the same value: proves the write path without changing seeded rules.
    const policyPatched = await adminApi.request<{ policy: { minOrderMinorUnits: number } }>(
      'PATCH',
      '/api/admin/checkout-policy',
    ...
    // --- the domain contract VoiceAI consumes -----------------------------------
    ...
    await erasedApi.request<{ added: boolean }>('POST', '/api/wishlist', {
      productId: fixtures.productId,
    });
    const erasure = await erasedApi.request<{ erased: boolean }>('DELETE', '/api/me/data');
    ...
    const tombstone = await one<{ email: string; display_name: string; deleted_at: string | null }>(
