...
/**
 * Live-slice invariants against the REAL Postgres and Redis.
 *
 * Not a vitest file on purpose: the properties under test are concurrency properties of
 * conditional SQL writes and a Redis compare-and-set, so they need genuine parallel
 * calls against real infrastructure rather than a mocked harness.
 *
...
/**
 * Outbound HTTP is replaced with a counting stub for the whole run. Two reasons:
 * the "no publish attempted" assertion must be about ATTEMPTS (not about whether Agora
 * answers), and the run must not depend on Signaling REST credentials that smoke check
 * S5 has not yet confirmed.
 */
...
/**
 * Lets a Redis pub/sub round trip land before a subscriber count is asserted.
 * Executor form because the workspace `lib` is ES2023 — `Promise.withResolvers`
 * is ES2024 and does not typecheck here.
 */
...
/**
 * Chat that could not go out over Signaling REST is republished over SSE as an
 * explicitly labelled degraded transport, so these events are the observable proof
 * that a message did — or did not — reach the publish stage at all.
 */
const chatEvents: ServerEvent[] = [];

const degradedMessageIds = (): string[] =>
  chatEvents.flatMap((event) => {
    const data = event.data;
    if (data === null || typeof data !== 'object') return [];
    if (!('action' in data) || data.action !== 'degraded_chat') return [];
    if (!('envelope' in data)) return [];
...
/** Pin and unpin ride the same event; `null` is the unpin signal both tiers act on. */
const pinnedProductIds = (): (string | null)[] =>
  pinEvents.flatMap((event) => {
    const data = event.data;
    if (data === null || typeof data !== 'object' || !('productId' in data)) return [];
...
const cleanup = async (sessionId: string | null, userIds: string[], sellerId: string | null) => {
  if (sessionId) {
    ...
    await redis.del(
      ...
      keys.sessionTier(sessionId),
      keys.sessionViewers(sessionId),
      keys.sessionReactions(sessionId),
      `${keys.sessionReactions(sessionId)}:delta`,
      keys.sessionBans(sessionId),
      keys.sessionMutes(sessionId),
    );
    // The end transition appends to the summary stream; drop only our own entry.
    const entries = await redis.xrange(keys.summaryStream, '-', '+');
    const mine = entries.filter(([, fields]) => fields.includes(sessionId)).map(([id]) => id);
    if (mine.length > 0) await redis.xdel(keys.summaryStream, ...mine);
  }
...
const run = async (): Promise<void> => {
  ...
  const [chatterUser] = await db
    ...
    .returning({ id: users.id });
  if (!hostUser || !chatterUser) throw new Error('fixture users not created');
  ...
  if (!seller) throw new Error('fixture seller not created');
  ...
  const [session] = await db
    ...
    .values({
      ...
      // 6 expected peak viewers / RTM_CHAT_SHARD_TARGET is the frozen shard count.
      expectedPeakViewers: 6,
    })
    .returning({ id: liveSessions.id });
  if (!session) throw new Error('fixture session not created');
  ...
  listener.on('message', (_channel, payload) => {
    ...
    if (event.event === EVENTS.sessionDeliveryTierChanged) tierEvents.push(event);
    if (event.event === EVENTS.sessionStatusChanged) statusEvents.push(event);
    if (event.event === EVENTS.chatModerated) chatEvents.push(event);
    if (event.event === EVENTS.sessionProductPinned) pinEvents.push(event);
  });

  console.log(`fixture session ${sessionId} (${slug})`);
  console.log(
    `config: RTC_TIER_MAX_VIEWERS=${env.RTC_TIER_MAX_VIEWERS} RTM_CHAT_SHARD_TARGET=${env.RTM_CHAT_SHARD_TARGET}`,
  );

  try {
    /* ------------------------------------------- 1. concurrent start is once */

    console.log('\n1. ten concurrent start calls');
    const expectedShards = frozenShardCount(6);
    const starts = await Promise.allSettled(
      Array.from({ length: 10 }, () => startSession(sessionId, { consentAcknowledged: true })),
    );
    ...
    check(
      'every start response reports the session live',
      startOk.every((r) => r.value.session.status === 'live'),
    );
    ...
    check(
      `chatShardCount frozen at ${expectedShards}`,
      afterStart?.chatShardCount === expectedShards,
      afterStart?.chatShardCount,
    );
    check(
      'redis status key written by the winner',
      (await redis.get(keys.sessionStatus(sessionId))) === 'live',
    );
    check(
      'redis tier key initialised to rtc',
      (await redis.get(keys.sessionTier(sessionId))) === 'rtc',
    );

    await settle(150);
    check(
      'exactly one session.status_changed published',
      statusEvents.length === 1,
      statusEvents.length,
    );

    /* ------------------------------- 2. tier CAS flips once and never back */
    ...
    const joins = await Promise.all(
      viewerIds.map((userId) => joinSession(sessionId, { userId, role: 'shopper' })),
    );
    check(
      'captions remain enabled while live transcription connects',
      joins.every((join) => join.captionsEnabled),
    );
    await db
      .update(liveSessions)
    ...
    check(
      `pruned viewer count reached the threshold (${pruned})`,
      pruned >= env.RTC_TIER_MAX_VIEWERS,
      pruned,
    );
    check(
      'exactly one session.delivery_tier_changed published',
      tierEvents.length === 1,
      tierEvents.length,
    );
    ...
    check(
      'the transition event carries an hls origin and its honest label',
      tierPayloadOk,
      tierData,
    );
    ...
    check(
      'at least one join already observed the cdn tier',
      joins.some((j) => j.deliveryTier === 'cdn'),
    );
    check(
      `every join reports the frozen chatShardCount (${expectedShards})`,
      joins.every((j) => j.chatShardCount === expectedShards),
      joins.map((j) => j.chatShardCount),
    );

    const laterJoins = await Promise.all(
      Array.from({ length: 5 }, () =>
        joinSession(sessionId, { userId: randomUUID(), role: 'shopper' }),
      ),
    );
    await settle(150);
    check(
      'later joiners read the stored cdn tier',
      laterJoins.every((j) => j.deliveryTier === 'cdn'),
    );
    check(
      'the tier never reverted and never re-published',
      tierEvents.length === 1,
      tierEvents.length,
    );

    /* ---------------------------------------- 3. shard index is view-stable */
    ...
    check(
      `viewer count actually changed (${countBefore} -> ${countAfter})`,
    ...
    check(
      'shardIndex matches the pure formula against the frozen count',
      after.shardIndex === shardIndexFor(probeUser, expectedShards),
    );
    ...
    if (!seededProduct) throw new Error('no seeded product to pin — run db:seed first');
    await setSessionProducts(sessionId, [{ productId: seededProduct.id }]);

    const pinned = await pinProduct(sessionId, seededProduct.id);
    await settle(150);
    check(
      'pin marks exactly one product featured with a pinnedAt',
      pinned.filter((p) => p.isFeatured && p.pinnedAt !== null).length === 1,
      pinned,
    );
    check(
      'pin publishes session.product_pinned with the productId',
      pinnedProductIds().at(-1) === seededProduct.id,
      pinnedProductIds(),
    );
    ...
    await settle(150);
    check(
      'unpin clears isFeatured and pinnedAt for the session',
      unpinned.every((p) => !p.isFeatured && p.pinnedAt === null),
      unpinned,
    );
    check(
      'unpin publishes session.product_pinned with productId null',
      pinnedProductIds().at(-1) === null,
      pinnedProductIds(),
    );
    check('no featured product remains', (await featuredProductId(sessionId)) === null);

    /* ------------------------------------- 3c. host-shared product chat card */
    ...
    check(
      'host chat envelope carries the attached session product',
      shared.message.product?.productId === seededProduct.id,
      shared.message.product,
    );
    const history = await listChatMessages(sessionId, 50);
    check(
      'late-join chat history restores the product card',
      history.find((message) => message.messageId === sharedMessageId)?.product?.productId ===
        seededProduct.id,
    );
    ...
    try {
      ...
      if (err instanceof AppError) viewerShareCode = err.code;
      else throw err;
    }
    check('viewers cannot attach product cards', viewerShareCode === 'not_session_host');

    /* ------------------------------------- 4. a muted user cannot publish */
    ...
    await settle(150);
    // Whichever transport carried it, the accepted message DID reach the publish stage.
    // Asserting that here is what makes the negative assertion below non-vacuous.
    check(
      `the accepted message reached the publish stage via ${accepted.transport}`,
      accepted.transport === 'rtm-rest'
        ? fetchCount > 0
    ...
    check(
      'mute recorded in redis',
      (await redis.sismember(keys.sessionMutes(sessionId), probeUser)) === 1,
    );
    ...
    check(
      'no degraded-transport publish attempted either',
      !degradedMessageIds().includes(`post-mute-${tag}`),
      degradedMessageIds(),
    );
    const blockedRows = await db
      ...
      .where(
        and(
          ...
          eq(chatMessages.clientMessageId, `post-mute-${tag}`),
        ),
      );
    check(
      'nothing persisted for the blocked message',
      blockedRows.length === 0,
      blockedRows.length,
    );

    /* --------------------------------------------- 5. concurrent end is once */
    ...
    check(
      'exactly one live -> ended transition',
      endTransitions.length === 1,
      endTransitions.length,
    );
    ...
    check(
      'redis status key is ended',
      (await redis.get(keys.sessionStatus(sessionId))) === 'ended',
    );
    check(
      'the frozen shard count survived the whole lifecycle',
      afterEnd?.chatShardCount === expectedShards,
      afterEnd?.chatShardCount,
    );
    check(
      'the tier still did not revert',
      afterEnd?.deliveryTier === 'cdn',
      afterEnd?.deliveryTier,
    );
    await settle(150);
    check(
      'exactly one session.status_changed on end',
      statusEvents.length === 1,
      statusEvents.length,
    );
    ...
    check(
      'exactly one session-summary job appended',
      summaryEntries.filter(([, fields]) => fields.includes(sessionId)).length === 1,
    );
  } finally {
