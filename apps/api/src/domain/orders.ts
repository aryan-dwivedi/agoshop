...
import { EVENTS, evaluatePromotions, type AppliedPromotion, type OrderDto, type PaymentMethod, type Surface, } from '@shop/shared';
...
export const priceCart = async (userId: string, now: Date, exec: Executor = db): Promise<Priced> => {
    const cartId = await cartIdFor(userId, exec);
    if (!cartId)
        throw badRequest('cart_empty', 'cart is empty');
...
export const createOrder = async (userId: string, idempotencyKey: string, input: CreateOrderInput): Promise<OrderDto> => {
    ...
    if (options.blockedReason)
        throw badRequest(options.blockedReason);
    ...
    try {
      ...
      `);
            ...
            if (!insertedId)
                throw new AppError(500, 'order_insert_failed');
        ...
        logger.warn({ err, paymentRef: preauth.paymentRef, userId }, 'checkout aborted; pre-authorization voided');
        throw err;
    }
    await invalidateCatalogCache();
    for (const ap of finalApplied)
        promotionAppliedTotal.inc({ code: ap.code });
...
export const listOrders = async (userId: string): Promise<OrderDto[]> => {
    const { rows } = await db.execute<OrderRow>(sql `${ORDER_PROJECTION} where o.user_id = cast(${userId} as uuid) order by o.created_at desc`);
    return rows.map(toOrderDto);
};
export const getOrder = async (userId: string, orderId: string): Promise<OrderDto | null> => {
    const { rows } = await db.execute<OrderRow>(sql `${ORDER_PROJECTION} where o.id = cast(${orderId} as uuid) and o.user_id = cast(${userId} as uuid)`);
    const row = rows[0];
...
export const listSellerOrders = async (sellerIds: string[], limit: number = SELLER_ORDERS_DEFAULT_LIMIT): Promise<OrderDto[]> => {
    if (sellerIds.length === 0)
        return [];
    const owners = sql.join(sellerIds.map((id) => sql `cast(${id} as uuid)`), sql `, `);
