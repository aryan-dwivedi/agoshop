...
const loadTranscript = async (conversationId: string): Promise<{
    role: string;
    text: string;
    at: string;
}[]> => {
    const rows = await db
        ...
        .from(aiMessages)
        .where(and(eq(aiMessages.conversationId, conversationId), inArray(aiMessages.role, ['user', 'assistant'])))
        .orderBy(asc(aiMessages.id))
        .limit(40);
    return rows.flatMap((r) => r.content ? [{ role: r.role, text: r.content, at: r.createdAt.toISOString() }] : []);
};
export const escalateToHuman = async (input: {
    ...
    const existing = await db
        ...
        .from(supportTickets)
        .where(and(eq(supportTickets.conversationId, input.conversationId), inArray(supportTickets.status, ['queued', 'assigned', 'active'])))
        .limit(1);
    if (existing[0]) {
        return {
    ...
    return {
        ...
        message: input.preference === 'callback'
...
export const acceptSupportTicket = async (ticketId: string, agentUserId: string): Promise<{
    ...
    const [claimed] = await db
        ...
        .where(and(eq(supportTickets.id, ticketId), or(eq(supportTickets.status, 'queued'), eq(supportTickets.assignedAgentId, agentUserId))))
        .returning();
...
export const activateSupportTicket = async (ticketId: string, agentUserId: string): Promise<void> => {
    const [row] = await db
        .select()
        .from(supportTickets)
        .where(eq(supportTickets.id, ticketId))
    ...
    if (!row)
        throw notFound('ticket_not_found');
    if (row.assignedAgentId !== agentUserId)
    ...
    if (row.status !== 'assigned')
        throw conflict('ticket_not_assigned');
...
export const closeSupportTicket = async (ticketId: string, agentUserId: string): Promise<void> => {
    ...
    const [closed] = await db
        ...
        .where(and(eq(supportTickets.id, ticketId), inArray(supportTickets.status, ['queued', 'assigned', 'active'])))
...
export const getTicketForShopper = async (userId: string, conversationId: string): Promise<SupportTicketDto | null> => {
    const [row] = await db
        .select()
        .from(supportTickets)
        .where(and(eq(supportTickets.conversationId, conversationId), eq(supportTickets.userId, userId)))
