...
export const recommend = async (a: {
    ...
    const basis: RecommendationBasis = a.basedOn ?? (a.productId ? 'similar' : a.userId ? 'recently_viewed' : 'similar');
    const cacheKey = cacheKeys.recommendations(a.userId ?? 'anon', `${basis}:${a.productId ?? '-'}:${limit}`);
    return cached(cacheKey, 60, async () => {
        const ids: string[] = [];
        const push = (candidates: string[]): void => {
            for (const id of candidates) {
                if (id !== a.productId && !ids.includes(id))
                    ids.push(id);
            }
        };
        if (basis === 'similar' && a.productId)
            push(await similarProducts(a.productId, limit));
        if (basis === 'recently_viewed' && a.userId)
            push(await recentlyViewed(a.userId, limit));
        if (basis === 'wishlist' && a.userId)
            push(await becauseYouWishlisted(a.userId, limit));
        if (ids.length < limit && a.userId)
            push(await becauseYouWishlisted(a.userId, limit));
        if (ids.length < limit)
            push(await topRated(limit));
