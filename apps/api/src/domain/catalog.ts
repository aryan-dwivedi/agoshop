...
/**
 * Catalog reads. Cache-aside on the query and per-product keys; every price shown
 * here still comes from `productVariants`, and checkout re-reads it through
 * `resolveLineContext` rather than trusting anything cached.
 */
...
export type CategoryDto = { id: string; slug: string; name: string; imageUrl: string | null };
...
/**
 * Must match `products_search_idx` in `db/extra.sql` verbatim, or Postgres cannot use it.
 *
 * Highlights and specs are part of the document because that is where the words a
 * shopper actually says live: "battery life", "wireless" and "waterproof" appear in
 * `specs`, almost never in `description`. Both are jsonb, so their text form is
 * de-punctuated rather than walked — the keys ("Battery life") are as searchable as
 * the values, and `translate` is immutable, which an index expression requires.
 */
...
/**
 * `plainto_tsquery` ANDs every lexeme. That is right for "denim jacket" and wrong for
 * how the assistant asks — "long battery life" required all three words and returned
 * nothing, so the model burned its whole tool budget re-searching. Re-punctuating the
 * SAME parsed query as an OR keeps normalisation and escaping inside Postgres, and
 * `ts_rank` still floats the products matching every word to the top.
 */
...
const toDto = (r: ProductRow): ProductDto => ({
  ...
  // The variants arrive ordered `is_default desc, price asc`, so the head IS the variant
  // whose price a listing shows — its MRP is the only one a listing may compare against.
...
/**
 * Strict first, relaxed only when strict finds nothing: a browse search for "denim
 * jacket" keeps its precision, and a conversational "long battery life" degrades to an
 * OR match instead of an empty page. Resolved once per query so the listing and its
 * facets are always counted under the same predicate.
 */
  ...
  return cached(cacheKeys.productQuery(queryHash({ ...normalized, mode })), 60, async () => {
    ...
    const [{ rows }, totals] = await Promise.all([
      db.execute<ProductRow>(
        sql`${PROJECTION}${FROM}${where}${buildOrder(normalized, mode)} limit ${pageSize} offset ${
          (page - 1) * pageSize
        }`,
      ),
      db.execute<{ total: number }>(sql`select count(*)::int as total${FROM}${where}`),
    ]);
    return { items: rows.map(toDto), total: totals.rows[0]?.total ?? 0 };
  });
};

/** Facets for the category/search listing: counts are computed under the same filters. */
export const productFacets = async (q: ProductQuery): Promise<ProductFacets> => {
  ...
  return cached(cacheKeys.productQuery(`facets:${queryHash({ ...q, mode })}`), 60, async () => {
    const [cats, sellers, price, ratings] = await Promise.all([
      db.execute<{ slug: string; name: string; count: number }>(
        sql`select c.slug, c.name, count(*)::int as count${FROM}${buildWhere(
          forCategories,
          mode,
        )} group by c.slug, c.name order by c.name asc`,
      ),
      db.execute<{ id: string; name: string; count: number }>(
        sql`select s.id, s.display_name as name, count(*)::int as count${FROM}${buildWhere(
          forSellers,
          mode,
        )} group by s.id, s.display_name order by s.display_name asc`,
      ),
      db.execute<{ min: number | null; max: number | null }>(
        sql`select min(coalesce(v.min_price, p.base_price_minor_units))::int as min,
                   max(coalesce(v.min_price, p.base_price_minor_units))::int as max${FROM}${buildWhere(
                     { ...q, maxPriceMinorUnits: undefined },
                     mode,
                   )}`,
      ),
      db.execute<{ min_rating: number; count: number }>(
...
export const getProductsByIds = async (ids: string[]): Promise<ProductDto[]> => {
  ...
  // Preserve the caller's order — comparison columns must line up with the request.
  return unique.map((id) => byId.get(id)).filter((p): p is ProductDto => p !== undefined);
};

/** Comparison table: the union of spec keys, in first-seen order, one row per product. */
export const compareProducts = async (ids: string[]): Promise<ComparisonDto> => {
  const items = await getProductsByIds(ids);
  const attributes: string[] = [];
  for (const item of items) {
    for (const key of Object.keys(item.specs)) if (!attributes.includes(key)) attributes.push(key);
  }
  return {
    attributes,
    rows: items.map((item) => {
...
/**
 * Stock decrements, session transitions and promotion edits invalidate the catalog
 * caches — a stale variant price or stock count is the one thing this cache must not serve.
 */
