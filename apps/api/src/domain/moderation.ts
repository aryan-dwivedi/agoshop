...
/**
 * Host/admin moderation. Authority lives here, not in the client, because chat is
 * server-mediated: `POST /chat` consults the Redis ban/mute sets on every message, so a
 * ban takes effect on the NEXT message rather than at the next token refresh. Postgres
 * keeps the audit trail the seller dashboard renders.
 *
 * A session-scoped ban deliberately does NOT refuse `POST /api/rtm/token` — that token
 * also carries the user's AI transcripts and chat reads. Only a platform-level ban
 * (`users.bannedAt`) does.
 */
...
export const applyModeration = async (a: {
  ...
  targetUserId?: string | null;
  targetMessageId?: string | null;
}): Promise<{ entryId: string; fanOutShards: number }> => {
  ...
  if (a.action === 'delete_message') {
    if (!a.targetMessageId) throw badRequest('target_message_required');
    ...
    if (!targetUserId) throw badRequest('target_user_required');
    await ensureChatModerationState(session.id);
    const bans = keys.sessionBans(session.id);
    const mutes = keys.sessionMutes(session.id);
    if (a.action === 'ban') await redis.sadd(bans, targetUserId);
    if (a.action === 'mute') await redis.sadd(mutes, targetUserId);
    if (a.action === 'unmute') {
  ...
  if (!entry) throw badRequest('moderation_not_recorded');
  ...
  // SSE reaches every client on every replica — which is what makes a delete visible
  // to a viewer attached to the OTHER API replica.
  ...
  // …and the same fact reaches the chat surface itself as a `chat-service` envelope on
  // every shard, so a client that is subscribed to chat but not to SSE still converges.
...
/** Data source for `GET /api/seller/sessions/:id/moderation` (owned by Platform). */
export const listModeration = async (sessionId: string): Promise<ModerationEntry[]> => {
  ...
  if (userIds.size > 0) {
    ...
    for (const user of userRows) names.set(user.id, user.displayName);
  }
