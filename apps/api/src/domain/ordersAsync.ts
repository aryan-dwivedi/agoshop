...
const insertPromotionHolds = async (tx: Executor, userId: string, orderId: string, applied: {
    code: string;
    minorUnits: number;
}[]): Promise<void> => {
...
const failPendingOrder = async (orderId: string, userId: string, paymentRef: string): Promise<void> => {
  ...
  `);
    ...
    if (failed)
        await publishToUser(userId, EVENTS.orderUpdated, failed);
};
...
export const createOrderIntent = async (userId: string, idempotencyKey: string, input: CreateOrderInput): Promise<OrderDto> => {
    const now = new Date();
...
export const captureOrder = async (orderId: string): Promise<void> => {
    ...
    `);
        ...
        if (failed)
            await publishToUser(order.user_id, EVENTS.orderUpdated, failed);
        return;
    }
    ...
    const { rows: paidRows } = await db.execute<{
        id: string;
    }>(sql `
...
export const expireStaleOrders = async (): Promise<number> => {
  ...
  `);
    ...
    `);
        ...
        if (dto)
            await publishToUser(row.user_id, EVENTS.orderUpdated, dto);
        expired += 1;
    }
    return expired;
};
