...
export const chatChannelsFor = (slug: string, chatShardCount: number): string[] => Array.from({ length: Math.max(1, chatShardCount) }, (_, i) => chatShardChannel(slug, i));
...
export const ensureChatModerationState = async (sessionId: string): Promise<void> => {
    ...
    for (const row of rows) {
        if (!row.targetUserId)
            continue;
        if (row.action === 'ban')
            bans.add(row.targetUserId);
        if (row.action === 'mute')
            mutes.add(row.targetUserId);
...
export const chatBlockReason = async (sessionId: string, userId: string): Promise<'banned' | 'muted' | null> => {
...
export const postChatMessage = async (a: {
    ...
    if (a.productId !== undefined) {
        if (!isHost)
            throw new AppError(403, 'not_session_host');
        sharedProduct = (await listSessionProducts(session.id)).find((product) => product.productId === a.productId);
        if (sharedProduct === undefined)
            throw notFound('session_product_not_found');
    }
    ...
    if (replayed) {
        const [existing] = await db
            ...
            .from(chatMessages)
            .where(and(eq(chatMessages.sessionId, session.id), eq(chatMessages.userId, a.actor.id), eq(chatMessages.clientMessageId, a.clientMessageId)));
        if (existing) {
            ...
            if (existing.productId !== sharedProduct?.productId) {
                sharedProduct =
                    existing.productId === null
                        ? undefined
                        : (await listSessionProducts(session.id)).find((product) => product.productId === existing.productId);
            }
        }
    }
    ...
    const outcome: PublishOutcome = channels.length === 1 && channels[0] !== undefined
...
export const listChatMessages = async (sessionId: string, limit: number): Promise<ChatEnvelope[]> => {
    const rows = await db
        .select({
    ...
    const productsById = new Map((rows.some((row) => row.productId !== null) ? await listSessionProducts(sessionId) : []).map((product) => [product.productId, product]));
    return rows.map((row) => ({
        v: 1,
        type: 'chat',
