...
/**
 * Wishlists feed two things besides the page itself: the `has_wishlisted` user
 * segment in `resolveLineContext` and the `because you wishlisted` recommendation
 * strip — so a write invalidates this user's recommendation cache.
 */

const invalidateUserRecommendations = (userId: string): Promise<number> =>
  invalidate(`rec:v1:${userId}`);

export const addToWishlist = async (
  userId: string,
  productId: string,
): Promise<{ added: boolean }> => {
  const { rows: exists } = await db.execute<{ id: string }>(
    sql`select id from products where id = cast(${productId} as uuid)`,
  );
  if (exists.length === 0) throw notFound('product_not_found');
