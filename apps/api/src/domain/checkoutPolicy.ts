...
const loadFromDb = async (): Promise<CheckoutPolicy> => {
    ...
    const row = rows[0];
    if (!row) {
        throw new AppError(503, 'checkout_policy_unavailable', 'no active checkout_policies row — run npm run db:seed');
    }
    return toPolicy(row);
};
export const loadActivePolicy = async (): Promise<CheckoutPolicy> => {
    try {
        ...
        if (hit !== null)
            return JSON.parse(hit) as CheckoutPolicy;
    }
    catch (err) {
...
export const availablePaymentMethods = async (a: {
    ...
    if (pincode && policy.blockedPincodes.includes(pincode))
        return blocked('pincode_blocked');
    if (policy.requireServiceablePincode) {
        if (!pincode)
            return blocked('pincode_required');
        if (!delivery?.serviceable)
            return blocked('pincode_not_serviceable');
    }
    if (a.totalMinorUnits < policy.minOrderMinorUnits)
        return blocked('below_min_order');
    const methods = policy.allowedMethods.filter((method) => {
        if (method === 'cod') {
            if (a.totalMinorUnits > policy.codMaxOrderMinorUnits)
                return false;
            if (delivery && !delivery.codAvailable)
                return false;
            return delivery !== null;
        }
        if (method === 'emi')
            return a.totalMinorUnits >= policy.emiMinOrderMinorUnits;
        return true;
    });
    return {
        methods,
