...
export const joinSession = async (sessionId: string, user: {
    userId: string;
    role: Role;
    ...
    const isPublisher = row.hostUserId === user.userId || row.coHostUserId === user.userId || user.role === 'admin';
    const rtcToken = mintRtcToken(row.rtcChannel, uid, isPublisher ? 'publisher' : 'subscriber');
    const viewers = row.status === 'live' && !isPublisher ? await touchPresence(sessionId, user.userId) : 0;
    const tier = row.status === 'live' ? await evaluateDeliveryTier(row, viewers) : row.deliveryTier;
...
export const heartbeatSession = async (sessionId: string, userId: string) => {
    ...
    if (!row)
        throw notFound('session_not_found');
    if (row.status !== 'live')
        return { viewerCount: 0, deliveryTier: row.deliveryTier };
    const count = row.hostUserId === userId
