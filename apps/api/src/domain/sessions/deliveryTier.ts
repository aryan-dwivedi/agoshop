...
return 1
`;
export const evaluateDeliveryTier = async (row: {
    id: string;
    slug: string;
    hlsUrl: string | null;
    hlsOriginKind: LiveSessionDto['hlsOriginKind'];
    mediaPushStatus: string;
}, viewers: number): Promise<'rtc' | 'cdn'> => {
    const key = keys.sessionTier(row.id);
    const stored = await redis.get(key);
    if (stored === 'cdn')
        return 'cdn';
    if (viewers < env.RTC_TIER_MAX_VIEWERS)
        return 'rtc';
    if (!hlsOriginReady(row))
        return 'rtc';
    ...
    logger.info({ sessionId: row.id, viewers, hlsOriginKind: origin.hlsOriginKind }, 'delivery tier transitioned rtc -> cdn');
    return 'cdn';
};
