...
import { SAMPLE_LIVE_SOURCE_URL, buildPriceLadder, evaluatePromotions, liveSourceForSlug, } from '@shop/shared';
import type { LineContext, LiveSessionDto, SessionProductDto } from '@shop/shared';
import { hlsOrigin } from '../../agora/mediapush.js';
import { db } from '../../db/client.js';
import { categories, liveSessionProducts, liveSessions, productVariants, products, } from '../../db/schema.js';
import { env } from '../../env.js';
import { LOW_STOCK_THRESHOLD } from '../analytics.js';
import { loadActivePromotions, withSessionLiveRule } from '../promotions.js';
...
const productsForSessions = async (sessionIds: string[]): Promise<Map<string, SessionProductDto[]>> => {
    ...
    for (const row of rows) {
        ...
        if (row.sessionStatus === 'live') {
            ...
            liveMinorUnits = evaluatePromotions(withSessionLiveRule(promotions, ctx, row.sessionDiscountPercent), ctx, env.MAX_TOTAL_DISCOUNT_PCT).netMinorUnits;
        }
...
const toDto = (row: SessionRow, sessionProducts: SessionProductDto[], viewers: number): LiveSessionDto => {
...
export const hydrate = async (rows: SessionRow[]): Promise<LiveSessionDto[]> => {
    ...
    const [grouped, counts] = await Promise.all([
        ...
        Promise.all(rows.map((r) => (r.status === 'live' ? viewerCount(r.id) : Promise.resolve(0)))),
    ]);
