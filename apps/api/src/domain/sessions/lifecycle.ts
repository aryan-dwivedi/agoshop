...
export const frozenShardCount = (expectedPeakViewers: number): number => Math.min(MAX_CHAT_SHARDS, Math.max(1, Math.ceil(expectedPeakViewers / env.RTM_CHAT_SHARD_TARGET)));
...
export const startSession = async (sessionId: string, options: {
    ...
    if (current.status === 'ended')
        throw conflict('session_already_ended');
    ...
    if (!winner) {
        ...
        if (!dto)
            throw notFound('session_not_found');
        if (dto.status !== 'live')
            throw conflict('session_not_startable');
...
export const endSession = async (sessionId: string) => {
    ...
    if (!winner) {
        ...
        if (!dto)
            throw notFound('session_not_found');
        if (dto.status !== 'ended')
            throw conflict('session_not_live');
        return { session: dto, transitioned: false };
    }
    await redis
        .pipeline()
...
export const startDuePremieres = async (): Promise<number> => {
    const due = await db
        ...
        .from(liveSessions)
        .where(and(eq(liveSessions.status, 'scheduled'), eq(liveSessions.autoStart, true), isNotNull(liveSessions.scheduledFor), lte(liveSessions.scheduledFor, new Date())));
    if (due.length === 0)
        return 0;
