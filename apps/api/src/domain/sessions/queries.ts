...
export const listSessions = async (filter: SessionFilter = {}): Promise<LiveSessionDto[]> => {
    ...
    if (filter.sellerSlug !== undefined)
        clauses.push(eq(sellers.slug, filter.sellerSlug));
...
export const getSessionByIdOrSlug = async (key: string): Promise<LiveSessionDto | null> => UUID_PATTERN.test(key) ? getSessionById(key) : getSessionBySlug(key);
...
export const featuredProductId = async (sessionId: string): Promise<string | null> => {
    const [row] = await db
        ...
        .where(and(eq(liveSessionProducts.sessionId, sessionId), eq(liveSessionProducts.isFeatured, true)))
