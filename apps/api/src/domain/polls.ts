...
const countsFor = async (pollId: string, optionIds: string[]): Promise<Record<string, number>> => {
    ...
    for (const row of rows)
        counts[row.optionId] = row.votes;
    if (rows.length > 0) {
        const flat: string[] = [];
        for (const [id, votes] of Object.entries(counts))
            flat.push(id, String(votes));
        await redis.hset(key, ...flat);
        await redis.expire(key, 7 * 24 * 60 * 60);
    }
    return counts;
};
const loadPolls = async (where: 'session' | 'poll' | 'open', value: string, userId: string | null): Promise<{
    dto: PollDto;
    sessionId: string;
}[]> => {
    const pollRows = where === 'session'
        ? await db
            .select()
            .from(polls)
    ...
    if (userId) {
        ...
        for (const vote of voteRows)
            myVotes.set(vote.pollId, vote.optionId);
    }
    const out: {
        dto: PollDto;
        sessionId: string;
    ...
    for (const poll of pollRows) {
        ...
        const counts = await countsFor(poll.id, options.map((o) => o.id));
...
export const listPolls = async (sessionId: string, userId: string | null): Promise<PollDto[]> => (await loadPolls('session', sessionId, userId)).map((p) => p.dto);
export const createPoll = async (a: {
    ...
    if (question.length === 0)
        throw badRequest('question_required');
    const labels = a.options.map((o) => o.trim()).filter((o) => o.length > 0);
    if (labels.length < 2)
        throw badRequest('need_two_options');
    if (labels.length > 6)
        throw badRequest('too_many_options');
    const pollId = await db.transaction(async (tx) => {
        ...
        if (!poll)
            throw conflict('poll_not_created');
        await tx
            .insert(pollOptions)
...
export const votePoll = async (a: {
    ...
    if (!poll)
        throw notFound('poll_not_found');
    if (poll.status !== 'open')
        throw conflict('poll_closed');
    const [option] = await db
        .select({ id: pollOptions.id })
        .from(pollOptions)
        .where(eq(pollOptions.id, a.optionId));
    if (!option || option.id !== a.optionId)
