...
const VIEWER_SAMPLE_INTERVAL_MS = 60000;
const SWEEP_INTERVAL_MS = 30000;
const RECORDING_POLL_INTERVAL_MS = 30000;
const PREMIERE_INTERVAL_MS = 5000;
...
export const runRetentionPurge = async (): Promise<{
    ...
    const cutoff = (days: number): Date => new Date(Date.now() - days * 24 * 60 * 60 * 1000);
...
export const startBackground = async (): Promise<void> => {
    ...
    loop('session-summaries', 2000, consumeSummariesOnce);
    loop('order-capture', 500, consumeOrderCaptureOnce);
    loop('search-index', 2000, consumeSearchIndexOnce);
    leaderLoop('reservation-expiry', 30000, expireStaleOrders);
