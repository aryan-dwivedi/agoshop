...
/**
 * THE ONLY RTM (Signaling) client in a browser session.
 *
 * Agora documents one Signaling client instance per app/client, and two clients would
 * also double this user's RTM account concurrency footprint. Chat and the ConvoAI
 * transcript channel are therefore both *subscriptions* on this one client, which is
 * also what `AgoraVoiceAI.init({ rtcEngine, rtmEngine })` receives.
 *
 * Live RTC and the private AI RTC channel keep separate `IAgoraRTCClient` instances
 * with separate numeric uids — that is a different SDK and a different constraint.
 */

type MessageHandler = (event: RTMEvents.MessageEvent) => void;

/** `POST /api/rtm/token` — the stable account plus its standalone Signaling token. */
type RtmTokenResponse = { account: string; rtmToken: string };

type RtmContextValue = {
  /** Ready only after login succeeds; the toolkit needs the live instance. */
  client: RTMClient | null;
  loggedIn: boolean;
  error: string | null;
  /** Reference-counted. Two callers on one channel produce one subscription. */
  subscribe: (channel: string, handler: MessageHandler) => Promise<() => void>;
};

const RtmContext = createContext<RtmContextValue | null>(null);

type Subscription = { refs: number; handlers: Set<MessageHandler> };

export const RtmProvider = ({
  appId,
  userId,
  children,
}: {
  ...
  /** One login for the whole session, with renewal before the token expires. */
  const ensureLogin = useCallback(async (): Promise<RTMClient> => {
    if (clientRef.current && loggedIn) return clientRef.current;
    if (loginPromiseRef.current) return loginPromiseRef.current;
    if (!appId || !userId) throw new Error('rtm_not_configured');

    loginPromiseRef.current = (async () => {
      // `RTMClient` is a TYPE-ONLY export: agora-rtm@2.3.0's bundle exports only
      // `default`, whose `.RTM` property is the constructor. Importing the named
      // class typechecks and then fails at runtime with "not a constructor".
      ...
      client.addEventListener('message', (event) => {
        ...
        if (!sub) return;
        for (const handler of sub.handlers) handler(event);
      });
  ...
  const subscribe = useCallback(
    async (channel: string, handler: MessageHandler): Promise<() => void> => {
      ...
      return () => {
        ...
        sub.refs -= 1;
        // Releasing the AI channel must leave chat — and the login — untouched.
        if (sub.refs <= 0) {
  ...
  const value = useMemo<RtmContextValue>(
    () => ({ client: clientRef.current, loggedIn, error, subscribe }),
    [loggedIn, error, subscribe],
  );
...
export const useRtm = (): RtmContextValue => {
  ...
  if (!ctx) throw new Error('useRtm must be used inside RtmProvider');
  return ctx;
};
