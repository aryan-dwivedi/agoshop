...
export const useLiveSession = (
    ...
    useEffect(() => {
        ...
        if (session.deliveryTier === 'cdn' && session.hlsUrl) {
            latchCdn({
                url: session.hlsUrl,
                originKind: session.hlsOriginKind ?? 'simulated-origin',
            });
        }
    }, [session, latchCdn]);
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                if (result.deliveryTier === 'cdn' && result.hlsUrl) {
                    latchCdn({
                        url: result.hlsUrl,
                        originKind: result.hlsOriginKind ?? 'simulated-origin',
                    });
                }
            } catch (err) {
                ...
                setJoinError(
                    err instanceof ApiError
                        ? err
                        : new ApiError(0, 'join_failed', String(err), null),
                );
            } finally {
                setJoining(false);
            }
                ...
                case EVENTS.sessionDeliveryTierChanged: {
                    ...
                    latchCdn(
                        data.hlsUrl
                            ? {
                                  url: data.hlsUrl,
                                  originKind: data.hlsOriginKind ?? 'simulated-origin',
                              }
                            : null,
                    );
                    break;
