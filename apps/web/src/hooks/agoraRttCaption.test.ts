...
describe('Agora RTT JSON caption parsing', () => {
  it('returns every segment from the current transcript.results protocol', async () => {
    const payload = encode({
      transcript: {
        uid: 42,
        textTs: 1_710_000_012_345,
        offset: 1_000,
        duration: 500,
        language: 'en-US',
        text: 'Hello.',
        isFinal: true,
        sentenceId: 1_710_000_012_000,
        results: [
          { text: 'Hello.', isFinal: true, offset: 1_000, duration: 200 },
          { text: 'How', isFinal: false, offset: 1_300, duration: 200 },
        ],
      },
    });
  ...
  it('inflates gzip-compressed protocol messages', async () => {
    const raw = encode({
      transcript: {
        ...
        offset: 1_000,
        language: 'en-US',
        results: [{ text: 'Ready to shop.', isFinal: true, offset: 1_000, duration: 200 }],
      },
    });
    ...
    const message = (text: string, isFinal: boolean) =>
      encode({
        transcript: {
          ...
          language: 'en-US',
          sentenceId: 1_710_000_012_000,
          results: [{ text, isFinal, offset: 1_300, duration: 200 }],
        },
      });
