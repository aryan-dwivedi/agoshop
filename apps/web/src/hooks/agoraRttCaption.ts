...
/**
 * Parses Agora Real-Time STT's current JSON protocol. `results` is authoritative:
 * one message can contain a finalized prefix and a mutable suffix, followed by an
 * empty-results message that must not repeat the top-level compatibility text.
 */
export const parseAgoraRttCaption = async (
  ...
  const envelope = JSON.parse(raw) as { transcript?: unknown };
