/**
 * Canvas particle engine for live reactions.
 *
 * Server aggregates taps at 1 Hz (decision 13), so a 10k-viewer room may deliver
 * deltas in the thousands per tick. We never spawn one DOM node per tap — we sample
 * proportionally and cap concurrent particles so the renderer stays O(pool size).
 */

export type ReactionDeltas = Record<string, number>;

export type FlyingReactionsConfig = {
  /** Hard cap on particles alive at once. */
  maxParticles: number;
  /** Max spawns per 1 Hz SSE tick (sampled proportionally across emojis). */
  maxSpawnPerTick: number;
  /** Immediate spawns for the viewer's own tap. */
...
/** Proportional sampling: every emoji with a delta gets representation without O(delta) spawns. */
export const allocateSpawnCounts = (deltas: ReactionDeltas, maxSpawn: number): ReactionDeltas => {
  ...
  // Distribute any rounding remainder to the largest deltas first.
