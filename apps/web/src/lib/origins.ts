/**
 * Cross-surface document navigation.
 *
 * Development keeps three Vite origins. The Render image serves the same bundles from
 * one origin at `/`, `/studio`, and `/support`, which preserves host-only sessions and
 * works on the generated `onrender.com` URL without requiring custom domains.
 */
...
/** An absolute URL on the storefront origin. `path` must start with `/`. */
export const customerUrl = (path = '/'): string => `${CUSTOMER_ORIGIN}${path}`;

/** An absolute URL on the seller-console origin. `path` must start with `/`. */
export const sellerUrl = (path = '/'): string => `${SELLER_ORIGIN}${path}`;

/** An absolute URL on the support-dashboard origin. `path` must start with `/`. */
