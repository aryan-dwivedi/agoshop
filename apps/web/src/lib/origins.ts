...
const runtimeOrigin = typeof window === 'undefined' ? 'http://localhost:5173' : window.location.origin;
const deployedPath = (value: string | undefined, fallback: string): string => `/${(value ?? fallback).replace(/^\/+|\/+$/g, '')}`;
export const CUSTOMER_ORIGIN = trimSlashes(import.meta.env.VITE_CUSTOMER_ORIGIN ??
    (import.meta.env.DEV ? 'http://localhost:5173' : runtimeOrigin));
export const SELLER_ORIGIN = trimSlashes(import.meta.env.VITE_SELLER_ORIGIN ??
    (import.meta.env.DEV
        ? 'http://localhost:5174'
        : `${runtimeOrigin}${deployedPath(import.meta.env.VITE_SELLER_BASE_PATH, 'studio')}`));
export const SUPPORT_ORIGIN = trimSlashes(import.meta.env.VITE_SUPPORT_ORIGIN ??
    (import.meta.env.DEV
        ? 'http://localhost:5175'
