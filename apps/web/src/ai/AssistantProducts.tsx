...
import { buildPriceLadder, type AiProductCard } from '@shop/shared';
...
/**
 * What the assistant just talked about, as products you can act on.
 *
 * The cards are not a second source of truth: they are the products the turn's own
 * catalog tools returned, and adding one goes through the same `/api/cart/items` the
 * product page uses — so the price the shopper is charged is still recomputed by the
 * server, and a live session still decides eligibility.
 *
 * They stack vertically inside the turn. A row is 72px — thumb, one line of title,
 * the price ladder, one amber `Add` — and three are visible before a disclosure,
 * because a horizontal scroller inside a scrolling transcript inside a sheet is
 * unusable on a phone. The card is also the receipt: nothing else announces the add.
 */
...
/** Rows shown before the disclosure. Four is a list; three is an answer. */
const VISIBLE_ROWS = 3;

/** How long the button holds the confirmation before it is an `Add` again. */
const ADDED_MS = 1_200;
...
export const AssistantProducts = ({
  ...
  /** Sent only from a live room, so the server can price the line as a live line. */
  ...
  const add = (card: AiProductCard): void => {
    ...
    void api
      ...
      .then(async () => {
        setStatus((current) => ({ ...current, [card.productId]: AddState.added }));
        // The top bar's total echoes off this invalidation — the same receipt the
        // product page gets, rather than a second notification surface.
        await queryClient.invalidateQueries({ queryKey: ['cart'] });
        // Continue the assisted journey in place: the cart sheet contains the compact
        // delivery, payment and order-placement flow.
        openSheet('cart');
        timers.current.push(
          window.setTimeout(() => {
            setStatus((current) => ({ ...current, [card.productId]: AddState.idle }));
          }, ADDED_MS),
        );
      })
      .catch((err: unknown) => {
  ...
  return (
    <div className="animate-fade-in mt-2 space-y-1.5">
        ...
        return (
          ...
          >
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-0.5">
              ...
              <PriceTag ladder={ladder} size="sm" />
            ...
            {card.inStock ? (
              <button
                ...
                onClick={() => add(card)}
              >
                {state === AddState.added ? 'Added ✓' : state === AddState.busy ? 'Adding…' : 'Add'}
              </button>
            ) : (
      ...
      {hidden > 0 && (
        <button type="button" className="btn-quiet btn-sm w-full" onClick={() => setShowAll(true)}>
          See {hidden} more
        </button>
      )}
    </div>
