...
/** Opening prompts for the browse-surface assistant — shared with the home page. */
...
/**
 * The empty state, per surface. Three real questions instead of a greeting: they are
 * the only thing on an empty panel, so they have to be answerable here, now. The
 * storefront has no host and no pinned product, so it cannot borrow the live room's
 * phrasing — a shopper reading "the featured one" there has nothing to point at.
 */
