...
/**
 * ONE conversation surface for the room.
 *
 * The room used to have two: a "Chat" tab and an "Assistant" tab. That split is what
 * made the assistant feel like a bolted-on widget — the shopper had to leave the
 * conversation they were in to ask a question *about* that conversation, and come back
 * to find it had moved on without them. Worse, it implied the assistant could not see
 * the room, which is exactly the thing a live-shopping assistant has to be able to do.
 *
 * So both live in one stream, ordered by when they happened, with two lanes:
 *   - public chat, visible to everyone;
 *   - the shopper's private assistant turns, in an inset lane marked "only you".
 *
 * That distinction is load-bearing, not decorative: a shopper must never be able to
 * mistake a private answer for something they said out loud to a room of strangers.
 * The composer therefore has an explicit, always-visible destination — and that
 * `Chat | Ask` switch is the *only* thing the old bottom segmented control kept,
 * because it answers "where does this text go", which is a control, not a place.
 * Navigation is the sidebar's three segments and nothing else.
 *
 * Context flows the other way too. Any chat line can be quoted straight into the
 * assistant ("what she just asked — does it fit me?"), and the server prefixes the
 * room's recent chat and speech-to-text onto every turn, so the answer is grounded in
 * the room rather than in the catalog alone.
 */

/** Where the composer sends. Never inferred — the shopper picks, and can see which. */
export type ComposerMode = 'chat' | 'ai';

/** A room line handed to the assistant as the subject of the next question. */
export type QuotedLine = { author: string; text: string; source: 'chat' | 'caption' };

/** The compact agent chip: colour answers "is it my turn to talk?" at a glance. */
const AGENT_STATE: Record<AgentState, { label: string; dot: string; live: boolean }> = {
...
/** Replaces the trailing ellipsis on lines the agent is still producing. */
...
type ChatItem = { kind: 'chat'; ts: number; key: string; message: ChatEnvelope };
type AiItem = { kind: 'ai'; ts: number; key: string; line: AssistantLine };
type Group =
  | { kind: 'chat'; key: string; message: ChatEnvelope }
  | { kind: 'ai'; key: string; items: AiItem[] };
...
export const RoomConversation = ({
  ...
  /** e.g. "4 products" — completes the "the assistant can see…" line. */
  contextNote,
  liveSessionId,
  className,
}: {
  ...
  contextNote?: string;
  /** Sent with an assistant add-to-cart so the line is priced as a live line. */
  ...
  // Voice product cards and tool receipts both ride the conversation's event stream.
  ...
  /*
   * Assistant lines carry no wall clock — a voice turn is identified by
   * `uid-turn_id` and a text turn by a uuid — so the first render that sees a key
   * stamps it. Stamps are strictly increasing, never merely `Date.now()`: the two
   * halves of a voice turn (question, answer) arrive in the same tick and a tie would
   * sort them by map order instead of by who spoke first.
   */
  ...
  const groups = useMemo<Group[]>(() => {
    ...
    // Consecutive assistant turns collapse into one private block, so a five-turn
    // conversation reads as one aside rather than five interruptions of the room.
    const out: Group[] = [];
  ...
  useEffect(() => {
    ...
    // Reading scrollback is a deliberate act; new traffic gets a counter, not a yank.
  ...
  const submit = async (): Promise<void> => {
    ...
    // The quote travels as part of the turn, so the model resolves "she" and "that"
    // against the exact line the shopper tapped rather than against the whole room.
    const question =
      quoted === null
        ? text
        : `About this ${quoted.source === 'chat' ? 'chat message' : 'thing the host said'} — ${quoted.author}: "${quoted.text}"\n\n${text}`;
    onQuotedChange(null);
    // A text turn against a live voice call hands the voice lease back, so the call is
    // ended here first rather than painting "Speaking" over a corpse.
  ...
  return (
    <section className={`relative flex min-h-0 flex-col ${className ?? ''}`}>
      ...
      >
        {chat.historyLoading && (
          <div className="space-y-2 px-1">
            {[0, 1, 2].map((i) => (
              <div key={i} className="skeleton h-4" style={{ width: `${80 - i * 18}%` }} />
            ))}
          </div>
        )}
        ...
        {groups.map((group) =>
          group.kind === 'chat' ? (
            ...
            <AssistantBlock key={group.key} items={group.items} liveSessionId={liveSessionId} />
          ),
        )}
      ...
      {aiMode && agent.capacityNotice !== null && (
        <p role="status" className="border-t border-line px-4 py-2 text-13 text-t2">
      ...
      {readOnly ? (
        ...
        <div className="shrink-0 border-t border-line bg-white p-3">
          {/* Destination first, and never hidden: one of these is a room of strangers. */}
          {assistantEnabled && (
            <div className="flex items-center gap-2">
              ...
              >
                <span
                  aria-hidden
                  className={`absolute inset-y-0.5 w-[calc(50%-2px)] rounded-full transition-all duration-ctl ease-out ${
                    aiMode ? 'left-[calc(50%+1px)] bg-accent' : 'left-0.5 bg-elev'
                  }`}
                />
                <button
                  type="button"
                  ...
                  className={`relative z-10 flex flex-1 items-center justify-center gap-1.5 rounded-full px-2 py-1.5 text-14 font-medium transition-colors duration-ctl ${
                    aiMode ? 'text-t2 hover:text-t1' : 'text-t1'
                  }`}
                >
                  Chat
                </button>
                <button
                  ...
                  className={`relative z-10 flex flex-1 items-center justify-center gap-1.5 rounded-full px-2 py-1.5 text-14 font-medium transition-colors duration-ctl disabled:cursor-not-allowed disabled:opacity-50 ${
                    aiMode ? 'text-accent-ink' : 'text-t2 hover:text-t1'
                  }`}
                >
                  <AskIcon className="h-3.5 w-3.5" />
                  Ask
          ...
          {/* What the assistant can actually see — the reason to ask it here at all. */}
          {aiMode && (
          ...
          {/* Voice is the same conversation, not a different assistant. */}
            ...
            <div className="mt-2 flex items-center gap-1.5">
              {voiceLive ? (
                <>
                  <span className={humanLive ? 'badge-live' : 'badge-neutral'}>
                    <span
                      ...
                      className={`h-1.5 w-1.5 rounded-full ${
                        humanLive ? 'animate-breathe bg-success' : (state?.dot ?? 'bg-t3')
                      } ${state?.live === true ? 'animate-breathe' : ''}`}
                    />
                  ...
                  <button
                    type="button"
                    aria-label={humanLive ? 'End the support call' : 'End the voice conversation'}
...
/** One public chat line, optionally carrying a host-shared product card. */
const ChatRow = ({
  message,
  own,
  canQuote,
  ...
  // `text` is optional on the envelope because a moderation notice carries none.
  // The stream only renders `type:'chat'`, but the type does not know that.
  const text = message.text ?? '';

  return (
    <div
      className={`group/row relative flex items-start gap-2.5 py-1 ${
        own ? 'flex-row-reverse' : ''
      }`}
    >
      <span
        aria-hidden
        className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-12 font-semibold uppercase ${avatarTone(
          { host },
        )}`}
      >
...
/**
 * A run of private assistant turns, inset from the public stream. The lane, the rail
 * and the "only you" label all say the same thing three ways on purpose: mistaking
 * this for public chat is the one error this surface must not allow.
 */
      ...
      {items.map(({ key, line }) => (
        <div key={key} className="space-y-1.5">
          <div className={`flex ${line.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-[92%] rounded-ctl px-3 py-1.5 text-14 ${
                line.role === 'user' ? 'bg-accent text-accent-ink' : 'bg-elev text-t1'
              }`}
            >
              {/* The model writes for a TTS voice and for this bubble at once, so its
                  markup is rendered rather than read out as asterisks. */}
          ...
          {/* An answer about products is only half an answer: the products the turn's
              catalog tools returned are addable right here, priced as live lines. */}
          {line.products.length > 0 && (
            <AssistantProducts products={line.products} liveSessionId={liveSessionId} />
          )}
        </div>
      ))}
    </div>
