...
export const ModerationPanel = ({
                ...
                .then(() => {
                    setLog((current) =>
                        [
                            {
                                id: `${Date.now()}-${action}`,
                                text: `${ACTION_LABEL[action]} · ${target.label}`,
                            },
                            ...current,
                        ].slice(0, 8),
                    );
    ...
    return (
        <section className="card overflow-hidden">
                ...
                {participants.length === 0 ? (
                    ...
                    <ul className="mt-1 divide-y divide-line">
                        {participants.map((p) => (
                            <li
                                key={p.userId}
                                className="flex items-center justify-between gap-2 py-2 text-14"
                            >
            ...
            {error !== null && (
                <p className="border-t border-line px-4 py-2.5 text-13 font-medium text-danger">
                    {error}
                </p>
            )}
        </section>
