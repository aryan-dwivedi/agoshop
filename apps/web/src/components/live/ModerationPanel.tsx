...
export const ModerationPanel = ({ sessionId, messages, selected, onClearSelection, notice, }: {
    sessionId: string;
    ...
    const act = useCallback((action: Action, target: {
        targetUserId?: string;
    ...
    return (<section className="card overflow-hidden">
      ...
      {selected ? (<div className="animate-fade-in border-b border-line bg-surface px-4 py-3">
          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            <button type="button" className="btn-danger btn-xs" disabled={busy !== null} onClick={() => act('delete_message', {
            ...
            <button type="button" className="btn-danger btn-xs" disabled={busy !== null} onClick={() => act('mute', { targetUserId: selected.userId, label: selected.displayName })}>
              Mute author
            </button>
        ...
        </div>) : (<p className="border-b border-line px-4 py-3 text-13 leading-relaxed text-t3">
          Select a message in chat to delete it or mute its author.
        </p>)}

      <div className="px-4 py-3">
        <p className="eyebrow">Most active participants</p>
        {participants.length === 0 ? (<p className="mt-2 text-13 text-t3">No chat activity yet.</p>) : (<ul className="mt-1 divide-y divide-line">
            {participants.map((p) => (<li key={p.userId} className="flex items-center justify-between gap-2 py-2 text-14">
                <span className="min-w-0 truncate font-medium text-t1">
                  {p.displayName}
                  <span className="tnum ml-2 text-13 text-t3">{p.messages}</span>
                </span>
                <span className="flex shrink-0 gap-1">
                  {(['mute', 'unmute', 'ban'] as const).map((action) => (<button key={action} type="button" className={action === 'unmute' ? 'btn-standard btn-xs' : 'btn-danger btn-xs'} disabled={busy === `${action}:${p.userId}`} onClick={() => act(action, { targetUserId: p.userId, label: p.displayName })}>
                      {ACTION_LABEL[action]}
                    </button>))}
                </span>
              </li>))}
      ...
      {(log.length > 0 || notice !== null) && (<div className="border-t border-line bg-surface px-4 py-3">
      ...
      {error !== null && (<p className="border-t border-line px-4 py-2.5 text-13 font-medium text-danger">{error}</p>)}
    </section>);
};
