...
/**
 * Replay media uses the same dark, over-video control language as the live theatre.
 * The native browser chrome is intentionally replaced: it cannot surface the recorded
 * transcript, does not match the room controls, and varies materially across browsers.
 */
export const ReplayPlayer = forwardRef<ReplayPlayerHandle, ReplayPlayerProps>(function ReplayPlayer(
  ...
  /**
   * MediaRecorder WebM files omit duration metadata. Chrome can resolve it from the
   * final cluster by seeking past EOF once, after which the player becomes normally
   * seekable. The original position is restored as soon as `durationchange` lands.
   */
  ...
  const revealControls = useCallback(() => {
    ...
    if (!videoRef.current?.paused) {
      hideTimerRef.current = window.setTimeout(() => setControlsVisible(false), 2_500);
    }
  }, [clearHideTimer]);
  ...
  useEffect(() => {
    ...
    video.src = src;
    // Cached WebM metadata can arrive before React observes `loadedmetadata`. Probe
    // from the source effect as well; the callback is idempotent.
  ...
  return (
    <div
      ...
      className={`group/player relative aspect-video w-full overflow-hidden rounded-panel bg-black text-white shadow-card outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2 ${
    ...
    >
      <video
        ...
        onTimeUpdate={(event) => {
          if (durationProbeRef.current === null) setCurrentTime(event.currentTarget.currentTime);
        }}
        onProgress={(event) => {
      ...
      <div
        aria-hidden="true"
        className={`pointer-events-none absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-black/60 to-transparent transition-opacity duration-200 ${
          controlsVisible ? 'opacity-100' : 'opacity-0'
        }`}
      />
      <div
        className={`pointer-events-none absolute left-3 top-3 flex items-center gap-2 transition-opacity duration-200 md:left-4 md:top-4 ${
          controlsVisible ? 'opacity-100' : 'opacity-0'
        }`}
      >
      ...
      {!playing && !probingDuration && mediaError === null && (
        ...
        >
          {currentTime >= duration && duration > 0 ? (
            <RotateCcw className="h-7 w-7" strokeWidth={1.8} />
          ) : (
      ...
      <div
        className={`absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black/90 via-black/55 to-transparent px-3 pb-3 pt-14 transition-opacity duration-200 md:px-4 md:pb-4 ${
