...
const Unit = ({ value, label }: {
    value: number;
    label: string;
}): JSX.Element => (<div className="flex min-w-[4rem] flex-col items-center px-3 py-2.5">
...
export const ScheduledStage = ({ session, skewMs, }: {
    session: LiveSessionDto;
    skewMs: number;
}): JSX.Element => {
    const startsAtMs = session.scheduledFor === null ? null : new Date(session.scheduledFor).getTime();
    ...
    return (<div className="relative flex h-full min-h-[18rem] w-full flex-1 items-center justify-center overflow-hidden bg-[#090b10] p-4 sm:p-6">

      {session.coverImageUrl !== null && (<img src={session.coverImageUrl} alt="" aria-hidden className="absolute inset-0 h-full w-full scale-110 object-cover opacity-45 blur-md"/>)}
      ...
      <div className="relative w-full max-w-xl rounded-panel border border-white/15 bg-[#07172e]/95 px-5 py-6 text-center text-white shadow-float sm:px-8 sm:py-7">
        ...
        <div className="mt-6">
          {remaining !== null && !due ? (<>
              ...
              <div className="mt-3 inline-flex max-w-full divide-x divide-white/10 overflow-hidden rounded-ctl border border-white/15 bg-white/[0.06]" role="timer" aria-live="off" aria-label={`Show starts in ${remaining.days} days, ${remaining.hours} hours, ${remaining.minutes} minutes and ${remaining.seconds} seconds`}>
                {remaining.days > 0 && <Unit value={remaining.days} label="days"/>}
                {(remaining.days > 0 || remaining.hours > 0) && (<Unit value={remaining.hours} label="hrs"/>)}
                <Unit value={remaining.minutes} label="min"/>
                <Unit value={remaining.seconds} label="sec"/>
              </div>
            </>) : (<p className="rounded-ctl border border-white/10 bg-white/[0.06] px-4 py-3 text-14 font-medium text-white" role="status">
              {startsAtMs === null
          ...
          {startsAtMs !== null && (<p className="tnum mt-3 text-13 font-medium text-white/65">
