...
/** A product the host shared into public chat, actionable without leaving the stream. */
export const ChatProductCard = ({
  ...
  live: boolean;
  signedIn: boolean;
  /** Host and replay surfaces show the attachment but do not expose a shopper action. */
  addable: boolean;
}): JSX.Element => {
  ...
  return (
    <div className="mt-2 flex overflow-hidden rounded-ctl border border-line bg-elev text-left not-italic">
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-1 px-2.5 py-2">
        <Link
          to={`/p/${product.slug}`}
          className="line-clamp-2 text-13 font-semibold text-t1 hover:underline"
        >
          {product.title}
        </Link>
        ...
        {addable && (
          <button
            type="button"
            className="btn-commit btn-xs mt-0.5 self-start"
            disabled={!signedIn || soldOut || status !== 'idle'}
            onClick={addToCart}
          >
            {soldOut
              ? 'Out of stock'
