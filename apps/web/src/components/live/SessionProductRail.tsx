...
/**
 * The room's full line-up. The host's pin moves a product to the front with a
 * `Featured now` marker in both delivery tiers, and an add-to-cart from a live room
 * carries `liveSessionId` so the server decides — from relational truth, not from a
 * client claim — whether the live rule applies to that line.
 *
 * Exactly one of these is ever on screen: the `Shop` segment of the room's sidebar
 * from 1024px up, the horizontal rail below the theatre under it.
 *
 * Numeric stock appears only for a host (`canPin`). A shopper gets `Out of stock` and
 * nothing else: a truthful count is indistinguishable from a manufactured one at a
 * glance, so it belongs to the console, where it is an operational fact.
 */
...
export const SessionProductRail = ({
  ...
  signedIn: boolean;
  /**
   * `rail` swipes horizontally under the stage; `column` stacks in a narrow sidebar;
   * `responsive` is both — the room's one line-up, which is a rail below the theatre
   * under 1024px and a column in the sidebar above it. One mount, so the room can
   * never show two of these at once.
   */
  variant?: 'rail' | 'column' | 'responsive';
  /**
   * Drops the panel's own card so the rail can sit inside a surface that already
   * supplies one — the room's sidebar — instead of stacking two cards.
   */
  seamless?: boolean;
}): JSX.Element => {
  ...
  // `productId: null` clears the highlight — the host un-features without picking a
  // replacement. The event fan-out is identical, so both tiers follow.
  const setPinned = useCallback(
    (productId: string, pinned: boolean) => {
      setBusyId(productId);
      void api
        .post(`/api/sessions/${sessionId}/pin`, { productId: pinned ? null : productId })
        .catch(() =>
          setErrorById((current) => ({
            ...current,
    ...
    <section className={seamless ? '' : 'card overflow-hidden'}>
      <div
        className={`flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 ${
          seamless ? 'pb-2' : 'border-b border-line px-4 py-3'
        }`}
      >
      ...
      <ul
        className={`${
          ...
          // Seamless drops the panel's own gutters so the rows line up with whatever
          // padding the surrounding surface already has.
          seamless ? 'py-1' : 'p-4'
        }`}
      >
        {ordered.map((product) => {
          ...
          // Server truth, guarded: a payload that predates stock reporting has no
          // opinion about it, which is not the same as being sold out.
          ...
          return (
            ...
            >
              <Link to={`/p/${product.slug}`} className="group relative block overflow-hidden">
                {product.imageUrl === null ? (
                  ...
                  <img
                    ...
                    alt={product.title}
                    /* Product photography is shot on white; the tile keeps it that way. */
                    className="aspect-[4/3] w-full bg-[#fff] object-cover transition duration-panel ease-out group-hover:scale-[1.03]"
                    loading="lazy"
                  />
                )}
                {pinned && <span className="badge-accent absolute left-2 top-2">Featured now</span>}
                {soldOut && (
                  <span className="badge-neutral absolute bottom-2 left-2">Out of stock</span>
                )}
              </Link>
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1 p-3">
                ...
                {/* The one surface where the live tier is actually on offer, so the
                    live price leads and the shop price it undercuts is struck beside it. */}
                <PriceTag ladder={product.price} size="sm" onDark className="tnum" />

                <div className="mt-auto flex flex-wrap items-center gap-2 pt-2">
                  <button
                    type="button"
                    className="btn-commit btn-sm flex-1"
                    disabled={!signedIn || soldOut || busyId === product.productId}
                  ...
                  {canPin && (
                    <button
                      type="button"
                      className={pinned ? 'btn-quiet btn-sm' : 'btn-standard btn-sm'}
                ...
                {!signedIn && <p className="text-13 text-t3">Sign in to add to cart.</p>}
                {applied.length > 0 && (
                  <p className="tnum text-13 font-medium text-success">
                    In cart at {formatInr(line?.pricing.netMinorUnits ?? 0)}
                  </p>
                )}
                {addedId === product.productId && applied.length === 0 && error === undefined && (
