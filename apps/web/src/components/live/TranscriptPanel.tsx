        ...
        <section className={`flex min-h-0 flex-col ${className ?? ''}`}>
            ...
            >
                ...
                {!history.isLoading && rows.length > 0 && visible.length === 0 && (
                    <p className="px-1 py-6 text-center text-14 text-t3">
                        Nothing said matches “{query}”.
                    </p>
                )}
                ...
                {visible.map((row) => (
                    ...
                    >
                        <p className="flex items-baseline gap-2">
                            <span className="tnum shrink-0 text-11 text-t3">
                                {stamp(row.startMs)}
                            </span>
            ...
            {rows.length > 0 && (
                <p className="border-t border-line px-3 py-1.5 text-11 leading-tight text-t3">
                    Auto-generated from speech — wording may be imperfect. The assistant reads this
                    too, so you can ask about anything above.
                </p>
            )}
        </section>
    );
