...
/** Below this the audience size is not reported at all — see `LiveBadge`. */
...
export const LiveTheatre = ({
  ...
  live: LiveSession;
  isLive: boolean;
  config: { agoraAppId: string | null } | null;
  offline: boolean;
  consentGate: boolean;
  onConsent: () => void;
      ...
      >
        <div className="absolute inset-0">
          {isLive ? (
            ...
            <ScheduledStage session={session} skewMs={live.serverSkewMs} />
          ) : (
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-30 flex items-start gap-3 p-3 md:p-4">
          ...
          <div className="relative flex min-w-0 flex-1 flex-wrap items-center gap-2">
            ...
            {offline && (
              <span role="status" className="on-video pill text-14">
                Reconnecting
              </span>
            )}
              ...
              {pollOpen ? (
                <div className="relative">
                  <PollCard
                    ...
                    canVote={Boolean(user) && live.poll.status === 'open' && !offline}
                  />
                  <button
                    type="button"
              ...
              >
                <span
                  className={`h-2 w-2 shrink-0 rounded-full ${
                    quality === option.id ? 'bg-accent' : 'bg-white/20'
                  }`}
                />
          ...
          <>
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 flex items-end justify-end gap-2 p-3 md:p-4">
              ...
              <button
                ...
                aria-label={chatCollapsed ? 'Open room sidebar' : 'Collapse room sidebar'}
                title={chatCollapsed ? 'Open room sidebar' : 'Collapse room sidebar'}
              ...
              <button
                ...
                title={captionsAvailable ? 'Closed captions' : 'Closed captions unavailable'}
                disabled={!captionsAvailable}
