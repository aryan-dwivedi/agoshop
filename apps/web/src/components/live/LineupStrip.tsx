...
/**
 * The line-up, one keystroke per product.
 *
 * This is the most-used control in the broadcast room, so it is always visible, never
 * behind a tab, and never scrolled away by anything else. Pinning is one key because
 * the seller is mid-sentence when they decide to do it.
 *
 * Stock is shown as a number because this is Studio: the seller needs to know, and
 * the shopper side deliberately never sees a count.
 */

export type PinControl = {
  /** `null` unpins — the host clears the highlight without picking a replacement. */
  pin: (productId: string | null) => void;
  /** The product a pin is in flight for, so only that tile shows the wait. */
  ...
  const pin = useCallback(
    (productId: string | null): void => {
      ...
      void api
        ...
        .catch(() =>
          setError(productId === null ? 'Could not clear the pin.' : 'Could not pin that product.'),
        )
        .finally(() => {
          setPendingId(null);
          setBusy(false);
...
/** Nine is the whole keyboard row; a tenth product is clickable, not keyed. */
const KEYED = 9;
...
export const LineupStrip = ({
  ...
  return (
    <section className={className} aria-label="Line-up">
      <div className="scroll-thin flex items-stretch gap-2 overflow-x-auto pb-1">
        {ordered.map((product, index) => {
          ...
          // `stock` arrives on the session rail; a deployment without it says nothing
          ...
          return (
            ...
            >
              <span className="flex items-center gap-1.5">
                ...
                {pinned && (
                  <span
                    className={`text-11 font-semibold uppercase tracking-[0.08em] ${
                      soldOut ? 'text-t2' : 'text-accent-ink'
                    }`}
                  >
              ...
              <span
                className={`line-clamp-2 text-14 font-medium ${
                  pinned && !soldOut ? 'text-accent-ink' : 'text-t1'
                }`}
              >
                {product.title}
              </span>
              ...
              <span className="mt-auto flex items-baseline gap-1.5">
                <span
                  className={`tnum text-14 font-semibold ${
                    pinned && !soldOut ? 'text-accent-ink' : 'text-t1'
                  }`}
                >
        ...
        {/* The `0` shortcut, visible: nothing in this room is keyboard-only. */}
        <button
          type="button"
