...
/** The sidebar shows one surface at a time so the room fits a single screen. */
export type RoomTab = 'chat' | 'products' | 'ask';

const SEGMENTS: readonly { id: RoomTab; label: string; icon: JSX.Element }[] = [
...
>(function LiveRoomSidebar(
  ...
  return (
    <aside
      ref={ref}
      className={`absolute inset-x-0 bottom-0 z-50 max-h-[75%] min-h-[24rem] min-w-0 flex-col rounded-t-panel border-t border-line bg-surface shadow-float lg:static lg:max-h-none lg:min-h-0 lg:rounded-none lg:border-l lg:border-t-0 lg:shadow-none ${
    ...
    >
        ...
        >
          {SEGMENTS.map((segment, index) => (
            <button
              ...
              className={`flex h-11 flex-1 items-center justify-center gap-1.5 px-2 text-13 font-medium transition-colors duration-ctl ${
                tab === segment.id ? 'text-t1' : 'text-t2 hover:text-t1'
              }`}
            >
              {segment.icon}
      ...
      >
        <RoomConversation
          ...
          contextNote={`the ${session.products.length} product${
      ...
      <div
        ...
        hidden={tab !== 'products'}
        className={`scroll-thin min-h-0 flex-1 overflow-y-auto px-2.5 ${
