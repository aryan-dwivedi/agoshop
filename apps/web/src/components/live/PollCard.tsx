...
/**
 * Poll votes are server-owned: one row per (poll, voter), results broadcast to the
 * room as an aggregate. The card renders the same shape live, on the host console and
 * in replay — only the affordances differ.
 */
  ...
  return (
    <div className="card animate-slide-up p-4">
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t border-line pt-3 text-13 text-t3">
        ...
        {canClose && !closed && (
          <button type="button" className="btn-quiet btn-sm" disabled={busy} onClick={close}>
            Close poll
          </button>
        )}
      </div>
