...
export const PollCard = ({ poll, onPollChange, canVote, canClose, }: {
    poll: PollDto;
    ...
    return (<div className="card animate-slide-up p-4">
      ...
      <ul className="mt-3 space-y-2">
        {poll.options.map((option) => {
            ...
            return (<li key={option.id}>
              {showResults ? (<div className="space-y-1">
                  <div className="flex items-baseline justify-between gap-3 text-13">
                    <span className={mine ? 'font-semibold text-accent' : 'text-t2'}>
                      {option.label}
                      {mine && ' · your vote'}
                    </span>
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-surface">
                    <div className={`h-full rounded-full ${mine ? 'bg-accent' : 'bg-line-ctl'}`} style={{
                        width: `${share}%`,
                        transition: 'width var(--d-panel) var(--ease-out)',
                    }}/>
                  </div>
                </div>) : (<button type="button" className="btn-standard w-full justify-start text-left" disabled={!canVote || busy} onClick={() => vote(option.id)}>
                  {option.label}
                </button>)}
            </li>);
        })}
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t border-line pt-3 text-13 text-t3">
        ...
        {canClose && !closed && (<button type="button" className="btn-quiet btn-sm" disabled={busy} onClick={close}>
            Close poll
          </button>)}
      </div>

      {error !== null && <p className="mt-2 text-13 font-medium text-danger">{error}</p>}
      {!canVote && !closed && !canClose && (<p className="mt-2 text-13 text-t3">Sign in to vote in this poll.</p>)}
    </div>);
};
