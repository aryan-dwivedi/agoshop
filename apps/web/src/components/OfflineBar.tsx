...
export const OfflineBar = (): JSX.Element | null => {
    ...
    if (!offline)
        return null;
    return (<div role="status" className="border-b border-line bg-surface">
      <div className="mx-auto flex max-w-page items-center gap-2 px-3 py-1.5 text-14 text-t2 md:px-5">
        You&rsquo;re offline.
        <button type="button" className="btn-quiet btn-sm" onClick={() => void queryClient.invalidateQueries()}>
          Retry
        </button>
      </div>
    </div>);
