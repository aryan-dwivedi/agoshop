...
export const ObsIngestPanel = ({
    ...
    if (ingest === null) {
        return (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-bg/90 p-6 text-center">
                <p className="text-14 text-t2">Preparing OBS credentials…</p>
            </div>
        );
    }
    return (
        <div className="absolute inset-0 z-20 flex flex-col justify-end bg-gradient-to-t from-bg via-bg/80 to-transparent p-4">
            <div className="rounded-panel border border-line bg-bg p-4 shadow-e1">
                <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                        className={`rounded-chip px-2 py-0.5 text-12 font-medium ${connected ? 'bg-success/15 text-success' : 'bg-accent/15 text-accent'}`}
                    >
                ...
                <dl className="mt-3 space-y-2 text-13">
                    <div>
                        ...
                        <dd className="mt-0.5 flex items-center gap-2">
                            <code className="tnum min-w-0 flex-1 truncate text-12 text-t1">
                                {ingest.rtmpServer}
                            </code>
                            <button
                                type="button"
                                className="btn-quiet btn-xs shrink-0"
                                onClick={() => void onCopy('server', ingest.rtmpServer)}
                            >
                                {copied === 'server' ? (
                                    <Check
                    ...
                    <div>
                        ...
                        <dd className="mt-0.5 flex items-center gap-2">
                            ...
                            <button
                                type="button"
                                className="btn-quiet btn-xs shrink-0"
                                onClick={() => void onCopy('key', ingest.streamKey)}
                            >
                                {copied === 'key' ? (
                                    <Check
                                        className="h-3.5 w-3.5"
                ...
                <p className="mt-3 text-12 text-t3">
                    In OBS: Settings → Output → keyframe interval 2s, profile baseline, 30 fps.
                    Replay capture is not recorded for OBS shows in this prototype.
                </p>
            </div>
        </div>
    );
