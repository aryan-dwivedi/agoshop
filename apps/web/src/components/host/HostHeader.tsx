...
export const HostHeader = ({ title, status, elapsed, viewersLabel, health, publishing, slug, isLive, broadcast, endHoldRef, onOpenKeyMap, onEndSession, isOwner = true, }: {
    title: string;
    ...
    onEndSession: () => void;
    isOwner?: boolean;
}): JSX.Element => (<header className="flex h-12 shrink-0 items-center gap-3 border-b border-line px-3">
    ...
    <div className="ml-auto flex items-center gap-2">
      {!publishing && isOwner && (<Link to={`/live/${slug}/preflight`} className="btn-standard btn-sm">
          Pre-flight
        </Link>)}
      {isOwner && (<>
          <button type="button" className="btn-quiet btn-sm" aria-keyshortcuts="?" onClick={onOpenKeyMap}>
            Shortcuts ?
          </button>
          <HoldToConfirm ref={endHoldRef} label={broadcast.state === 'ending' ? 'Ending…' : 'Hold to end'} subLabel={viewersLabel} disabled={!isLive || broadcast.state === 'ending' || broadcast.state === 'ended'} onConfirm={onEndSession}/>
        </>)}
    </div>
  </header>);
