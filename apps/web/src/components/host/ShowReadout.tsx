...
/**
 * What this show has done so far, and what is left on the shelf. Four numbers and a
 * stock list — the only figures that change a decision while the seller is on air.
 */
export const ShowReadout = ({
  sessionId,
  products,
  live,
}: {
  ...
  const analytics = useQuery<SessionAnalyticsDto, Error>({
    ...
    // A live show moves; an ended one does not.
  ...
  const metrics = [
    ...
    {
      label: 'Conversion',
      // A rate over zero viewers is not zero: it is unknown, and a false 0.0% here
      // would be read as a failing show.
      value:
        data === null || data.uniqueViewers === 0
          ? '—'
  ...
  return (
    <section className="border-t border-line p-3" aria-label="This show">
      ...
      <ul className="mt-1 space-y-1">
        {products.map((product) => {
          ...
          return (
            <li key={product.productId} className="flex items-baseline gap-2 text-14">
              <span className="truncate text-t2">{product.title}</span>
              <span
                className={`tnum ml-auto font-medium ${
                  stock !== null && stock <= 0
                    ? 'text-danger'
