...
/**
 * Host-only panel for inviting a second publisher onto the stage. Authorization to
 * publish is enforced server-side via co-host token roles; this UI only sets who may
 * receive a PUBLISHER token.
 */
export const CoHostPanel = ({
  ...
  return (
    <section className="border-t border-line p-3" aria-label="Co-host">
      ...
      {session.coHostName !== null ? (
        <div className="mt-3 flex items-center gap-2 rounded-ctl border border-line px-3 py-2">
          ...
          <button
            ...
            disabled={busy}
            onClick={() =>
              void remove.mutateAsync(session.id).then((updated) => onSessionUpdated(updated))
            }
          >
