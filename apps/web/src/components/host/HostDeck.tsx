...
export const HostDeck = ({
  ...
  broadcast: Broadcast;
  session: { id: string; discountPercent: number | null };
  poll: PollDto | null;
    ...
    <>
      ...
      <div className="relative flex h-16 shrink-0 items-center gap-2 border-t border-line px-3">
        <DeckButton
          ...
          disabled={
            source === 'obs' || broadcast.state === 'idle' || reconnecting || disconnected
          }
        ...
        {isOwner && (
          <>
            <DeckButton
              ...
              label="Price"
              value={
                session.discountPercent === null ? 'no markdown' : `${session.discountPercent}% off`
              }
