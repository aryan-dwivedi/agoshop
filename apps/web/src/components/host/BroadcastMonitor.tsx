    ...
    return (
        ...
        >
            <div
                ...
                style={{ width: monitorWidth === null ? '100%' : `${monitorWidth}px` }}
            >
                <div
                    className="relative overflow-hidden rounded-t-panel bg-[#101210]"
                    style={{ aspectRatio: aspect, boxShadow: inset }}
                >
                    <div
                        ref={previewRef}
                        className={`h-full w-full transition-opacity duration-ctl ${reconnecting ? 'opacity-60' : ''}`}
                    />
                    ...
                    {broadcast.remotePublishers.map((publisher) => (
                        <CoHostPip
                            key={publisher.uid}
                            publisher={publisher}
                            label={session.coHostName ?? 'Co-host'}
                        />
                    ))}

                    {source === 'obs' &&
                        (broadcast.state === 'live' || broadcast.state === 'starting') && (
                            <ObsIngestPanel
                                ingest={broadcast.obsIngest}
                                connected={broadcast.obsFeedConnected}
                            />
                        )}

                    {!broadcast.videoPublished &&
                        publishing &&
                        !disconnected &&
                        source !== 'obs' && (
                            <p className="on-video absolute left-3 top-3 z-20 rounded-chip px-2 py-1 text-14 font-medium">
                                Audio only — viewers hear you
                            </p>
                        )}

                    {(broadcast.state === 'idle' || broadcast.state === 'error') && (
                        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-bg p-6 text-center">
                            ...
                            <div className="flex flex-wrap items-center justify-center gap-2">
                                ...
                                {isOwner && (
                                    <Link
                                        to={`/live/${session.slug}/preflight`}
                    ...
                    {broadcast.state === 'preview' && (
                        <div className="scrim-bottom absolute inset-x-0 bottom-0 z-30 flex flex-wrap items-end justify-between gap-3 p-3">
                            <div className="flex flex-col gap-2">
                                ...
                                {!consent && (
                                    <label className="on-video flex max-w-md items-start gap-2 rounded-ctl px-3 py-2 text-14 font-medium">
                                        <input
                                            type="checkbox"
                                        ...
                                        />
                                        This show is recorded and published as a replay. Viewers are
                                        told on join.
                                    </label>
                                )}
                            </div>
                            <HoldToConfirm
                                ...
                                subLabel={consent ? 'Viewers see you at once' : undefined}
                                className="btn-commit btn-lg"
                                disabled={!canGoLive}
                                onConfirm={() => void broadcast.goLive()}
                            />
                        </div>
                    )}

                    {broadcast.state === 'starting' && (
                        <p className="on-video absolute inset-x-0 bottom-3 z-30 mx-auto w-fit rounded-chip px-3 py-1.5 text-14 font-medium">
                            Going on air…
                        </p>
                    )}

                    {reconnecting && (
                        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-2 p-4 text-center">
                            ...
                            {reconnectSeconds >= AUDIO_ONLY_AFTER_S && broadcast.videoPublished && (
                                <button
                                    type="button"
                                    className="btn-standard on-video"
                                    onClick={() => void broadcast.publishVideo(false)}
                                >
                                    Audio only
                                </button>
                            )}
                    ...
                    {disconnected && (
                        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-bg p-6 text-center">
                            <p className="text-19 font-semibold text-t1">
                                Your camera dropped off air.
                            </p>
                            <p className="max-w-md text-14 text-t2">
                                The show is still live for viewers — they are watching the standby
                                feed. Rejoin to put your camera back on it.
                            </p>
                            <div className="flex flex-wrap items-start justify-center gap-2">
                                <button
                                    type="button"
                                    className="btn-commit btn-lg"
                                    onClick={() => void broadcast.rejoin()}
                                >
                                    Rejoin
                                </button>
                                {isOwner && (
                                    <HoldToConfirm
                                        label="Hold to end the show"
                                        subLabel={viewersLabel}
                                        onConfirm={() => void onEndSession()}
                                    />
                                )}
                            </div>
