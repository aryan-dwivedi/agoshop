...
export const BroadcastMonitor = ({
  ...
  onStartPreview: () => void;
  onEndSession: () => void;
  /** When false, co-host controls only — no end-show affordances in overlays. */
  isOwner?: boolean;
}): JSX.Element => {
    ...
    <div ref={stageRef} className="flex min-h-0 flex-1 items-center justify-center p-2">
      ...
      >
        ...
        >
          <div
            ref={previewRef}
            className={`h-full w-full transition-opacity duration-ctl ${
          ...
          {source === 'obs' && (broadcast.state === 'live' || broadcast.state === 'starting') && (
            <ObsIngestPanel
          ...
          {!broadcast.videoPublished && publishing && !disconnected && source !== 'obs' && (
          ...
          {(broadcast.state === 'idle' || broadcast.state === 'error') && (
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-bg p-6 text-center">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2">
                {source !== 'obs' && (
                  <button type="button" className="btn-commit" onClick={onStartPreview}>
                ...
                {isOwner && (
                  <Link to={`/live/${session.slug}/preflight`} className="btn-standard">
                    Run pre-flight
                  </Link>
                )}
              </div>
