...
export const uploadSourceVideo = async (sessionId: string, file: File): Promise<LiveSessionDto> => {
    ...
    if (!res.ok) {
        ...
        throw new Error(failure?.message ?? failure?.code ?? `The server rejected the upload (${res.status}).`);
    }
    const body = parsed as {
        session?: LiveSessionDto;
    } | null;
    if (!body?.session)
        throw new Error('The upload succeeded but the server returned no session.');
    return body.session;
};
const Column = ({ title, note, children, }: {
    title: string;
    note?: string;
    children: ReactNode;
}): JSX.Element => (<section className="min-w-0 border-line px-3 py-3 lg:border-l lg:first:border-l-0">
...
export const SessionScheduler = ({ show, mode, onSaved, onClose, }: {
    ...
    const [draft, setDraft] = useState<Draft>(() => ({
        ...
        liveDiscountPercent: source?.session.discountPercent == null ? '' : String(source.session.discountPercent),
        coverImageUrl: source?.session.coverImageUrl ?? '',
    }));
    const [startMode, setStartMode] = useState<StartMode>(source?.session.autoStart === true ? 'premiere' : 'manual');
    const [videoFile, setVideoFile] = useState<File | null>(null);
    const videoInputRef = useRef<HTMLInputElement>(null);
    const [query, setQuery] = useState('');
    const [picked, setPicked] = useState<string[]>(() => source === null
        ? []
        : [...source.session.products]
            .sort((a, b) => a.sortOrder - b.sortOrder)
            .map((product) => product.productId));
    const [featured, setFeatured] = useState<string | null>(() => source?.session.products.find((product) => product.isFeatured)?.productId ?? null);
    ...
    const matches = useMemo(() => {
        ...
        if (needle === '')
            return catalogue;
        return catalogue.filter((product) => product.title.toLowerCase().includes(needle) ||
            ...
            if (existing === null) {
                ...
                if (discount !== (existing.session.discountPercent ?? null)) {
                    const priced = await api.patch<{
                        session: LiveSessionDto;
                    }>(`/api/sessions/${session.id}/pricing`, { discountPercent: discount });
    ...
    const premiereNeedsVideo = startMode === 'premiere' && videoFile === null && existing?.session.sourceVideoUrl == null;
    const premiereNeedsTime = startMode === 'premiere' && localInputToIso(draft.scheduledFor) === null;
    ...
    const emptiedLineUp = existing !== null && picked.length === 0 && existing.session.products.length > 0;
    const blocked = draft.title.trim().length < 3
                ...
                : premiereNeedsTime
                    ...
                    : discountOutOfRange
                        ...
                        : emptiedLineUp
                            ...
                            : null;
    return (<form className="card mb-4 overflow-hidden" onSubmit={(event) => {
      ...
      <header className="flex flex-wrap items-baseline justify-between gap-2 border-b border-line px-3 py-2">
        ...
        <div className="flex items-center gap-2">
          {onAir && (<span className="badge-live">
      ...
      <div className="grid lg:grid-cols-3">
        <Column title="Details">
          <label className="mb-2 block">
            ...
            <input className="input" value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} placeholder="Festive Edit — handloom sarees" required/>
          </label>

          <label className="mb-2 block">
            <span className="label">Address</span>
            <input className="input" value={draft.slug} disabled={existing !== null} onChange={(event) => setDraft({ ...draft, slug: event.target.value })} placeholder="derived from the title if left blank" pattern="[a-z0-9][a-z0-9-]{1,60}"/>
          ...
          <label className="mb-2 block">
            ...
            <textarea className="input min-h-[64px]" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} placeholder="What shoppers see on the show card."/>
          </label>

          <label className="mb-2 block">
            <span className="label">Host name</span>
            <input className="input" value={draft.hostName} onChange={(event) => setDraft({ ...draft, hostName: event.target.value })}/>
          </label>

          <label className="mb-2 block">
            <span className="label">Language</span>
            <select className="input" value={draft.language} onChange={(event) => setDraft({ ...draft, language: event.target.value })}>
              {languages.length === 0 ? (<option value="en-US">en-US</option>) : (languages.map((code) => (<option key={code} value={code}>
                    {code}
                  </option>)))}
            </select>
          </label>
          ...
          <label className="mb-2 block">
            ...
            <input className="input" value={draft.coverImageUrl} onChange={(event) => setDraft({ ...draft, coverImageUrl: event.target.value })} placeholder="/media/covers/festive-edit.jpg"/>
          </label>

          <label className="block">
            <span className="label">Expected peak viewers</span>
            <input type="number" min={1} className="input" value={draft.expectedPeakViewers} disabled={onAir} onChange={(event) => setDraft({ ...draft, expectedPeakViewers: event.target.value })}/>
        ...
        <Column title="Line-up" note={picked.length === 0
            ? 'only products here can carry the live price'
            : `${picked.length} selected${outOfStock === 0 ? '' : ` · ${outOfStock} out of stock`}`}>
          <input className="input mb-2" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search your catalog" aria-label="Search your catalog"/>

          {products.isLoading && (<div className="space-y-1">
              {[0, 1, 2, 3, 4].map((row) => (<div key={row} className="skeleton h-8"/>))}
            </div>)}
          {products.isError && (<p role="alert" className="text-13 text-danger">
              Could not read your catalog, so nothing can go on the line-up right now.
            </p>)}
          {products.isSuccess && catalogue.length === 0 && (<p className="text-13 text-t2">
              No products are assigned to you yet, so there is nothing to sell.
            </p>)}
          {products.isSuccess && catalogue.length > 0 && matches.length === 0 && (<p className="text-13 text-t2">Nothing in your catalog matches that.</p>)}

          {matches.length > 0 && (<ul className="max-h-[22rem] divide-y divide-line overflow-y-auto scroll-thin">
              {matches.map((product) => {
                const index = picked.indexOf(product.productId);
                const isPicked = index >= 0;
                return (<li key={product.productId} className="flex items-center gap-2 py-1">
                    <input type="checkbox" className="h-3.5 w-3.5 shrink-0 accent-accent" checked={isPicked} onChange={() => {
                        setPicked((current) => current.includes(product.productId)
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-1.5">
                        ...
                        {product.totalStock === 0 && (<span className="shrink-0 text-11 font-semibold text-danger">OUT</span>)}
                        {product.totalStock > 0 && product.lowStock && (<span className="shrink-0 text-11 font-semibold text-accent">LOW</span>)}
                      </span>
                      <span className="block truncate text-11 text-t3">
                        {product.brand} · {formatInr(product.priceMinorUnits)}
                      </span>
                    </span>
                    <label className={`flex shrink-0 items-center gap-1 text-11 ${isPicked ? 'text-t2' : 'text-t3'}`} title="On camera first">
                      <input type="radio" name="featured-product" className="h-3 w-3 accent-accent" disabled={!isPicked} checked={featured === product.productId} onChange={() => setFeatured(product.productId)}/>
          ...
          {outOfStock > 0 && (<p className="mt-1 text-11 font-medium text-danger">
        ...
        <Column title="Pricing & schedule">
          <fieldset className="mb-3">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              {DISCOUNT_PRESETS.map((percent) => (<button key={percent} type="button" className={liveDiscount === percent ? 'chip-active' : 'chip'} onClick={() => setDraft({
                ...draft,
                liveDiscountPercent: liveDiscount === percent ? '' : String(percent),
            })}>
                  −{percent}%
                </button>))}
              <input type="number" min={0} max={MAX_LIVE_DISCOUNT_PERCENT} className="input w-20" value={draft.liveDiscountPercent} onChange={(event) => setDraft({ ...draft, liveDiscountPercent: event.target.value })} placeholder="none" aria-label="Live price, percent off"/>
            </div>
            ...
            {discountOutOfRange && (<p className="field-error">
          ...
          <label className="mb-3 block">
            ...
            <input type="datetime-local" className="input" value={startMode === 'now' ? '' : draft.scheduledFor} disabled={startMode === 'now' || onAir} onChange={(event) => setDraft({ ...draft, scheduledFor: event.target.value })}/>
            <span className="mt-1 block text-11 leading-tight text-t3">
              {onAir
          ...
          {!onAir && (<fieldset className="mb-3">
              <legend className="label">Mode</legend>
              <div className="space-y-1">
                {START_MODES.filter((option) => existing === null || option.value !== 'now').map((option) => (<label key={option.value} className="flex cursor-pointer gap-2 rounded-ctl px-1 py-1 transition duration-ctl hover:bg-surface">
                      <input type="radio" name="start-mode" className="mt-0.5 h-3.5 w-3.5 shrink-0 accent-accent" checked={startMode === option.value} onChange={() => setStartMode(option.value)}/>
          ...
          <label className="block">
            ...
            <input ref={videoInputRef} type="file" accept="video/mp4,video/webm" className="input py-1 file:mr-2 file:rounded-chip file:border-0 file:bg-surface file:px-2 file:py-1 file:text-11 file:font-semibold file:text-t1" onChange={(event) => setVideoFile(event.target.files?.[0] ?? null)}/>
            <span className="mt-1 block text-11 leading-tight text-t3">
              {videoFile !== null
      ...
      <footer className="flex flex-wrap items-center gap-3 border-t border-line bg-surface px-3 py-2">
        ...
        {error !== null && (<span role="alert" className="text-13 text-danger">
            {error}
          </span>)}
      </footer>
    </form>);
