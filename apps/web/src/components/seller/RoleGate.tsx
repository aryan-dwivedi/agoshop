...
/**
 * The frame every Studio screen sits in: seller gate, page header, storefront scope.
 *
 * Studio contains only seller pages. This gate keeps customer sessions out even when
 * someone types a Studio URL directly. `theme` remains for the broadcast canvas when
 * a nested moving-media surface needs dark tokens.
 */
...
export const RoleGate = ({
  ...
  subtitle?: string;
  actions?: ReactNode;
  /** Storefront switcher: only rendered once the API reports more than one. */
  scope?: {
    sellers: SellerRef[];
    value: string | null;
    /** What "no explicit storefront" means on this page — the API decides, not the UI. */
    allLabel?: string;
    onChange: (sellerId: string | null) => void;
  };
  /** Studio is light by default; dark is reserved for a nested moving-media canvas. */
  theme?: 'dark' | 'light';
  children: ReactNode;
}): JSX.Element => {
  ...
  const frame = (body: ReactNode): JSX.Element => (
    <div data-theme={theme} className="min-h-full bg-bg px-4 py-3 text-t1">
      {body}
    </div>
  );
  ...
  if (loading) {
    return frame(
      <div className="mt-12 space-y-2" aria-busy="true" aria-label="Checking your session">
    ...
    <>
      <header className="mb-3 flex flex-wrap items-end justify-between gap-3 border-b border-line pb-3">
        <div className="min-w-0">
          ...
          {subtitle !== undefined && (
            <p className="mt-0.5 max-w-2xl text-13 leading-relaxed text-t2">{subtitle}</p>
          )}
        </div>
          ...
          {scope !== undefined && scope.sellers.length > 1 && (
            <label className="flex items-center gap-2 text-13 font-medium text-t2">
              ...
              <select
                ...
                value={scope.value ?? ''}
                onChange={(e) => scope.onChange(e.target.value === '' ? null : e.target.value)}
              >
