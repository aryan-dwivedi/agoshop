...
/**
 * Studio's command set, and the palette that runs it.
 *
 * Mounted above the shell rather than inside it, because the one screen that needs a
 * palette most — the broadcast room — deliberately drops the rail and renders
 * full-bleed. A palette that only existed alongside the rail would be missing from the
 * only place you cannot use a rail.
 *
 * Every show command acts on the show that is actually on air, and then puts you where
 * the consequence is visible: the room. Studio has no toasts outside the broadcast room
 * (§2.6), so "did that work?" is answered by the screen changing, not by a notification.
 */
...
/** For the rail's `⌘K` hint — the shortcut itself is owned by the palette. */
...
export const StudioCommands = (): JSX.Element => {
  ...
  /** The soonest thing that could be taken on air when nothing is live yet. */
  const upcoming =
    rows
      .filter((row) => row.status === 'scheduled' && row.scheduledFor !== null)
      .sort((a, b) => Date.parse(a.scheduledFor!) - Date.parse(b.scheduledFor!))[0] ?? null;

  // The pin bar is positional — `pin 3` means the third card on the rail — so the
  // palette needs the rail itself, and only while something is on air.
  const rail = useQuery({
    queryKey: ['session', live?.slug ?? 'none'],
    queryFn: async () =>
      (await api.get<{ session: LiveSessionDto }>(`/api/sessions/${live?.slug ?? ''}`)).session,
    enabled: live !== null,
    staleTime: 15_000,
  });
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    const destinations: { label: string; keywords: string; to: string }[] = [
    ...
    const show: Command[] = [
      {
        ...
        run: () =>
          navigate(live !== null ? `/live/${live.slug}` : `/live/${upcoming!.slug}/preflight`),
      },
      {
        id: 'pin',
        label: 'pin',
        ...
        run: async (argument) => {
          ...
          if (product === undefined) {
            throw new Error(
              `This show has ${lineUp.length} product${lineUp.length === 1 ? '' : 's'}, so there is no #${argument ?? 0}.`,
            );
          }
