...
export const StudioCommands = (): JSX.Element => {
    ...
    const commands = useMemo<Command[]>(() => {
        ...
        const destinations: {
            ...
            {
                label: 'Catalog',
                keywords: 'products inventory stock price',
                to: '/catalog',
            },
            {
                label: 'List a product',
                keywords: 'new product add listing',
                to: '/catalog/new',
            },
            { label: 'Orders', keywords: 'sales receipts payment', to: '/orders' },
            {
                label: 'Payouts',
                keywords: 'money settlement gross sales',
                to: '/payouts',
            },
            {
                label: 'Audience',
                keywords: 'moderation chat muted banned viewers',
                to: '/audience',
            },
        ];
        ...
        const show: Command[] = [
            {
                ...
                run: () =>
                    navigate(
                        live !== null ? `/live/${live.slug}` : `/live/${upcoming!.slug}/preflight`,
                    ),
            },
            {
                id: 'pin',
                ...
                run: async (argument) => {
                    ...
                    await api.post(`/api/sessions/${live!.id}/pin`, {
            ...
            },
            {
                ...
                run: async (argument) => {
                    ...
                    if (percent > MAX_LIVE_DISCOUNT_PERCENT) {
                        throw new Error(
                            `A live markdown cannot exceed ${MAX_LIVE_DISCOUNT_PERCENT}%.`,
                        );
                    }
