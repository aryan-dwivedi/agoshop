...
/**
 * Studio's command palette.
 *
 * The rail teaches the console; this is the half that is usable while you are talking
 * to a camera and cannot look away from the lens. Everything here is keyboard-only by
 * construction: no command needs a pointer, nothing is hover-only, and the list is a
 * real `listbox` so a screen reader reads what the eye reads.
 *
 * Three rules this component enforces so no caller has to remember them:
 *
 *  1. A command that cannot run right now is *shown, disabled, with the reason*. An
 *     absent command reads as "not built"; a disabled one reading "no show is live"
 *     reads as "not now", which is the truth.
 *  2. A command carrying `confirm` never fires on `Enter`. It swaps the list for a
 *     confirmation row and waits for a second, deliberate keystroke. Ending a show is
 *     one shortcut away from every screen in Studio; it must not be one shortcut away
 *     from happening by accident.
 *  3. A failing command keeps the palette open and says why. Studio has no toasts
 *     outside the broadcast room, so this is the only place the failure can appear —
 *     and a write that silently did nothing is the worst outcome available.
 */

/** `⌘` on Apple keyboards, `Ctrl` everywhere else — the palette prints what you press. */
export const MOD_LABEL: string =
  typeof navigator !== 'undefined' && /Mac|iPhone|iPad/.test(navigator.userAgent) ? '⌘' : 'Ctrl';

export type Command = {
  id: string;
  /** The verb, as typed: `pin`, `price`, `end show`, `Catalog`. */
  label: string;
  /** Words that should match but are not worth showing. */
  keywords?: string;
  /** What it will do, in the seller's words. One line. */
  hint?: string;
  /** Non-null → rendered disabled, with this sentence as the reason. */
  disabledReason?: string | null;
  /** Present → `Enter` opens a confirmation row instead of running. */
  confirm?: string;
  /** Commands that take a number, e.g. `pin 3`, `price 20`. */
 ...
 *
 * Ranking by string position alone puts `Promotions` above `Payouts` for the query
 * `pay`, which is wrong twice over: later *and* less contiguous. Runs of adjacent
 * characters cost nothing; every gap costs, and a gap away from the start costs more.
 */
...
/** `price 20` → the command words, and the number they carry. */
const splitArgument = (query: string): { words: string; argument: number | null } => {
...
export const CommandPalette = ({
  ...
  // Mounted on every Studio screen, so the shortcut lives here rather than in eight
  // pages that would each have to remember it.
  useEffect(() => {
  ...
  const matches = useMemo(() => {
    ...
    return commands
      ...
      .filter((row): row is { command: Command; rank: number } => row.rank !== null)
          ...
          <>
            <div className="flex items-center gap-2 border-b border-line px-3">
              ...
              <input
                ...
                aria-activedescendant={active === null ? undefined : `command-${active.id}`}
            ...
            {failure !== null && (
              <p role="alert" className="border-b border-line px-3 py-2 text-13 text-danger">
                {failure}
              </p>
            )}
                ...
                return (
                  <li key={command.id}>
                    ...
                    <div
                      ...
                      className={`flex min-h-ctl cursor-default items-center gap-3 px-3 py-1.5 ${
                    ...
                    >
                      ...
                      >
                        <span className="font-medium">
                          ...
                          {command.argument !== undefined && (
                            <span className={argument === null ? 'text-t3' : 'text-accent'}>
                              {' '}
                              {argument ?? command.argument.placeholder}
                            </span>
                          )}
                        </span>
                        {(command.disabledReason ?? command.hint) !== undefined && (
                      ...
                      {selected && !disabled && (
                        <span className="shrink-0 text-11 font-semibold text-t3">↵</span>
                      )}
                    </div>
                  </li>
                );
