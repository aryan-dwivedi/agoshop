                ...
                {isLoading ? (
                    <div className="flex gap-3 overflow-hidden">
                        {[0, 1, 2, 3, 4, 5].map((i) => (
                            <div
                                key={i}
                                className="w-[11.5rem] shrink-0 sm:w-[12.5rem]"
                            >
                                <TileSkeleton />
                            </div>
                        ))}
                    ...
                    <div
                        ref={scroller}
                        className="scroll-none flex snap-x gap-2 overflow-x-auto"
                    >
                        {products.map((p) => (
                            <div
                                key={p.id}
                                className="w-[11.5rem] shrink-0 snap-start sm:w-[13rem]"
                            >
                                <ProductCard
                                    product={p}
                                    badge={badgeFor?.(p)}
    ...
    >
        {[0, 1, 2, 3, 4].map((i) => (
            ...
            >
                <path
                    d={STAR_PATH}
                    className={i < filled ? 'fill-accent-text' : 'fill-line'}
                />
            </svg>
        ))}
    </span>
                ...
                {priceBounds === undefined ? (
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            ...
                            <button
                                type="submit"
                                className="btn-standard shrink-0"
                            >
                                Go
                            </button>
                        </form>
    ...
    return (
        <aside className="w-full shrink-0 lg:w-56 xl:w-64">
            ...
            >
                Filters
                <ChevronDown
                    className={`h-4 w-4 transition duration-ctl ${open ? 'rotate-180' : ''}`}
                />
            </button>
...
export const InfiniteScrollTrigger = ({
    ...
    return (
        <div
            ref={triggerRef}
            className="flex min-h-20 flex-col items-center justify-center gap-2 pt-4"
        >
            {loading ? (
                <>
