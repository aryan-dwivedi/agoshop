...
/**
 * Catalog list wire shapes. `packages/shared` carries the item DTOs; these two
 * envelopes are storefront-only, so they live with the components that read them.
 */
...
export type ProductFacetsDto = {
  ...
  priceMinorUnits: { min: number; max: number };
  ratingBuckets: { minRating: number; count: number }[];
};
...
export const SORT_LABELS: { value: ProductSort; label: string }[] = [
...
/** Reads the facet state a listing page keeps in its URL. */
...
/** Facet state -> the exact `GET /api/products` query string the API accepts. */
export const productsQueryString = (
  facets: ListingFacetState,
  extra: { category?: string; q?: string },
): string => {
  const query = new URLSearchParams({
    sort: facets.sort,
    pageSize: String(PAGE_SIZE),
  });
  if (extra.category !== undefined) query.set('category', extra.category);
  if (extra.q !== undefined && extra.q.length > 0) query.set('q', extra.q);
  if (facets.maxPriceMinorUnits !== '') {
    query.set('maxPriceMinorUnits', facets.maxPriceMinorUnits);
  }
  if (facets.minRating !== '') query.set('minRating', facets.minRating);
  if (facets.sellerId !== '') query.set('sellerId', facets.sellerId);
...
/** One tile's worth of skeleton: the same 4:3 picture and three lines under it. */
const TileSkeleton = (): JSX.Element => (
...
/**
 * Horizontal rail for home-page merchandising. Handles its own three states, and
 * scrolls by a viewport at a time so a rail is usable without a trackpad.
 */
  ...
  return (
    <section className="overflow-hidden rounded-panel bg-surface">
      <SectionHeader title={title} subtitle={subtitle} to={to} toLabel={toLabel}>
        {products !== undefined && products.length > 4 && (
          <span className="hidden items-center gap-1 sm:flex">
            <button
              type="button"
              aria-label={`Scroll ${title} left`}
              className="rounded-full border border-line p-1 text-t2 transition duration-ctl hover:border-line-ctl hover:text-t1"
              onClick={() => nudge(-1)}
            >
      ...
      <div className="p-3 md:p-4">
        {isLoading ? (
          <div className="flex gap-3 overflow-hidden">
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="w-[11.5rem] shrink-0 sm:w-[12.5rem]">
                <TileSkeleton />
              </div>
            ))}
          </div>
        ) : error !== null ? (
          <ErrorState title={`${title} did not load`} error={error} onRetry={onRetry} />
        ) : products === undefined || products.length === 0 ? (
          <EmptyState title={emptyTitle} body={emptyBody} />
        ) : (
          <div ref={scroller} className="scroll-none flex snap-x gap-2 overflow-x-auto">
            {products.map((p) => (
              <div key={p.id} className="w-[11.5rem] shrink-0 snap-start sm:w-[13rem]">
...
/** Responsive grid for category and search listings. */
export const ProductGrid = ({
  ...
  products: ProductDto[];
  compare?: { ids: string[]; onToggle: (productId: string) => void; max: number };
  badgeFor?: (product: ProductDto) => ReactNode;
}): JSX.Element => (
...
export const GridSkeleton = (): JSX.Element => (
  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
...
const buildPriceBrackets = (maxMinor: number): { label: string; value: string }[] => {
...
const FilterStars = ({ filled }: { filled: number }): JSX.Element => (
...
/**
 * One facet value. A chip rather than a checkbox: every one of these is single-select
 * against a URL parameter, and the chip is the system's own "filter value, selected or
 * not" primitive.
 */
const FacetChip = ({
  selected,
...
/**
 * The listing's left sidebar. Facets are URL state, never component state: a filtered
 * listing is shareable, survives reload, and a live-driven refetch cannot lose it.
 */
export const FacetSidebar = ({
  ...
  const panel = (
    <div className="card overflow-hidden">
      ...
      <FilterSection title="Price">
        {priceBounds === undefined ? (
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap gap-1.5">
              <FacetChip
                selected={maxPriceMinorUnits === ''}
                onSelect={() => onChange({ maxPriceMinorUnits: '' })}
              >
                Any price
              </FacetChip>
              {priceBrackets.map((bracket) => (
                <FacetChip
                  key={bracket.value}
                  selected={maxPriceMinorUnits === bracket.value}
                  onSelect={() => onChange({ maxPriceMinorUnits: bracket.value })}
                >
                  {bracket.label}
                </FacetChip>
              ))}
      ...
      <FilterSection title="Rating">
        <div className="flex flex-wrap gap-1.5">
          ...
          {RATING_CHOICES.map((choice) => (
            <FacetChip
              key={choice.value}
              selected={minRating === choice.value}
              onSelect={() => onChange({ minRating: choice.value })}
            >
      ...
      {facets !== undefined && facets.sellers.length > 1 && (
        <FilterSection title="Seller">
          <div className="flex flex-wrap gap-1.5">
            ...
            {facets.sellers.map((seller) => (
              <FacetChip
                key={seller.id}
                selected={sellerId === seller.id}
                onSelect={() => onChange({ sellerId: seller.id })}
              >
                {seller.name}
      ...
      {showCategories && facets !== undefined && facets.categories.length > 1 && (
          ...
          <div className="space-y-1">
            {facets.categories.map((category) => (
              <Link
                key={category.slug}
                to={`/c/${category.slug}`}
                className="link flex items-center justify-between gap-2 py-1 text-13"
              >
  ...
  return (
    <aside className="w-full shrink-0 lg:w-56 xl:w-64">
      <button
        type="button"
        className="btn-standard mb-3 w-full lg:hidden"
...
/** Results header for a growing infinite list. */
export const ListingToolbar = ({
  ...
  <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-line pb-3">
    ...
    <label className="flex items-center gap-2 text-14 text-t2">
      ...
      <select
        className="h-ctl rounded-ctl border border-line-ctl bg-transparent px-3 text-14 text-t1 focus:border-accent"
        value={sort}
...
/**
 * Loads the next result batch before the shopper reaches the end. The button remains
 * as a keyboard and reduced-capability fallback; neither path exposes page numbers.
 */
export const InfiniteScrollTrigger = ({
  ...
  return (
    <div ref={triggerRef} className="flex min-h-20 flex-col items-center justify-center gap-2 pt-4">
      {loading ? (
        <>
        ...
        <button type="button" className="btn-standard btn-sm" onClick={onLoadMore}>
          Load more products
        </button>
      ) : (
