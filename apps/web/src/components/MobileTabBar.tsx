...
/**
 * Under 640px the top bar keeps only the wordmark, search, the live pill and the
 * avatar; everything with a destination moves down here, where a thumb is. Four
 * tabs, 44px targets, no hamburger and no floating button — §2.2.
 *
 * `Browse` and `You` open the same panels the desktop bar opens, so there is one
 * menu implementation per surface rather than a mobile fork.
 */
          ...
          <span className="flex h-5 items-center">
            <span
              className={`h-2 w-2 rounded-full ${
                liveCount > 0 ? 'bg-live animate-breathe' : 'bg-line-ctl'
              }`}
            />
          </span>
          Live
