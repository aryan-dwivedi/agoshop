...
const QtyStepper = ({ quantity, busy, onChange, }: {
    quantity: number;
    busy: boolean;
    onChange: (next: number) => void;
}): JSX.Element => (<div className="flex items-center rounded-ctl border border-line-ctl">
    <button type="button" aria-label="Reduce quantity" disabled={busy || quantity <= 1} onClick={() => onChange(quantity - 1)} className="flex h-9 w-9 items-center justify-center text-16 text-t2 disabled:opacity-40">
      −
    </button>
    <span aria-live="polite" className="tnum w-7 text-center text-14 font-medium text-t1">
      {quantity}
    </span>
    <button type="button" aria-label="Increase quantity" disabled={busy || quantity >= 10} onClick={() => onChange(quantity + 1)} className="flex h-9 w-9 items-center justify-center text-16 text-t2 disabled:opacity-40">
      +
    </button>
  </div>);
const Line = ({ line, busy, onQuantity, onRemove, }: {
    line: CartLineDto;
    busy: boolean;
    ...
    return (<li className="flex gap-3 px-4 py-3">
      <Link to={`/p/${line.productSlug}`} className="shrink-0">
        ...
        <span className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-chip bg-[#fff]">
          {line.imageUrl !== null && (<img src={line.imageUrl} alt={line.productTitle} className="h-full w-full object-contain"/>)}
        </span>
      </Link>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="shrink-0 text-right">
            ...
            {line.pricing.discountMinorUnits > 0 && (<p className="tnum text-13 text-t3 line-through">
                {formatInr(line.pricing.grossMinorUnits)}
              </p>)}
          </div>
        </div>

        {line.liveSessionId !== null && (<p className="mt-1.5 flex items-center gap-1.5 text-13">
            {line.liveEligible ? (<>
        ...
        {line.suppressed.length > 0 && (<ul className="mt-1.5 space-y-0.5 text-13 text-t3">
            {line.suppressed.map((s) => (<li key={`${s.code}-${s.reason}`}>{suppressionReason(s, appliedCodes)}</li>))}
          </ul>)}

        <div className="mt-2.5 flex items-center justify-between gap-3">
          <QtyStepper quantity={line.quantity} busy={busy} onChange={onQuantity}/>
          <button type="button" disabled={busy} onClick={onRemove} className="btn-quiet btn-sm text-danger hover:text-danger">
            Remove
          </button>
        </div>
      </div>
...
export const CartSheet = (): JSX.Element => {
    ...
    if (placed !== null) {
        return (<RightSheet label="Cart" title="Order placed" onClose={closeSheet}>
        <div className="p-4">
          ...
          <div className="mt-4 flex gap-2">
            <Link to={`/orders?highlight=${placed.orderId}`} className="btn-standard" onClick={closeSheet}>
              View order
            </Link>
    ...
    if (checkout !== null) {
        return (<RightSheet label="Checkout" title={<button type="button" onClick={() => setCheckout(null)} className="flex items-center gap-2 text-19 font-semibold text-t1">
            <ChevronLeft className="h-5 w-5"/>
            Checkout
          </button>} onClose={closeSheet}>
        <Suspense fallback={<div className="skeleton m-4 h-64"/>}>
          <CheckoutFlow compact onPlaced={(orderId) => setPlaced({ orderId, totalMinorUnits: checkout.totalMinorUnits })}/>
        </Suspense>
      </RightSheet>);
    }
    if (user === null) {
        return (<RightSheet label="Cart" title="Cart" onClose={closeSheet}>
        <div className="p-4">
    ...
    if (cart.isPending) {
        ...
        <div className="grid gap-3 p-4" aria-hidden="true">
          {[0, 1, 2].map((i) => (<span key={i} className="skeleton h-20"/>))}
        </div>
      </RightSheet>);
    }
    ...
    return (<RightSheet label="Cart" title={`Cart (${data.items.length})`} onClose={closeSheet} footer={<>
          <dl className="mb-3 grid gap-1.5 text-14">
            ...
            {data.totals.discountMinorUnits > 0 && (<div className="flex justify-between gap-4">
          ...
          <button type="button" className="btn-commit btn-lg tnum w-full" onClick={() => {
                if (inLiveRoom) {
          ...
          <Link to="/cart" onClick={closeSheet} className="link mt-3 block text-center text-13 text-t2">
            Open the full cart
          </Link>
        </>}>
      {data.notices.includes('live_discount_expired') && (<p role="alert" className="border-b border-line px-4 py-3 text-14 text-t2">
          The show ended, so its live price came off. Everything else is unchanged.
        </p>)}

      {mutationError !== null && (<p role="alert" className="border-b border-line px-4 py-3 text-14 text-danger">
          {mutationError.message}
        </p>)}

      <ul className="divide-y divide-line">
        {data.items.map((line) => (<Line key={line.id} line={line} busy={busy} onQuantity={(quantity) => setQuantity.mutate({ id: line.id, quantity })} onRemove={() => removeLine.mutate(line.id)}/>))}
      </ul>
    </RightSheet>);
};
