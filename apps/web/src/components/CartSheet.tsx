import type { CartDto, CartLineDto, SuppressedPromotion } from '@shop/shared';
...
const QtyStepper = ({
    ...
    <div className="flex items-center rounded-ctl border border-line-ctl">
        ...
        <span
            aria-live="polite"
            className="tnum w-7 text-center text-14 font-medium text-t1"
        >
            {quantity}
        </span>
        <button
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                        <Link
                            to={`/p/${line.productSlug}`}
                            className="block truncate text-14 text-t1"
                        >
                            {line.productTitle}
                        </Link>
                    ...
                    <ul className="mt-1.5 space-y-0.5 text-13 text-t3">
                        {line.suppressed.map((s) => (
                            <li key={`${s.code}-${s.reason}`}>
                                {suppressionReason(s, appliedCodes)}
                            </li>
                        ))}
                    </ul>
                )}
            ...
            >
                <div className="p-4">
                    ...
                    <p className="mt-1 text-14 text-t2">
                        The show is still running behind this sheet.
                    </p>
    ...
    return (
        ...
        >
            {data.notices.includes('live_discount_expired') && (
                <p
                    role="alert"
                    className="border-b border-line px-4 py-3 text-14 text-t2"
                >
            ...
            {mutationError !== null && (
                <p
                    role="alert"
                    className="border-b border-line px-4 py-3 text-14 text-danger"
                >
