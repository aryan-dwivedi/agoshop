...
/** Wire values are terse; these are what the shopper reads. */
...
/**
 * `blockedReason` is a policy code the API also rejects checkout with; these are the
 * shopper-facing sentences for the four codes `availablePaymentMethods` can return.
 * An unknown code falls through to a plain sentence rather than printing the code.
 */
...
export const checkoutOptionsQuery = (
  pincode: string,
): { queryKey: [string, string]; queryFn: () => Promise<CheckoutOptionsDto> } => ({
  // Prefix ['checkout-options'] so the shell's live invalidation reaches every pincode.
  queryKey: ['checkout-options', pincode],
  queryFn: () =>
    api.get<CheckoutOptionsDto>(`/api/checkout/options?pincode=${encodeURIComponent(pincode)}`),
});

/**
 * Serviceability, ETA and payment methods for one PIN code, read from the same
 * `GET /api/checkout/options` the checkout page and the assistant use — so the PDP
 * promise and the checkout page can never disagree.
 */
    ...
    <div className="card overflow-hidden">
      ...
      <div className="p-4">
        ...
        {draft.length > 0 && !valid && (
          <p className="field-error">Indian PIN codes are 6 digits.</p>
        )}

        {options.isFetching && <p className="mt-3 text-13 text-t2">Checking {submitted}…</p>}

        {options.error !== null && !options.isFetching && (
          <p className="mt-3 text-13 text-danger">
            We could not check {submitted} just now. Try again in a moment.
          </p>
        )}

        {data !== undefined && !options.isFetching && (
          <div className="mt-3 animate-fade-in space-y-2.5 text-14">
            {data.pincodeServiceable === false ? (
              <p className="rounded-ctl bg-surface px-3 py-2 text-t2">
                We do not deliver to {submitted} yet. Try another PIN code.
              </p>
            ) : (
              <p className="flex items-start gap-2 rounded-ctl bg-success-wash px-3 py-2 font-medium text-success">
            ...
            {data.blockedReason !== null && (
              <p className="rounded-ctl bg-accent-wash px-3 py-2 text-13 font-medium text-accent-text">
            ...
            <div className="flex flex-wrap gap-1.5">
              {data.methods.length === 0 ? (
                <span className="text-13 text-t2">
