...
/**
 * The comparison tray. Bottom-anchored and full width: the assistant lives in a
 * sheet now, so nothing needs to be dodged and the table gets every pixel it needs.
 * Two products are the minimum the server will compare, so below that this is a tray
 * with a prompt, not a table.
 */
export const CompareDrawer = ({
  ...
  return (
    <aside className="fixed bottom-0 left-0 right-0 z-30 animate-slide-up rounded-t-sheet border border-line bg-elev shadow-sheet">
      <div className="mx-auto max-w-page px-4 py-3">
        ...
        {ids.length < COMPARE_MIN ? (
          <p className="mt-2.5 text-14 text-t2">
          ...
          <p className="mt-2.5 text-14 text-danger">
            We could not build this comparison. Remove a product and try again.
          </p>
        ) : comparison.data !== undefined ? (
          <div className="scroll-thin mt-2.5 max-h-[46vh] overflow-auto">
              ...
              <thead className="sticky top-0 bg-elev">
                <tr className="divide-x divide-line">
                  ...
                  {comparison.data.rows.map((row) => (
                    <th
                      key={row.productId}
                      className="border-b border-line px-3 py-2 text-left align-top"
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button
                          ...
                          className="shrink-0 rounded-chip p-0.5 text-t3 transition duration-ctl hover:bg-surface hover:text-t1"
              ...
              <tbody className="divide-y divide-line">
                <tr className="divide-x divide-line">
                  ...
                  {comparison.data.rows.map((row) => (
                    <td
                      key={row.productId}
                      className="tnum px-3 py-2 text-14 font-semibold text-t1"
                    >
                ...
                <tr className="divide-x divide-line bg-surface">
                  ...
                  {comparison.data.rows.map((row) => (
                    <td key={row.productId} className="px-3 py-2">
                      <Rating value={row.rating} />
                    </td>
                  ))}
                </tr>
                {comparison.data.attributes.map((attr, i) => (
                  <tr
                    key={attr}
                    className={`divide-x divide-line ${i % 2 === 1 ? 'bg-surface' : ''}`}
                  >
                    <th className="py-2 pr-3 text-left text-13 font-medium text-t2">{attr}</th>
                    {comparison.data.rows.map((row) => (
                      <td key={row.productId} className="px-3 py-2 text-t1">
