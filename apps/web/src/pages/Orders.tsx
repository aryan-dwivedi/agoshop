 ...
 * (`Layout` owns `px-3 md:px-5` and no vertical padding): an order history is a
 * receipt, read in daylight.
 */
...
/**
 * Money taken is the only success; anything else is neutral until it fails, and a
 * failure is red *text*, never a filled pill — that shape belongs to `LIVE`.
 */
    ...
    <article className={`card overflow-hidden ${highlighted ? 'border-accent' : ''}`}>
      ...
      <ul className="divide-y divide-line">
        {order.items.map((item, i) => (
          <li key={`${item.productId}-${i}`} className="flex flex-wrap gap-x-6 gap-y-2 p-4">
            {/* Order lines carry no image, so the tile carries the product's initial
              ...
              {item.appliedPromotionCodes.length > 0 && (
                <p className="mt-2 flex flex-wrap gap-1.5">
                  {item.appliedPromotionCodes.map((code) => (
                    <span key={code} className="badge-accent">
                      {code}
                    </span>
                  ))}
                </p>
