import type { CartDto, CartLineDto, ProductDto } from '@shop/shared';
...
const QuantityStepper = ({
    ...
    <div className="inline-flex items-center rounded-ctl border border-line-ctl">
        ...
        <span
            className="w-8 text-center text-14 font-medium tnum text-t1"
            aria-live="polite"
        >
            {line.quantity}
        </span>
            ...
            <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
                    <div className="min-w-0">
                        <Link
                            to={`/p/${line.productSlug}`}
                            className="link text-16 font-medium"
                        >
                            {line.productTitle}
                        </Link>
                            ...
                            <p className="mt-2">
                                {line.liveEligible ? (
                                    ...
                                    <span className="badge-neutral">
                                        Show ended — back to{' '}
                                        <span className="tnum">
                                            {formatInr(line.pricing.unitPriceMinorUnits)}
                                        </span>
                                    </span>
                                )}
                            </p>
                            ...
                            {line.pricing.unitMrpMinorUnits !== null && (
                                <>
                                    ...
                                    <span className="line-through">
                                        {formatInr(line.pricing.unitMrpMinorUnits)}
                                    </span>
                                    {' / unit'}
                                </>
                            )}
        ...
        >
            ...
            {liveActive && (
                <div className="animate-slide-down flex items-center gap-2.5 rounded-ctl border border-line bg-live-wash p-4">
                    ...
                    <p className="text-14 leading-relaxed text-t1">
                        <span className="font-semibold text-live">Live price on this cart.</span>{' '}
                        Check out while the show is on and the saving below stays on your order.
                    </p>
                </div>
            )}
                    ...
                    <ul className="divide-y divide-line">
                        {data.items.map((line) => (
                            <CartLine
                                ...
                                busy={busy}
                                onQuantity={(quantity) =>
                                    setQuantity.mutate({ id: line.id, quantity })
                                }
