...
const QuantityStepper = ({ line, busy, onQuantity, }: {
    line: CartLineDto;
    busy: boolean;
    onQuantity: (quantity: number) => void;
}): JSX.Element => (<div className="inline-flex items-center rounded-ctl border border-line-ctl">
    <button type="button" className="flex h-8 w-8 items-center justify-center rounded-l-ctl text-16 text-t2 transition duration-ctl hover:text-t1 disabled:cursor-not-allowed disabled:opacity-40" aria-label={`Decrease quantity of ${line.productTitle}`} disabled={busy || line.quantity <= 1} onClick={() => onQuantity(line.quantity - 1)}>
      −
    </button>
    <span className="w-8 text-center text-14 font-medium tnum text-t1" aria-live="polite">
      {line.quantity}
    </span>
    <button type="button" className="flex h-8 w-8 items-center justify-center rounded-r-ctl text-16 text-t2 transition duration-ctl hover:text-t1 disabled:cursor-not-allowed disabled:opacity-40" aria-label={`Increase quantity of ${line.productTitle}`} disabled={busy || line.quantity >= 10} onClick={() => onQuantity(line.quantity + 1)}>
      +
    </button>
  </div>);
const CartLine = ({ line, onQuantity, onRemove, busy, }: {
    line: CartLineDto;
    onQuantity: (quantity: number) => void;
    onRemove: () => void;
    busy: boolean;
    ...
    return (<li className="flex flex-col gap-4 p-4 sm:flex-row">
      <Link to={`/p/${line.productSlug}`} className="shrink-0">
        {line.imageUrl === null ? (<div className="flex h-24 w-24 items-center justify-center rounded-ctl border border-line bg-surface text-23 font-semibold text-t3">
            {line.productTitle.slice(0, 1).toUpperCase()}
          </div>) : (<img src={line.imageUrl} alt={line.productTitle} className="h-24 w-24 rounded-ctl border border-line bg-[#fff] object-cover"/>)}
      </Link>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
          <div className="min-w-0">
            ...
            {line.liveSessionId !== null && (<p className="mt-2">
                {line.liveEligible ? (<span className="pill bg-live-wash text-live">
                    <span className="h-1.5 w-1.5 rounded-full bg-live" aria-hidden="true"/>
                    Live price — locked while the show runs
                  </span>) : (<span className="badge-neutral">
                    Show ended — back to{' '}
          ...
          <div className="text-right">
            ...
            {line.pricing.discountMinorUnits > 0 && (<p className="text-13 tnum text-t3 line-through">
        ...
        {(line.applied.length > 0 || line.suppressed.length > 0) && (<dl className="mt-3 space-y-1 border-t border-line pt-3 text-13">
            {line.applied.map((a) => (<div key={a.code} className="flex items-baseline justify-between gap-3">
            ...
            {line.suppressed.map((s) => (<div key={`${s.code}-${s.reason}`}>
...
const Cart = (): JSX.Element => {
    ...
    const recommended = useQuery({
        ...
        queryFn: () => api.get<{
            items: ProductDto[];
        }>('/api/recommendations?basedOn=recently_viewed&limit=8'),
        enabled: user !== null,
    });
    ...
    if (user === null) {
        return (<div data-theme="light" className={PAPER}>
        <EmptyState title="Sign in to see your cart" body="Your cart follows you between devices, and a live show's price applies while the show is running." action={{ to: '/login?next=/cart', label: 'Sign in' }}/>
      </div>);
    }
    if (cart.isPending) {
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="card divide-y divide-line overflow-hidden">
            {[0, 1].map((i) => (<div key={i} className="flex gap-4 p-4">
    ...
    if (data.items.length === 0) {
        return (<div data-theme="light" className={`${PAPER} space-y-8`}>
        <EmptyState title="Your cart is empty" body="Add anything from the catalog, or join a live show — a line added while the show runs keeps its live price for as long as the show is on." action={{ to: '/live', label: 'See live shows' }}/>
        <ProductRail title="Pick up where you left off" subtitle="Recently viewed, newest first" products={recommended.data?.items} isLoading={recommended.isPending} error={recommended.error} emptyTitle="Nothing viewed yet" emptyBody="Open a product and it shows up here, so there is always a way back into the catalog."/>
      </div>);
    }
    return (<div data-theme="light" className={`${PAPER} space-y-5`}>
      ...
      {expired && (<div role="alert" className="animate-slide-down rounded-ctl border border-line bg-surface p-4" data-testid="live-discount-expired">
          <p className="text-14 leading-relaxed text-t1">
            The show ended, so its live price came off. Everything else is unchanged.
          </p>
        </div>)}

      {liveActive && (<div className="animate-slide-down flex items-center gap-2.5 rounded-ctl border border-line bg-live-wash p-4">
      ...
      {mutationError !== null && (<p role="alert" className="rounded-ctl border border-line bg-live-wash px-4 py-3 text-14 font-medium text-danger">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="card overflow-hidden">
          <ul className="divide-y divide-line">
            {data.items.map((line) => (<CartLine key={line.id} line={line} busy={busy} onQuantity={(quantity) => setQuantity.mutate({ id: line.id, quantity })} onRemove={() => removeLine.mutate(line.id)}/>))}
          </ul>
        </div>
        ...
        <aside className="h-fit lg:sticky lg:top-[calc(var(--bar-h)+16px)]">
          <div className="card overflow-hidden">
            ...
            : `−${formatInr(data.totals.discountMinorUnits)}`}
              ...
              {data.totals.discountMinorUnits > 0 && (<p className="badge-success mt-3">
