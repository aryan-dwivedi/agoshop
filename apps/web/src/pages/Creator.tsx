import type { ProductListDto } from '../components/ProductRail';
...
import { GridSkeleton, ProductGrid } from '../components/ProductRail';
...
const CoverCta = ({ session }: { session: LiveSessionDto }): JSX.Element | null => {
    if (!isPlayable(session)) {
        return (
            <p className="text-on-video text-14 font-medium">
    ...
    if (session.status === 'live') {
        return (
            <Link
                ...
                className="btn-commit btn-lg shrink-0"
            >
    ...
    return (
        <Link
            to={sessionHref(session)}
            className="btn on-video shrink-0"
        >
            ...
            <div className="scrim-bottom absolute inset-x-0 bottom-0 p-4 pt-10 sm:p-5 sm:pt-14">
                <div className="flex flex-wrap items-end justify-between gap-x-4 gap-y-3">
                    <div className="min-w-0">
                        ...
                        {starts !== null && (
                            <p className="text-on-video tnum mt-0.5 text-14 font-medium">
                                {starts}
                            </p>
                        )}
                    </div>
            ...
            <div className="flex flex-1 flex-col gap-1 px-1 pb-1">
                <h3 className="text-14 font-medium leading-snug text-t1">
                    {playable ? (
                        <Link
                            to={href}
                            className="transition-colors duration-ctl hover:text-accent"
                        >
                            {session.title}
                        </Link>
                    ) : (
                ...
                {!playable && (
                    <p className="text-13 text-t2">The replay will be here in a few minutes.</p>
                )}
            </div>
        </li>
    );
            ...
            {segments.map((segment) => {
                ...
                return (
                    ...
                    >
                        {segment.label}
                        {segment.count !== null && (
                            <span className="tnum text-t3">{segment.count}</span>
                        )}
                    </button>
                );
            })}
...
const AboutPanel = ({ creator, view }: { creator: Creator; view: CreatorView }): JSX.Element => {
    ...
    if (view.languages.length > 0) {
        facts.push({
            term: 'Hosted in',
            value: view.languages.map(languageName).join(', '),
        });
    }
    ...
    return (
        <div className="card p-5">
            ...
            {newest !== null && (
                ...
                >
                    {newest.status === 'live'
                        ? 'Watch the current show'
                        : `Latest show: ${newest.title}`}
...
const Creator = (): JSX.Element => {
    ...
    const profile = useQuery<CreatorSource, Error, CreatorView>({
        ...
        refetchInterval: (query) =>
            query.state.data?.sessions.some((s) => s.status === 'live') === true ? 30000 : false,
    });
                ...
                <section className="card flex flex-wrap items-center justify-between gap-x-4 gap-y-3 px-4 py-3">
                    <div className="min-w-0">
                        ...
                        {countdown(next.scheduledFor, nowMs) !== null && (
                            <p className="tnum mt-0.5 text-13 text-t2">
                                {countdown(next.scheduledFor, nowMs)}
                            </p>
                        )}
                    </div>
                    <RemindMe
