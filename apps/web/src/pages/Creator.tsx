...
/**
 * The creator profile. The first thing this page answers is whether this seller is on
 * air *right now*: the cover is their live stage if there is one, their next show's
 * cover if there is not, and their newest replay otherwise. Everything below it is one
 * navigation model — three segments, no second tab bar, no sub-nav.
 *
 * Dark surface throughout: the cover holds video, and §3.3 keeps anything next to live
 * state dark.
 */

/* ------------------------------------------------------------------ wire shapes */

/** `GET /api/sellers/:slug` — the profile header and every show this creator owns. */
type Creator = {
  id: string;
  slug: string;
  name: string;
...
/** What the loader hands the `select` below. */
type CreatorSource = { seller: Creator | null; sessions: LiveSessionDto[] };

const loadCreator = async (slug: string): Promise<CreatorSource> => {
  try {
    const res = await api.get<{ seller: Creator; sessions: LiveSessionDto[] }>(
      `/api/sellers/${encodeURIComponent(slug)}`,
    );
    return { seller: res.seller, sessions: res.sessions };
  } catch (err) {
    // A creator nobody has is a state, not a failure: an unknown slug (404) and a
    // malformed one (400) both land on the empty state, never on an error card.
...
type CreatorView = {
  ...
  /** Live stage, else next scheduled cover, else newest replay. */
...
const sessionWhen = (session: LiveSessionDto): string => {
  ...
  if (session.status === 'scheduled') return formatWhen(session.scheduledFor);
  const ended = formatWhen(session.endedAt);
  return ended === '' ? 'Recorded earlier' : `Recorded ${ended}`;
};

/**
 * The one countdown the design keeps: time until a *show starts*, never attached to a
 * price or a stock level, and never in the live red or the commit amber.
 */
const countdown = (iso: string | null, nowMs: number): string | null => {
  if (iso === null) return null;
  const ms = new Date(iso).getTime() - nowMs;
  if (!Number.isFinite(ms) || ms <= 0 || ms > 24 * 60 * 60 * 1000) return null;
...
/** An ended session with no finished recording has nothing to open. */
const isPlayable = (session: LiveSessionDto): boolean =>
  session.status !== 'ended' ||
  (session.recordingStatus === 'ready' && session.recordingUrl !== null);

const sessionHref = (session: LiveSessionDto): string =>
  session.status === 'ended' ? `/replay/${session.slug}` : `/live/${session.slug}`;

/** Below ten, an audience number is noise. */
const shownViewers = (session: LiveSessionDto): number | undefined =>
  session.status === 'live' && session.viewerCount >= 10 ? session.viewerCount : undefined;

/**
 * Server clock, so a device with a skewed clock counts down to the same moment as
 * everyone else. The skew is measured once per payload and the display ticks a minute
 * at a time — a second hand on a show that starts in two hours is motion for nothing.
 */
const useServerNow = (serverNowMs: number | null): number => {
  ...
  useEffect(() => {
    if (serverNowMs !== null) setSkewMs(serverNowMs - Date.now());
  }, [serverNowMs]);

  useEffect(() => {
    const timer = window.setInterval(() => setLocalMs(Date.now()), 30_000);
...
/**
 * `Remind me` promises exactly one thing: a calendar entry. There is no notification
 * service behind this page, so the button links out to Google Calendar and offers the
 * same event as a file — both built in the browser from the show's start time.
 */
const calendarLinks = (
  session: LiveSessionDto,
): { google: string; ics: string; filename: string } | null => {
  if (session.scheduledFor === null) return null;
  const start = new Date(session.scheduledFor);
  if (Number.isNaN(start.getTime())) return null;
  const end = new Date(start.getTime() + SHOW_MINUTES * 60_000);
...
const RemindMe = ({
  ...
  // A show whose start time has already gone by would write a calendar entry in the
  // past, so the row keeps the show and loses the button.
  if (links === null || time(session.scheduledFor) <= nowMs) return null;
  ...
  return (
    <div ref={wrap} className="relative">
      ...
      {open && (
        <div
          role="menu"
          aria-label="Add this show to a calendar"
          className="animate-slide-down absolute right-0 z-20 mt-2 w-72 rounded-ctl border border-line bg-menu p-2 shadow-sheet"
        >
          <a
            role="menuitem"
            href={links.google}
            target="_blank"
            rel="noreferrer"
            className="block rounded-ctl px-3 py-2 text-14 text-t1 hover:bg-surface"
...
const CoverCta = ({ session }: { session: LiveSessionDto }): JSX.Element | null => {
  if (!isPlayable(session)) {
    return (
      <p className="text-on-video text-14 font-medium">The replay will be here in a few minutes.</p>
    );
  }
  if (session.status === 'live') {
    return (
      <Link to={sessionHref(session)} className="btn-commit btn-lg shrink-0">
        <PlayIcon className="h-4 w-4" />
        Watch now
      </Link>
    );
  }
  // One commit colour per screen, and on this page it belongs to `Watch now`. An idle
  // cover gets the same shape on an over-video chip instead.
  return (
    <Link to={sessionHref(session)} className="btn on-video shrink-0">
    ...
    <section className="card relative overflow-hidden">
      <div className="aspect-video w-full overflow-hidden sm:aspect-[2/1] lg:aspect-[21/9]">
        {session.status === 'live' ? (
          ...
          <SessionCoverImage session={session} variant="hero" />
        )}
      </div>
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              {session.status === 'live' ? (
                ...
                <span className="pill on-video text-14">
                  {session.status === 'scheduled' ? 'Scheduled' : 'Replay'}
                </span>
              )}
              {viewers !== undefined && (
                <span className="pill on-video tnum text-14">
            ...
            <p className="text-on-video mt-0.5 text-14 font-medium">
              {session.title}
              {session.status !== 'live' && sessionWhen(session) !== '' && (
                <span> · {sessionWhen(session)}</span>
              )}
            </p>
            {starts !== null && (
              <p className="text-on-video tnum mt-0.5 text-14 font-medium">{starts}</p>
            )}
          </div>
...
/** No shows at all: the creator still needs a name, a rating and a catalog. */
const IdentityHeader = ({ creator }: { creator: Creator }): JSX.Element => (
...
const ShowTile = ({ session, nowMs }: { session: LiveSessionDto; nowMs: number }): JSX.Element => {
  const playable = isPlayable(session);
  const href = sessionHref(session);
  const starts = session.status === 'scheduled' ? countdown(session.scheduledFor, nowMs) : null;

  // The picture is a second route to the same place, so it leaves the tab order and the
  // reading order. The status badge sits outside that link: "LIVE" has to be announced.
  const picture =
    session.status === 'live' ? (
  ...
  return (
    <li className="card-hover flex flex-col gap-2.5 p-2.5">
      <div className="relative aspect-video w-full overflow-hidden rounded-ctl">
        {playable ? (
          <Link to={href} className="block h-full w-full" tabIndex={-1} aria-hidden="true">
            {picture}
          </Link>
        ) : (
          picture
        ...
        <div className="absolute left-2 top-2">
          <LiveBadge
            ...
            viewerCount={shownViewers(session)}
      ...
      <div className="flex flex-1 flex-col gap-1 px-1 pb-1">
        <h3 className="text-14 font-medium leading-snug text-t1">
          {playable ? (
            <Link to={href} className="transition-colors duration-ctl hover:text-accent">
              {session.title}
            </Link>
          ) : (
            session.title
  ...
  <section>
    ...
    <ul className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {sessions.map((session) => (
        <ShowTile key={session.id} session={session} nowMs={nowMs} />
      ))}
    </ul>
  </section>
);
...
type Segment = { id: SegmentId; label: string; count: number | null };
...
const Segments = ({
  ...
  // Three known keys, so a plain record holds the roving-tabindex focus targets.
      ...
      {segments.map((segment) => {
        ...
        return (
          <button
            key={segment.id}
            ref={(node) => {
              if (node === null) delete buttons.current[segment.id];
              else buttons.current[segment.id] = node;
            }}
            type="button"
            role="tab"
            id={`creator-tab-${segment.id}`}
            aria-selected={selected}
            aria-controls={`creator-panel-${segment.id}`}
...
const AboutPanel = ({ creator, view }: { creator: Creator; view: CreatorView }): JSX.Element => {
  const facts: { term: string; value: string }[] = [
    { term: 'Rating', value: `${creator.rating.toFixed(1)} out of 5` },
    { term: 'Products', value: String(creator.productCount) },
  ];
  if (view.showCount > 0) facts.push({ term: 'Shows', value: String(view.showCount) });
  ...
  // Prose, not a bio: the server stores no seller copy, so this states what it does
  // know and stops. Inventing a voice for a creator is worse than saying less.
  const shows =
    view.showCount === 0
      ? ''
  ...
  return (
    <div className="card p-5">
      ...
      {newest !== null && (
        <Link
          to={sessionHref(newest)}
          className="mt-5 inline-flex items-center gap-1 text-14 font-medium text-t1 hover:text-accent"
        >
...
const CreatorSkeleton = (): JSX.Element => (
...
const Creator = (): JSX.Element => {
  ...
  const profile = useQuery<CreatorSource, Error, CreatorView>({
    ...
    select: toView,
    // A room on air moves its audience number; nothing else on this page changes.
    refetchInterval: (query) =>
      query.state.data?.sessions.some((s) => s.status === 'live') === true ? 30_000 : false,
  });
  ...
  const catalog = useQuery<ProductListDto, Error>({
    ...
    queryFn: () =>
      api.get<ProductListDto>(`/api/products?sellerId=${creator?.id ?? ''}&pageSize=24`),
  ...
  const segments = useMemo<Segment[]>(() => {
    if (view === undefined || creator === null) return [];
    const rows: Segment[] = [];
    // A creator with no shows gets no empty shows tab: the catalog is the page.
    if (view.showCount > 0) rows.push({ id: 'shows', label: 'Shows', count: view.showCount });
    rows.push({
      id: 'catalog',
      label: 'Catalog',
  ...
  const active: SegmentId =
    segment !== null && segments.some((s) => s.id === segment)
      ? segment
      : (segments[0]?.id ?? 'catalog');

  if (profile.isPending) return <CreatorSkeleton />;

  if (profile.isError) {
    return (
      <div className="py-4">
        <ErrorState
          title="This creator could not be loaded."
          error={profile.error}
  ...
  if (view === undefined || creator === null) {
    return (
      <div className="py-4">
        <EmptyState
          title="We don’t have a creator at this link."
          body="The link may be out of date. Live shows are the quickest way to find someone selling right now."
          action={{ to: '/live', label: 'See what’s live' }}
        />
      </div>
    );
  }
  ...
  // The soonest show that has not come round yet; if every scheduled slot is already
  // past, the earliest one is still the next thing this creator will do.
  const next = view.scheduled.find((s) => time(s.scheduledFor) > nowMs) ?? view.next;

  return (
    <div className="space-y-4 py-4">
      {view.cover !== null ? (
        <Cover creator={creator} session={view.cover} nowMs={nowMs} />
      ) : (
        <IdentityHeader creator={creator} />
      )}

      {next !== null && (
        <section className="card flex flex-wrap items-center justify-between gap-x-4 gap-y-3 px-4 py-3">
          <div className="min-w-0">
            ...
            {countdown(next.scheduledFor, nowMs) !== null && (
              <p className="tnum mt-0.5 text-13 text-t2">{countdown(next.scheduledFor, nowMs)}</p>
            )}
          </div>
      ...
      {active === 'shows' && (
        <div
          role="tabpanel"
          id="creator-panel-shows"
          aria-labelledby="creator-tab-shows"
          tabIndex={0}
          className="space-y-6"
        >
          {view.live.length > 0 && (
            <ShowGroup title="On air now" sessions={view.live} nowMs={nowMs} />
          )}
          {view.scheduled.length > 0 && (
            <ShowGroup title="Coming up" sessions={view.scheduled} nowMs={nowMs} />
          )}
          {view.replays.length > 0 && (
            <ShowGroup title="Watch again" sessions={view.replays} nowMs={nowMs} />
          )}
        </div>
      )}
      ...
      {active === 'catalog' && (
        <div
          role="tabpanel"
          id="creator-panel-catalog"
          aria-labelledby="creator-tab-catalog"
          tabIndex={0}
        >
          {catalog.isPending ? (
            <GridSkeleton />
            ...
            <EmptyState
              title={`${creator.name} has nothing listed yet.`}
              body="Their shows are the place to see what they sell next."
              action={{ to: '/live', label: 'See what’s live' }}
            />
          )}
        </div>
      ...
      {active === 'about' && (
        <div
          role="tabpanel"
          id="creator-panel-about"
          aria-labelledby="creator-tab-about"
          tabIndex={0}
        >
