...
const Login = (): JSX.Element => {
  ...
  /**
   * Set by an operator persona's button, which reserves its console tab while the click
   * gesture is still live (see `reserveDashboardTab`) and hands it over here once the
  ...
  const submit = useMutation({
    mutationFn: async (credentials: { email: string; password: string; displayName?: string }) => {
      ...
      return api.post<{ user: PublicUser }>(path, credentials);
    },
    onSuccess: async () => {
      const landed = pendingTab.current?.show() ?? null;
      pendingTab.current = null;
      // A refused popup sent this tab to the console instead; routing it again here
      // would cancel that navigation.
      if (landed === 'self') return;
      // Identity changed: every cached server read belonged to the previous user.
      queryClient.clear();
      await refresh();
      navigate(next);
    },
    onError: () => {
      // Nothing was signed in, so a reserved console tab would only show a denied gate.
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-4 py-4 md:py-6">
      ...
      {/* The one place the demo identities live. One tap signs in as that persona. */}
            ...
            {DEMO_PERSONAS.map((persona) => (
              <li key={persona.email}>
                <button
                  ...
                  onClick={() => {
                    // Reserved inside the gesture: a popup opened after the sign-in
                    // request resolves is blocked by the browser.
