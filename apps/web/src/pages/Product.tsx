...
/**
 * The variant chip's price. Compact on purpose — the shop price with the MRP it is
 * marked down from, no percent, so a five-variant row still reads as a row of chips.
 * The ladder is built rather than compared inline so an MRP that is not a real
 * markdown is dropped here exactly as it is in the buy box.
 */
  ...
  return (
    <span className="tnum flex flex-wrap items-baseline gap-x-1.5">
      ...
      {ladder.mrpMinorUnits !== null && (
        <span className="text-11 text-t3 line-through">{formatInr(ladder.mrpMinorUnits)}</span>
      )}
    </span>
  );
};
...
/**
 * A fixed 4:5 tile with the picture contained inside it, which is the same box the
 * catalog tile crops to — so a shopper who taps a card never sees the product jump
 * or re-crop on arrival. `#fff` is the tile, not the theme: cut-out product shots are
 * authored against white.
 */
  ...
  return (
    <div className="space-y-3">
      <div className="card overflow-hidden p-3">
        {images.length === 0 ? (
          <CardImage product={product} className="aspect-[4/5] w-full rounded-ctl" />
        ) : (
        ...
        <div className="scroll-thin flex gap-2 overflow-x-auto pb-1">
          {images.map((src, i) => (
            <button
              ...
              className={`h-16 w-16 shrink-0 overflow-hidden rounded-ctl border bg-[#fff] transition duration-ctl ${
                i === active ? 'border-accent' : 'border-line hover:border-line-ctl'
              }`}
            >
...
const Product = (): JSX.Element => {
  ...
  const [added, setAdded] = useState<{ discountMinorUnits: number } | null>(null);

  const productQuery = useQuery({
    queryKey: ['product', slug],
    queryFn: () => api.get<{ product: ProductDto }>(`/api/products/${encodeURIComponent(slug)}`),
  });
  const product = productQuery.data?.product;

  const shows = useQuery({
    queryKey: ['sessions', 'live-scheduled'],
    queryFn: () => api.get<{ sessions: LiveSessionDto[] }>('/api/sessions?status=live,scheduled'),
  ...
  // The shell already caches this list, so the breadcrumb costs no extra request.
  ...
  /**
   * The show this product is in, if any: a room that is on air is the only source of
   * a live price, and a scheduled one is only ever an invitation to be there.
   */
  const show = useMemo(() => {
  ...
  const addToCart = useMutation({
    mutationFn: async (): Promise<CartDto> => {
      if (product === undefined || variant === undefined) throw new Error('Product not loaded');
      return api.post<CartDto>(
        '/api/cart/items',
        {
          ...
          quantity,
          // Sending the session id is what lets the server price tier 3; it re-checks
          // status and attachment and refuses a mismatch.
  ...
  const toggleWishlist = useMutation({
    mutationFn: async (): Promise<void> => {
      ...
      if (wishlisted) await api.del(`/api/wishlist?productId=${encodeURIComponent(product.id)}`);
      else await api.post('/api/wishlist', { productId: product.id });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['wishlist'] });
      void queryClient.invalidateQueries({ queryKey: ['recommendations'] });
      // Wishlisting can change which promotion the shopper qualifies for, so the
      // cart's own prices may move.
  ...
  if (productQuery.error !== null || product === undefined || variant === undefined) {
    return (
      <div data-theme="light" className="py-4 md:py-6">
        <ErrorState
          title="Product unavailable"
          error={productQuery.error ?? new Error('This product has no purchasable variant.')}
  ...
  /*
   * The buy box must quote what the button will charge. Adding from here sends
   * `liveSessionId`, so when a room on air features this product the shopper pays
   * tier 3 — quoting only tiers 1 and 2 would be the page disagreeing with its own CTA.
   *
   * The session rail prices its default variant, so tier 3 is only quoted when the
   * selected variant is the one that ladder was computed for. The alternative would
   * be re-deriving a price on the client, which is exactly what must never happen.
   */
  ...
  return (
    <div data-theme="light" className="space-y-8 py-4 md:py-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,42%)_minmax(0,1fr)] xl:grid-cols-[minmax(0,36%)_minmax(0,1fr)_20rem]">
        ...
        <div className="min-w-0 space-y-4">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 border-b border-line pb-3">
              ...
              {specs.length > 0 && (
                <a href="#specifications" className="link text-13">
                  Specifications
                </a>
              )}
            </div>
        ...
        <div className="space-y-3 lg:col-span-2 xl:col-span-1 xl:sticky xl:top-[calc(var(--bar-h)+1rem)] xl:self-start">
          <div className="card space-y-4 p-4">
            {/* One row, above the price, when a show has this product on its line-up. */}
            {show !== undefined && (
            ...
            <div className="space-y-2">
              {user === null ? (
                <Link to={`/login?next=/p/${product.slug}`} className="btn-commit btn-lg w-full">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  title={user === null ? 'Sign in to use your wishlist' : undefined}
                >
                  <HeartIcon
            ...
            {addError !== null && (
              <p role="alert" className="rounded-ctl bg-live-wash px-3 py-2 text-14 text-danger">
                {addError}
              </p>
            )}
      ...
      <ProductRail
        ...
        badgeFor={(p) =>
          compareIds.includes(p.id) ? <span className="badge-accent">Comparing</span> : null
        }
      />
            ...
            {similar.data.items
              ...
              .map((p) => (
                <button
                  ...
                  className={`${
                    compareIds.includes(p.id) ? 'chip-active' : 'chip'
                  } disabled:opacity-40`}
                >
                  {p.title}
                </button>
