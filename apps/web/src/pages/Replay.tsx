...
const MissingRecording = ({ recordingStatus }: {
    recordingStatus: string;
}): JSX.Element => {
    const pending = recordingStatus === 'recording' || recordingStatus === 'processing';
    return (<div role="status" className="flex aspect-video w-full flex-col items-center justify-center gap-2 rounded-panel bg-[#101210] p-6 text-center">
...
const PollResult = ({ poll }: {
    poll: PollDto;
}): JSX.Element => (<div className="rounded-ctl border border-line p-4">
    <p className="text-14 font-semibold text-t1">{poll.question}</p>
    <ul className="mt-3 space-y-3">
      {poll.options.map((option) => {
        const share = poll.totalVotes === 0 ? 0 : Math.round((option.votes / poll.totalVotes) * 100);
        ...
        return (<li key={option.id}>
            ...
            <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-bg">
              <div className={mine ? 'h-full rounded-full bg-accent' : 'h-full rounded-full bg-line-ctl'} style={{ width: `${share}%` }}/>
            </div>
          </li>);
    })}
    </ul>
...
const Replay = (): JSX.Element => {
    ...
    if (sessionQuery.isError || !session) {
        return (<div className="card mx-auto my-12 max-w-lg p-8 text-center">
    ...
    return (<div className="space-y-8 py-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        {!assistantOpen && (<button type="button" className="btn-standard" onClick={() => setAssistantOpen(true)}>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="min-w-0">
          {recordingUrl ? (<ReplayPlayer ref={playerRef} src={recordingUrl} poster={session.coverImageUrl} title={session.title} captions={transcript.data?.lines ?? []}/>) : (<MissingRecording recordingStatus={session.recordingStatus}/>)}
        </div>
        ...
        <aside className="card flex max-h-[32rem] min-h-[22rem] flex-col overflow-hidden">
          ...
          <form className="flex gap-2 border-b border-line p-3" onSubmit={(event) => {
            event.preventDefault();
            setSearch(query.trim());
        }}>
            <label className="relative min-w-0 flex-1">
              <SearchIcon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-t3"/>
              <input className="input pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search transcript" aria-label="Search the transcript"/>
            </label>
          ...
          <div className="scroll-thin min-h-0 flex-1 overflow-y-auto p-2">
            {displayedTranscript.isLoading && (<p className="p-2 text-13 text-t3">Loading transcript…</p>)}
            {displayedTranscript.isError && (<p className="p-2 text-13 text-danger">Transcript unavailable.</p>)}
            {displayedTranscript.data?.lines.length === 0 && (<p className="p-2 text-13 text-t3">
                {search ? `No moments match “${search}”.` : 'No transcript was captured.'}
              </p>)}
            <ul className="space-y-1">
              {displayedTranscript.data?.lines.map((line) => (<li key={line.id}>
                  <button type="button" className="w-full rounded-ctl px-2 py-2 text-left transition hover:bg-bg" onClick={() => {
      ...
      <section>
        ...
        <SessionProductRail sessionId={session.id} products={session.products} pinnedProductId={null} live={false} signedIn/>
      </section>

      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="space-y-5">
          {summary && (<section className="card p-5">
              <h2 className="section-title">Show highlights</h2>
              <p className="mt-3 text-14 leading-relaxed text-t2">{summary}</p>
            </section>)}
          {closedPolls.length > 0 && (<section className="card p-5">
              <h2 className="section-title">Poll results</h2>
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {closedPolls.map((poll) => (<PollResult key={poll.id} poll={poll}/>))}
              </div>
            </section>)}
          {session.products.length > 0 && (<div className="rounded-panel border border-line bg-accent-wash px-4 py-3">
        ...
        <div className="flex min-h-[32rem] flex-col gap-4 lg:sticky lg:top-20">
          {assistantOpen && (<AssistantPanel surface="replay" liveSessionId={session.id} contextLabel={session.title} onClose={() => setAssistantOpen(false)} className="min-h-[24rem]"/>)}
          <ChatPanel chat={chat} canSend={false} signedIn readOnly emptyHint="No chat was recorded for this show." className="min-h-[24rem]"/>
        </div>
      </div>
    </div>);
};
