...
import { useSellerSessions, useSessionModeration, type ModerationAction, type SellerSessionsDto, } from '../../lib/sellerApi';
...
const Entries = ({ sessionId }: {
    ...
    if (moderation.isError) {
        ...
        <button type="button" className="btn-standard mt-3" onClick={() => void moderation.refetch()}>
          Try again
        </button>
      </div>);
    }
    ...
    if (entries.length === 0) {
        return (<div className="card px-4 py-12 text-center">
    ...
    return (<div className="card overflow-x-auto">
      <table className="w-full min-w-[720px] text-13">
        ...
        <tbody className="divide-y divide-line">
          {entries.map((entry) => (<tr key={entry.id} className="transition hover:bg-bg">
...
const Body = ({ sessions }: {
    sessions: UseQueryResult<SellerSessionsDto>;
}): JSX.Element => {
    const [params, setParams] = useSearchParams();
    const rows = sessions.data?.sessions ?? [];
    const sellerId = params.get('sellerId');
    const requested = params.get('sessionId');
    const selected = requested !== null && rows.some((s) => s.id === requested) ? requested : (rows[0]?.id ?? null);
    return (<div className="space-y-5">
      <div className="card flex flex-wrap items-center gap-3 px-4 py-3">
        <label className="flex items-center gap-2 text-14 font-semibold text-t2">
          Session
          <select className="input min-w-[280px]" value={selected ?? ''} disabled={rows.length === 0} onChange={(e) => setParams(sellerId === null
            ? { sessionId: e.target.value }
            : { sessionId: e.target.value, sellerId })}>
            {rows.length === 0 && <option value="">No sessions</option>}
            {rows.map((s) => (<option key={s.id} value={s.id}>
                {s.title} — {s.status}
              </option>))}
          </select>
        </label>
        {sessions.isLoading && <span className="text-13 text-t3">Loading shows…</span>}
        {sessions.isError && (<span className="text-13 font-semibold text-danger">Could not load your shows.</span>)}
      </div>

      {selected === null ? (<div className="card px-4 py-12 text-center text-14 text-t2">
          {sessions.isSuccess
                ? 'There are no shows with audience activity yet.'
                : 'Choose a show when the list loads.'}
        </div>) : (<Entries key={selected} sessionId={selected}/>)}
    </div>);
};
const ModerationLog = (): JSX.Element => {
    ...
    return (<RoleGate roles={['seller']} title="Audience" subtitle="Review moderation actions taken during each show." scope={sessions.data === undefined
            ? undefined
            : {
