...
/** Server caps, mirrored so the obvious mistakes never cost a round trip. */
const MAX_LIST_ROWS = 8;
const MAX_VARIANTS = 12;

/**
 * Every repeatable row carries its own key.
 *
 * Indexes would do for rendering, but removing a middle row shifts every index below
 * it, which hands React a different row under the same key and moves the caret out of
 * whatever the seller was typing.
 */
...
type VariantDraft = {
  key: string;
  label: string;
  /** Blank is meaningful: the server generates a SKU from the slug when none is sent. */
  sku: string;
  /** Rupees as typed. Paise are derived once, at submit. */
  price: string;
  /** Rupees as typed; blank means "this variant is not marked down". */
  mrp: string;
  stock: string;
};
...
type VariantErrors = { label?: string; sku?: string; price?: string; mrp?: string; stock?: string };
...
type FormErrors = {
  ...
  description?: string;
  categorySlug?: string;
  /** Keyed by variant row key, so a message stays with its row across add/remove. */
  variants: Record<string, VariantErrors>;
};

/**
 * Client-side mirror of the server's validator.
 *
 * It exists to keep the seller out of a submit/reject loop, not to replace the server:
 * `unknown_category`, `sku_taken` and `seller_not_owned` can only be decided by the
 * database, and those come back as codes the failure panel translates.
 */
const validate = (draft: {
  ...
  if (draft.categorySlug === '') errors.categorySlug = 'Pick the department this listing sells in.';
  ...
  for (const variant of draft.variants) {
    ...
    if (variant.mrp.trim().length > 0) {
      ...
      // Same rule as the pricing editor: an equal MRP is not a markdown, and clamping it
      // silently would hide which of the two figures the storefront actually took.
...
/** Codes a seller can act on; anything else keeps the server's own sentence. */
...
/**
 * What the seller just published.
 *
 * The storefront link is a plain anchor, not a `<Link>`: the console is served on its
 * own port, so a router push would only rewrite the path on :5174 and land on a route
 * this app does not have. "Back to products" stays a `<Link>` — that one is local.
 */
const Published = ({
  product,
  onAnother,
...
const Form = ({
  ...
  // Messages stay hidden until the seller has actually tried to publish; a form that
  // scolds you for fields you have not reached yet is noise.
  ...
  /**
   * The API wants exactly one default variant, so dropping the flagged row promotes the
   * first survivor instead of sending a product nothing is default on. The last row is
   * never removable: a product with no variants has no price.
   */
  ...
  const submit = (): void => {
    ...
    const rows: NewProductVariantInput[] = variants.map((variant) => {
      ...
      return {
        ...
        ...(sku === '' ? {} : { sku }),
        // Rupees are what a seller types and paise are what the API stores, so the
        // crossing happens exactly once, here, through the shared rounding helper.
        priceMinorUnits: rupeesToMinorUnits(Number(variant.price)),
        // Explicit `null` rather than an omitted key: "no markdown" is a decision the
        // seller made, and 0 would read as a free list price.
    ...
    // Rows the seller left blank are not content, so they never reach the body.
                ...
                {highlights.map((row, index) => (
                  <div key={row.key} className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setHighlights((rows) =>
                          rows.length === 1 ? [textRow()] : rows.filter((r) => r.key !== row.key),
                        )
                      }
        ...
        <section className="card p-5">
          ...
          <div className="mt-4 space-y-3">
            {variants.map((variant, index) => {
              ...
              return (
                <div key={variant.key} className="rounded-lg border border-slate-200 p-3">
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <label className="block">
                      ...
                      <input
                        className="input"
                        value={variant.label}
                        onChange={(e) => patchVariant(variant.key, { label: e.target.value })}
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        value={variant.sku}
                        onChange={(e) => patchVariant(variant.key, { sku: e.target.value })}
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        inputMode="decimal"
                        value={variant.price}
                        onChange={(e) => patchVariant(variant.key, { price: e.target.value })}
                        placeholder="2499"
                      />
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        inputMode="decimal"
                        value={variant.mrp}
                        onChange={(e) => patchVariant(variant.key, { mrp: e.target.value })}
                        placeholder="none"
                      />
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        inputMode="numeric"
                        value={variant.stock}
                        onChange={(e) => patchVariant(variant.key, { stock: e.target.value })}
                      />
...
const NewProduct = (): JSX.Element => {
  ...
  // Same query the storefront shell uses, so the department list is usually already cached.
  ...
  // Only for the storefront switcher: a multi-storefront caller has to say which one the
  // listing belongs to, and this is the same cache entry the products table filled.
