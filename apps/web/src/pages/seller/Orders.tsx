...
const Orders = (): JSX.Element => {
    ...
    return (<RoleGate roles={['seller']} title="Orders" subtitle="Paid orders containing products from your storefront." theme="light" scope={query.data
            ? {
            ...
            : undefined}>
      {query.isLoading && (<div className="card space-y-2 p-4" aria-label="Loading orders" aria-busy="true">
          <div className="skeleton h-8 w-full"/>
          <div className="skeleton h-8 w-full"/>
          <div className="skeleton h-8 w-full"/>
        </div>)}

      {query.isError && (<div className="card p-5">
      ...
      {!query.isLoading && !query.isError && orders.length === 0 && (<div className="card p-8 text-center">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] border-collapse text-left text-13">
              ...
              <tbody className="divide-y divide-line">
                {orders.map((order) => (<tr key={order.id} className="align-top hover:bg-bg">
                    ...
                    <td className="max-w-md px-3 py-3">
                      <ul className="space-y-1.5">
                        {order.items.map((item) => (<li key={`${order.id}-${item.productId}-${item.variantLabel}`}>
                            <a href={customerUrl(`/p/${item.productSlug}`)} className="font-medium text-t1 hover:text-accent-text">
                              {item.productTitle}
                            </a>
