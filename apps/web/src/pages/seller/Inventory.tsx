...
import { useSellerProducts, useUpdateVariantPricing, type SellerProduct, type SellerProductsDto, type SellerProductVariant, type VariantPricingInput, } from '../../lib/sellerApi';
...
const VariantEditor = ({ productId, variant, save, }: {
    productId: string;
    variant: SellerProductVariant;
    ...
    const dirty = draft.mrp !== server.mrp || draft.price !== server.price || draft.stock !== server.stock;
    ...
    const invalid = !Number.isFinite(priceMinorUnits) || priceMinorUnits <= 0
    ...
    return (<>
      <tr>
        ...
        <td className="py-1.5 pr-2">
          <input type="text" inputMode="decimal" className="input w-24 px-2 py-1 text-right text-xs tabular-nums" value={draft.mrp} onChange={(e) => setDraft({ ...draft, mrp: e.target.value })} placeholder="none" aria-label={`MRP in rupees for ${variant.sku}`}/>
        </td>
        <td className="py-1.5 pr-2">
          <input type="text" inputMode="decimal" className="input w-24 px-2 py-1 text-right text-xs tabular-nums" value={draft.price} onChange={(e) => setDraft({ ...draft, price: e.target.value })} aria-label={`Price in rupees for ${variant.sku}`}/>
        </td>
        <td className="py-1.5 pr-2">
          <input type="text" inputMode="numeric" className="input w-20 px-2 py-1 text-right text-xs tabular-nums" value={draft.stock} onChange={(e) => setDraft({ ...draft, stock: e.target.value })} aria-label={`Stock for ${variant.sku}`}/>
        </td>
        <td className="py-1.5 pr-3">
          {invalid === null ? (<div className="flex flex-wrap items-baseline justify-end gap-1.5">
              {mrpMinorUnits !== null && off !== null && (<span className="tabular-nums text-slate-400 line-through">
            ...
            </div>) : (<div className="text-right text-slate-400">—</div>)}
        </td>
        <td className="py-1.5 text-right">
          <button type="button" className="btn-standard btn-sm" disabled={!dirty || invalid !== null || pending} onClick={() => save.mutate({
            productId,
      ...
      {(invalid !== null || failure !== null) && dirty && (<tr>
...
const Body = ({ products, sellerId, }: {
    ...
    const rows = useMemo(() => {
        ...
        const filtered = all.filter((p) => (!lowOnly || p.lowStock) &&
            (needle === '' ||
    ...
    const stockValue = all.reduce((sum, p) => sum + p.variants.reduce((s, v) => s + v.priceMinorUnits * v.stock, 0), 0);
    return (<div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Metric label="Products" value={nf.format(all.length)} hint={`${nf.format(all.reduce((count, product) => count + product.variants.length, 0))} variants`}/>
        <Metric label="Low stock" value={nf.format(lowCount)} hint={`At or below ${nf.format(threshold)} units across variants`}/>
        <Metric label="Out of stock" value={nf.format(outCount)} hint="Unavailable for checkout until stock is added."/>
        <Metric label="Stock at list price" value={formatInr(stockValue)} hint="Units on hand × current variant price."/>
      </div>

      <div className="card flex flex-wrap items-center gap-3 px-4 py-3">
        <input className="input max-w-xs" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by title, brand, category or SKU" aria-label="Filter inventory"/>
        <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
          <input type="checkbox" className="h-4 w-4 accent-accent" checked={lowOnly} onChange={(e) => setLowOnly(e.target.checked)}/>
          Low stock only
        </label>
      ...
      {all.length === 0 ? (<div className="card px-4 py-12 text-center">
        ...
        </div>) : rows.length === 0 ? (<div className="card px-4 py-10 text-center text-sm text-slate-600">
          Nothing matches that filter.
        </div>) : (<div className="card overflow-x-auto">
          <table className="w-full min-w-[760px] text-sm">
            <thead className="bg-slate-50 text-[11px] uppercase tracking-widest text-slate-500">
              <tr className="border-b border-slate-200">
                {COLUMNS.map((col) => (<th key={col.key} className={`px-4 py-2 font-semibold ${col.align === 'right' ? 'text-right' : 'text-left'}`}>
                    <button type="button" className="inline-flex items-center gap-1 transition hover:text-slate-900" onClick={() => {
                    setAscending(sortKey === col.key ? !ascending : true);
                    setSortKey(col.key);
                }} aria-sort={sortKey === col.key ? (ascending ? 'ascending' : 'descending') : 'none'}>
                      {col.label}
                      <span className={sortKey === col.key ? 'text-accent-text' : 'text-transparent'}>
                        {ascending ? '↑' : '↓'}
                      </span>
                    </button>
                  </th>))}
            ...
            <tbody className="divide-y divide-slate-200">
              {rows.map((p) => (<Fragment key={p.productId}>
                  <tr className="transition hover:bg-slate-50">
                    <td className="px-4 py-2.5">
                      <a href={customerUrl(`/p/${p.slug}`)} target="_blank" rel="noreferrer" className="font-semibold text-t1 hover:text-accent-text hover:underline">
                        {p.title}
                      </a>
                    ...
                    <td className="px-4 py-2.5 text-right tabular-nums text-slate-600">
                      ...
                      {p.rating === 0 ? (<span className="text-slate-400">—</span>) : (p.rating.toFixed(1))}
                    </td>
                    <td className="px-4 py-2.5 text-right">
                      <span className={`font-semibold tabular-nums ${p.totalStock === 0
                    ? 'text-rose-600'
                    : p.lowStock
                        ? 'text-accent-text'
                        : 'text-slate-900'}`}>
                      ...
                      {p.totalStock === 0 && (<span className="ml-2 pill bg-rose-50 text-rose-600">out</span>)}
                      {p.totalStock > 0 && p.lowStock && (<span className="badge-accent ml-2">low</span>)}
                    </td>
                    <td className="px-4 py-2.5 text-right">
                      <button type="button" className="text-13 font-semibold text-accent-text hover:underline" onClick={() => setExpanded(expanded === p.productId ? null : p.productId)}>
                  ...
                  {expanded === p.productId && (<tr className="bg-slate-50">
                      <td colSpan={6} className="px-4 py-3">
                        <table className="w-full text-xs">
                          ...
                          <tbody className="divide-y divide-slate-200">
                            {p.variants.map((v) => (<VariantEditor key={`${v.id}:${v.mrpMinorUnits ?? 'none'}:${v.priceMinorUnits}:${v.stock}`} productId={p.productId} variant={v} save={save}/>))}
                          </tbody>
                        </table>
...
const Inventory = (): JSX.Element => {
    ...
    return (<RoleGate roles={['seller']} title="Inventory" subtitle="Manage listings, prices, variants, and the stock available to shoppers." actions={<Link to="/catalog/new" className="btn-commit">
          List a product
