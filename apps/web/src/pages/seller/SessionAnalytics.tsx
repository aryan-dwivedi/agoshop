...
const Body = ({ sessionId }: { sessionId: string }): JSX.Element => {
    ...
    if (data === undefined) {
        return (
            <div className="card p-8 text-14 text-t3">No report is available for this show.</div>
        );
    }
    return (
        <div className="space-y-6">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                />
                <Metric
                    label="Checkout declines"
                    value={nf.format(data.checkoutDeclines)}
                />
                <Metric
                    label="Chat messages"
                    value={nf.format(data.chatMessages)}
                />
                <Metric
                    label="Reactions"
                    value={nf.format(data.reactions)}
                />
                <Metric
                    label="Poll votes"
                    value={nf.format(data.pollVotes)}
                />
            </div>

            <Panel
                title="Viewers over time"
                note="one point per minute"
            >
                <div className="p-4">
                    <ViewerChart
                        series={data.viewerSeries}
            ...
            <div className="grid gap-5 lg:grid-cols-2">
                ...
                <Panel
                    title="Top products"
                    note="while each product was featured"
                >
