...
const Panel = ({ title, note, children, }: {
    ...
    note?: string;
    children: ReactNode;
}): JSX.Element => (<section className="card overflow-hidden">
...
const DiscountTable = ({ discountByCode, }: {
    ...
    if (rows.length === 0) {
        return (<p className="px-4 py-8 text-center text-14 text-t3">
        No promotions were used during this show.
      </p>);
    }
    return (<table className="w-full text-13">
      ...
      <tbody className="divide-y divide-line">
        {rows.map(([code, minorUnits]) => (<tr key={code}>
...
const Body = ({ sessionId }: {
    ...
    return (<div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Metric label="Peak viewers" value={nf.format(data.peakViewers)} hint={`${nf.format(data.uniqueViewers)} unique viewers`}/>
        <Metric label="Average watch" value={`${Math.floor(data.avgWatchSeconds / 60)}m ${String(Math.round(data.avgWatchSeconds % 60)).padStart(2, '0')}s`}/>
        <Metric label="Sales" value={formatInr(data.gmvMinorUnits)} hint={`${nf.format(data.orders)} paid orders`}/>
        <Metric label="Conversion" value={`${(data.conversionRate * 100).toFixed(1)}%`} hint={`${nf.format(data.addToCarts)} added to cart`}/>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Panel title="Top products" note="while each product was featured">
          {data.topProducts.length === 0 ? (<p className="px-4 py-8 text-center text-14 text-t3">
              No product activity was recorded for this show.
            </p>) : (<table className="w-full text-13">
              <thead className="border-b border-line bg-bg text-left text-t3">
                <tr>
              ...
              <tbody className="divide-y divide-line">
                {data.topProducts.map((product) => (<tr key={product.productId} className="transition hover:bg-bg">
      ...
      {session !== null && (<div className="card flex flex-wrap items-center gap-3 px-4 py-3 text-13 text-t2">
          <span className="font-medium text-t1">{session.title}</span>
          <Link to={`/audience?sessionId=${sessionId}`} className="font-semibold text-accent-text hover:underline">
            Audience log →
          </Link>
          {session.status === 'ended' && session.recordingStatus === 'ready' && (<a href={customerUrl(`/replay/${session.slug}`)} target="_blank" rel="noreferrer" className="font-semibold text-accent-text hover:underline">
              Watch replay →
            </a>)}
        </div>)}
    </div>);
...
const SessionAnalytics = (): JSX.Element => {
    ...
    return (<RoleGate roles={['seller']} title="Show report" subtitle="Audience, engagement, product activity, and paid sales from this show." actions={<Link to="/shows" className="btn-standard">
          All shows
        </Link>}>
      {id === undefined ? (<div className="card p-6 text-sm text-slate-600">No session id in the URL.</div>) : (<Body sessionId={id}/>)}
    </RoleGate>);
};
