...
import {
  ...
  uploadSourceVideo,
  type EditableShow,
} from '../../components/seller/SessionScheduler';
...
/**
 * Shows — every show this storefront has run, is running, or has scheduled.
 *
 * The builder opens *into* this page rather than on a route of its own: a show is
 * always edited against the list it belongs to, and `?show=<id>` makes that state a
 * link, so Today's `Edit line-up` can point straight at it.
 *
 * A failed video upload never costs the seller the form. The show exists the moment
 * create returns, so the failure is carried as a retry row under the show's own row —
 * the file is still in memory, and the retry writes to the show that already exists
 * instead of inviting a second one.
 */

/**
 * Whether a show premieres by itself comes from the public session DTO, not the seller
 * row: one list read, indexed by slug, rather than a fetch per row.
 */
type BroadcastFlags = { autoStart: boolean; hasUploadedVideo: boolean };

/** A show whose video did not upload, held with its file so the retry has something to send. */
type FailedUpload = { sessionId: string; message: string; file: File };
    ...
    <tr className="h-[var(--row-h)] hover:bg-surface">
      <td className="max-w-[22rem] px-3">
        <span className="flex items-center gap-1.5">
          ...
          {broadcast?.hasUploadedVideo === true && (
            <span className="shrink-0 text-11 text-t3" title="A video stands in for the camera.">
              video
            </span>
          )}
        </span>
      ...
      <td className="px-3">
        <div className="flex items-center justify-end gap-2">
          {show.status === 'live' && (
            <Link to={`/live/${show.slug}`} className="btn-commit btn-xs">
              Open room
            </Link>
          )}
...
const Sessions = (): JSX.Element => {
  ...
  // The premiere and video markers, and the full DTO the builder prefills from.
  ...
  /**
   * The builder needs the whole session — its line-up, cover and live price — which the
   * seller row does not carry, plus the capacity hint, which the public DTO does not.
   */
  const prefillId = editingId ?? duplicateOf;
  const prefillRead = useQuery({
    queryKey: ['session', prefillId ?? 'none'],
    queryFn: async () =>
      (await api.get<{ session: LiveSessionDto }>(`/api/sessions/${prefillId ?? ''}`)).session,
    ...
    >
      {builderOpen &&
        (prefillId !== null && prefill === null ? (
          ...
          <SessionScheduler
            /** Remounts on the show being edited, so no draft outlives its subject. */
            key={prefillId ?? 'new'}
            show={prefill}
            ...
            onSaved={(result) => {
              ...
              // A failed upload is now a row on the saved show, so the form has nothing
      ...
      {sessions.isLoading && (
        <div className="card space-y-1 p-3" aria-busy="true" aria-label="Loading your shows">
          {[0, 1, 2, 3, 4].map((row) => (
              ...
              {visible.map((show) => (
                <>
                  <ShowRow
                    ...
                    onEdit={() =>
                      setParams(sellerId === null ? { show: show.id } : { show: show.id, sellerId })
                    }
                    onDuplicate={() => {
                      closeBuilder();
