...
const LiveHero = ({ session, canHost, }: {
    session: LiveSessionDto;
    canHost: boolean;
}): JSX.Element => (<section className="card animate-fade-in overflow-hidden lg:grid lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]">
    <div className="group relative block overflow-hidden bg-bg">
      ...
      <div className="absolute left-3 top-3">
        <LiveBadge status={session.status} viewerCount={session.viewerCount} deliveryTier={session.deliveryTier} onDark/>
      </div>
    </div>
    ...
    <div className="flex flex-col justify-center gap-3 p-5 md:p-6">
      ...
      {session.description && (<p className="line-clamp-2 text-13 leading-relaxed text-t2">{session.description}</p>)}

      <div className="flex flex-wrap items-center gap-2">
        {session.viewerCount >= 10 && (<span className="badge-neutral tnum">
      ...
      {session.products.length > 0 && (<div className="flex items-center gap-2">
          <span className="flex -space-x-2">
            {session.products
            .slice(0, 5)
            .map((p) => p.imageUrl === null ? null : (<img key={p.productId} src={p.imageUrl} alt="" loading="lazy" className="h-10 w-10 rounded-ctl border-2 border-white bg-bg object-cover shadow-e1"/>))}
          </span>
      ...
      <div className="flex flex-wrap items-center gap-2 pt-1">
        ...
        {canHost && (<a href={sellerUrl(`/live/${session.slug}`)} target="_blank" rel="noreferrer" className="btn-standard">
            Host console
          </a>)}
      </div>
    </div>
  </section>);
const IdleHero = (): JSX.Element => (<section className="card flex flex-col justify-center gap-3 p-6 md:p-8">
...
const HeroSkeleton = (): JSX.Element => (<section className="card overflow-hidden lg:grid lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]">
...
const SessionTile = ({ session, canHost, }: {
    session: LiveSessionDto;
    canHost: boolean;
}): JSX.Element => {
    const target = session.status === 'ended' ? `/replay/${session.slug}` : `/live/${session.slug}`;
    return (<li className="card-hover group flex flex-col overflow-hidden">
      <Link to={target} className="relative block overflow-hidden bg-bg">
        <div className="aspect-video w-full overflow-hidden">
          {session.status === 'live' ? (<LivePreviewMedia session={session} variant="tile"/>) : (<SessionCoverImage session={session} variant="tile"/>)}
        </div>
        <div className="absolute left-2.5 top-2.5">
          <LiveBadge status={session.status} viewerCount={session.viewerCount} deliveryTier={session.deliveryTier} peakViewers={session.peakViewers}/>
        </div>
      </Link>

      <div className="flex flex-1 flex-col gap-1.5 p-4">
        <Link to={target} className="line-clamp-2 text-14 font-semibold leading-snug text-t1 group-hover:text-accent-text">
          {session.title}
        </Link>
        ...
        <div className="mt-auto flex flex-wrap items-center gap-2 pt-3">
          <Link to={target} className={`${session.status === 'live' ? 'btn-commit' : 'btn-standard'} btn-sm`}>
            {session.status === 'live'
            ? 'Watch live'
          ...
          {canHost && session.status !== 'ended' && (<a href={sellerUrl(`/live/${session.slug}`)} target="_blank" rel="noreferrer" className="btn-quiet btn-sm">
              Host console
            </a>)}
          {session.products.length > 0 && (<span className="ml-auto text-13 font-medium text-t3">
...
const Rail = ({ status, title, blurb, empty, playable, canHost, }: (typeof RAILS)[number] & {
    canHost: boolean;
}): JSX.Element => {
    ...
    return (<section className="card overflow-hidden">
      ...
      <div className="p-4">
        {query.isLoading && (<ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[0, 1, 2].map((i) => (<li key={i} className="skeleton aspect-[4/5]"/>))}
          </ul>)}

        {query.isError && (<p className="text-14 text-danger">Shows could not be loaded. {query.error.message}</p>)}

        {sessions && sessions.length === 0 && <p className="text-14 text-t3">{empty}</p>}

        {sessions && sessions.length > 0 && (<ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {sessions.map((session) => (<SessionTile key={session.id} session={session} canHost={canHost}/>))}
          </ul>)}
      </div>
    </section>);
};
const LiveIndex = (): JSX.Element => {
    ...
    return (<div className="space-y-8 py-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Link to="/search" className="inline-flex items-center gap-1 text-14 font-semibold text-accent-text hover:underline">
          Browse the catalog
          <ChevronRight className="h-4 w-4"/>
        </Link>
      </header>

      {liveQuery.isLoading ? (<HeroSkeleton />) : onAir ? (<LiveHero session={onAir} canHost={canHost}/>) : (<IdleHero />)}

      {alsoLive.length > 0 && (<section className="card overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 border-b border-line px-4 py-3">
          ...
          <ul className="grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-3">
            {alsoLive.map((session) => (<SessionTile key={session.id} session={session} canHost={canHost}/>))}
          </ul>
        </section>)}

      {RAILS.map((rail) => (<Rail key={rail.status} {...rail} canHost={canHost}/>))}
    </div>);
};
