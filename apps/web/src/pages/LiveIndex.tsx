...
const LiveHero = ({
    ...
    <section className="card animate-fade-in overflow-hidden lg:grid lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]">
        ...
        <div className="flex flex-col justify-center gap-3 p-5 md:p-6">
            ...
            {session.description && (
                <p className="line-clamp-2 text-13 leading-relaxed text-t2">
                    {session.description}
                </p>
            )}
            ...
            {session.products.length > 0 && (
                <div className="flex items-center gap-2">
                    <span className="flex -space-x-2">
                        {session.products.slice(0, 5).map((p) =>
                    ...
                    <span className="text-13 font-medium text-t3">
                        {session.products.length} product
                        {session.products.length === 1 ? '' : 's'} in the line-up
                    </span>
                </div>
            )}
...
const SessionTile = ({
    ...
    return (
        <li className="card-hover group flex flex-col overflow-hidden">
            <Link
                to={target}
                className="relative block overflow-hidden bg-bg"
            >
                <div className="aspect-video w-full overflow-hidden">
                    {session.status === 'live' ? (
                        <LivePreviewMedia
            ...
            <div className="flex flex-1 flex-col gap-1.5 p-4">
                ...
                <p className="line-clamp-2 pt-0.5 text-13 leading-relaxed text-t3">
                    {session.description}
                </p>
            ...
            <div className="p-4">
                ...
                {query.isError && (
                    <p className="text-14 text-danger">
                        Shows could not be loaded. {query.error.message}
                    </p>
                )}
            ...
            <header className="flex flex-wrap items-end justify-between gap-3">
                <div className="max-w-2xl">
                    ...
                    <p className="mt-1 text-14 leading-relaxed text-t2">
                        See products demonstrated by hosts, ask questions, and add them to the same
                        cart you use across the store.
                    </p>
                </div>
                <Link
                    to="/search"
