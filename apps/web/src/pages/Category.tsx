...
const Category = (): JSX.Element => {
    ...
    return (<div className="space-y-4 py-4 md:py-6">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        {data !== undefined && (<p className="tnum text-14 text-t2">{data.total.toLocaleString('en-IN')} products</p>)}
      </header>

      <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
        <FacetSidebar maxPriceMinorUnits={facets.maxPriceMinorUnits} minRating={facets.minRating} sellerId={facets.sellerId} priceBounds={data?.facets.priceMinorUnits} facets={data?.facets} onChange={patch} onReset={() => setParams(new URLSearchParams(), { replace: true })}/>

        <div className="min-w-0 flex-1">
          {listing.isPending ? (<GridSkeleton />) : listing.error !== null ? (<ErrorState title="This category did not load" error={listing.error} onRetry={() => void listing.refetch()}/>) : data === undefined || products.length === 0 ? (<EmptyState title="Nothing matches these filters" body="Raise the price cap or drop the rating floor to widen the results." action={{ to: `/c/${slug}`, label: 'Clear filters' }}/>) : (<div className="card space-y-4 p-4">
