...
/**
 * Immersive live shopping room — orchestrates theatre playback, savings banner,
 * offline guard, and the tabbed room sidebar.
 */
...
const Live = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setCaptionNotice(null), 3_000);
  ...
  const liveAssistantExamples = [
    latestRoomText === ''
      ...
      : `What did ${latestRoomMessage?.displayName ?? 'the room'} mean by “${
          latestRoomText.length > 72 ? `${latestRoomText.slice(0, 72)}…` : latestRoomText
        }”?`,
  ...
  const chatDot =
    chat.status === 'ready' ? 'bg-success' : chat.status === 'error' ? 'bg-danger' : 'bg-accent';
