import type { CartDto, OrderDto, PaymentMethod } from '@shop/shared';
...
const classify = (err: unknown): Failure => {
    ...
    switch (err.code) {
        ...
        default:
            return {
                kind: 'other',
                message: OTHER_MESSAGES[err.code] ?? err.message,
            };
    }
};
const Step = ({
...
export const CheckoutFlow = ({
    ...
    const placeOrder = useMutation({
        mutationFn: async (): Promise<OrderDto> => {
            ...
            if (order.status === 'payment_failed' || order.status === 'expired') {
                throw new ApiError(402, 'payment_failed', 'Payment could not be completed', {
                    order,
                });
            }
            return order;
    ...
    if (cart.isPending) {
        return (
            ...
            >
                ...
                <div
                    className={
                        compact ? 'space-y-4' : 'grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]'
                    }
                >
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    {!pincodeReady ? (
                        <p className="text-14 text-t2">
                            Payment options and delivery dates need a PIN code.
                        </p>
                    ) : options.isPending ? (
                                ...
                                <span className="inline-flex items-center gap-1.5 font-medium text-success">
                                    ...
                                    {options.data.etaDays !== null && (
                                        <span className="font-normal text-t2">
                                            {' '}
                                            · in {options.data.etaDays}{' '}
                                            {options.data.etaDays === 1 ? 'day' : 'days'}
                                        </span>
                                    )}
                                </span>
                            )}
                ...
                <dl className="space-y-2 text-14">
                    <div className="flex justify-between gap-4">
                        ...
                        <dd className="font-medium tnum text-t1">
                            {formatInr(totals.subtotalMinorUnits)}
                        </dd>
                    </div>
                    <div className="flex justify-between gap-4">
    ...
    const failurePanel =
        failure === null ? null : (
            <section
                role="alert"
                className={`animate-slide-up rounded-ctl border border-line p-4 ${failure.kind === 'pricing_changed' ? 'bg-accent-wash' : 'bg-live-wash'}`}
            >
                ...
                {failure.kind === 'pricing_changed' && (
                    <>
                        <p className="text-14 font-medium leading-relaxed text-t1">
                            ...
                            <span className="tnum">
                                {formatInr(
                                    failure.totals?.totalMinorUnits ?? totals.totalMinorUnits,
                                )}
                            </span>{' '}
                            — confirm?
                        </p>
