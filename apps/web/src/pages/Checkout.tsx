...
import { BLOCKED_REASON_LABELS, PAYMENT_METHOD_LABELS, checkoutOptionsQuery, } from '../components/PincodeCheck';
...
const OTHER_MESSAGES: Record<string, string> = {
    ...
    payment_method_unavailable: 'That payment method is no longer available for this order — pick another one below.',
    ...
    switch (err.code) {
        ...
        case 'out_of_stock':
            return {
                kind: 'out_of_stock',
                message: err.message,
                variantId: (err.body as {
                    error?: {
                        variantId?: string;
                    };
                } | null)?.error?.variantId ?? null,
            };
...
const Step = ({ step, title, collapsed, children, }: {
    step: number;
    title: string;
    collapsed: boolean;
    children: ReactNode;
}): JSX.Element => collapsed ? (<div className="border-b border-line p-4 last:border-b-0">
      <p className="eyebrow mb-2">{title}</p>
      {children}
    </div>) : (<section className="card overflow-hidden">
...
export const CheckoutFlow = ({ compact = false, onPlaced, }: {
    compact?: boolean;
    ...
    if (user === null) {
        return (<div data-theme={compact ? undefined : 'light'} className={compact ? '' : PAPER}>
        <EmptyState title="Sign in to check out" body="Your cart and its prices are waiting — signing in takes a moment." action={{ to: '/login?next=/checkout', label: 'Sign in' }}/>
      </div>);
    }
    if (cart.isPending) {
        return (<div data-theme={compact ? undefined : 'light'} className={compact ? 'space-y-4' : `${PAPER} space-y-5`}>
    ...
    if (cart.error !== null || cart.data === undefined) {
        return (<div data-theme={compact ? undefined : 'light'} className={compact ? '' : PAPER}>
        <ErrorState title="Cart unavailable" error={cart.error ?? new Error('We could not load your cart just now.')} onRetry={() => void cart.refetch()}/>
      </div>);
    }
    if (cart.data.items.length === 0) {
        return (<div data-theme={compact ? undefined : 'light'} className={compact ? '' : PAPER}>
        <EmptyState title="Nothing to check out" body="Your cart is empty. Add a product — or join a live show and its price applies while the show runs." action={{ to: '/', label: 'Browse the catalog' }}/>
      </div>);
    }
    const items = cart.data.items;
    const totals = cart.data.totals;
    const soldOut = failure?.kind === 'out_of_stock'
        ? (items.find((l) => l.variantId === failure.variantId) ?? null)
        : null;
    const blockedReason = options.data !== undefined && options.data.blockedReason !== null
        ? (BLOCKED_REASON_LABELS[options.data.blockedReason] ?? options.data.blockedReason)
        : null;
    const shortfall = options.data !== undefined ? options.data.minOrderMinorUnits - totals.totalMinorUnits : 0;
    const belowMin = options.data !== undefined && shortfall > 0;
    const quoted = failure?.kind === 'pricing_changed' && failure.totals !== null ? failure.totals : totals;
    const oneScreen = compact && pincodeReady && selected !== null;
    const canPlace = pincodeReady &&
        selected !== null &&
        !belowMin &&
    ...
    const steps = (<>
      <Step step={1} title="Deliver to" collapsed={oneScreen}>
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input ref={pincodeRef} className="input w-40 pl-9 tnum" inputMode="numeric" maxLength={6} placeholder="PIN code" aria-label="Delivery PIN code" value={pincode} onChange={(e) => {
            setPincode(e.target.value.replace(/\D/g, '').slice(0, 6));
            setFailure(null);
        }}/>
          </div>
          {!pincodeReady ? (<p className="text-14 text-t2">Payment options and delivery dates need a PIN code.</p>) : options.isPending ? (<p className="text-14 text-t3">Checking delivery…</p>) : options.data !== undefined ? (<p className="text-14">
              {options.data.pincodeServiceable === false ? (<span className="font-medium text-danger">
                  We do not deliver here — try another PIN code.
                </span>) : (<span className="inline-flex items-center gap-1.5 font-medium text-success">
                  <TruckIcon className="h-4 w-4"/>
                  Delivers here
                  {options.data.etaDays !== null && (<span className="font-normal text-t2">
                      {' '}
                      · in {options.data.etaDays} {options.data.etaDays === 1 ? 'day' : 'days'}
                    </span>)}
                </span>)}
            </p>) : options.error !== null ? (<p className="text-14 font-medium text-danger">{options.error.message}</p>) : null}
        </div>
        {blockedReason !== null && (<div className="mt-3 rounded-ctl border border-line bg-live-wash p-3">
      ...
      <Step step={2} title="Pay with" collapsed={oneScreen}>
        <div ref={methodsRef}>
          {!pincodeReady ? (<p className="text-14 text-t2">Enter a PIN code to see how you can pay.</p>) : options.isPending ? (<p className="text-14 text-t3">Loading payment options…</p>) : methods.length === 0 ? (<div className="rounded-ctl border border-line bg-live-wash p-3">
              ...
              {methods.map((m) => (<label key={m} className={`flex min-h-ctl cursor-pointer items-center gap-3 rounded-ctl border px-3 text-14 font-medium transition duration-ctl ${selected === m
                    ? 'border-accent bg-accent-wash text-t1'
                    : 'border-line-ctl text-t2 hover:text-t1'}`}>
                  <input type="radio" name="paymentMethod" className="accent-accent" checked={selected === m} onChange={() => {
                    setMethod(m);
                    setFailure(null);
                }}/>
        ...
        {needsCard && (<div className="animate-slide-up mt-4 grid gap-3 border-t border-line pt-4 sm:grid-cols-2">
            <label className="sm:col-span-2">
              <span className="label">Card number</span>
              <input className="input tnum" inputMode="numeric" autoComplete="cc-number" placeholder="4111 1111 1111 1111" value={card.number} onChange={(e) => setCard({ ...card, number: e.target.value })}/>
            </label>
            <label>
              <span className="label">Expiry</span>
              <input className="input tnum" autoComplete="cc-exp" placeholder="12/29" value={card.expiry} onChange={(e) => setCard({ ...card, expiry: e.target.value })}/>
            </label>
            <label>
              <span className="label">CVV</span>
              <input className="input tnum" inputMode="numeric" autoComplete="cc-csc" maxLength={4} placeholder="123" value={card.cvv} onChange={(e) => setCard({ ...card, cvv: e.target.value })}/>
            </label>
          </div>)}
      </Step>
      ...
      <Step step={3} title="Review" collapsed={oneScreen}>
        <ul className="space-y-3">
          {items.map((line) => (<li key={line.id} className="flex items-center gap-3">
              {line.imageUrl === null ? (<span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-ctl border border-line bg-surface text-16 font-semibold text-t3">
                  {line.productTitle.slice(0, 1).toUpperCase()}
                </span>) : (<img src={line.imageUrl} alt="" className="h-12 w-12 shrink-0 rounded-ctl border border-line bg-[#fff] object-cover"/>)}
              <span className="min-w-0 flex-1">
                ...
                <span className="block text-13 text-t2">
                  ...
                  {line.liveEligible && (<span className="font-medium text-live"> · live price</span>)}
                </span>
              </span>
              <span className="shrink-0 text-right">
                ...
                {line.pricing.discountMinorUnits > 0 && (<span className="block text-11 tnum text-t3 line-through">
    ...
    const summary = (<div className="card overflow-hidden">
      ...
      <div className="p-4">
        ...
        {belowMin && (<div className="mt-3 rounded-ctl border border-line bg-surface p-3">
            <p className="text-14 font-medium text-t1">
        ...
        <button type="button" className="btn-commit btn-lg mt-4 w-full" disabled={!canPlace || placeOrder.isPending} onClick={() => {
            setAttempted(totals);
            placeOrder.mutate();
        }}>
    ...
    const failurePanel = failure === null ? null : (<section role="alert" className={`animate-slide-up rounded-ctl border border-line p-4 ${failure.kind === 'pricing_changed' ? 'bg-accent-wash' : 'bg-live-wash'}`}>
        ...
        {failure.kind === 'out_of_stock' && (<>
            ...
            {soldOut === null ? (<Link to="/cart" className="btn-standard btn-sm mt-3">
                Review cart
              </Link>) : (<button type="button" className="btn-danger btn-sm mt-3" disabled={removeLine.isPending} onClick={() => removeLine.mutate(soldOut.id)}>
                Remove it
              </button>)}
          </>)}
        ...
        {failure.kind === 'pricing_changed' && (<>
            ...
            {failure.totals !== null && attempted !== null && (<dl className="mt-3 flex items-baseline gap-3 text-14">
        ...
        {failure.kind === 'rate_limited' && (<>
            ...
            <button type="button" className="btn-standard btn-sm mt-3" disabled={placeOrder.isPending} onClick={() => {
                setAttempted(totals);
                placeOrder.mutate();
            }}>
              Try again
    ...
    return (<div data-theme="light" className={`${PAPER} space-y-5`}>
