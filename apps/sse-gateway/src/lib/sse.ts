...
/**
 * Cross-replica SSE fan-out for the dedicated gateway tier.
 *
 * ONE Redis subscriber connection per gateway process (never one per browser). The
 * process reference-counts SUBSCRIBE/UNSUBSCRIBE for the channels its currently
 * connected clients need, and holds `events:global` permanently.
 */

type Client = { res: Response; userId: string; sessionIds: string[] };
...
/** End every open stream so nginx can drain the replica on SIGTERM. */
