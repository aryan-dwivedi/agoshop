#!/usr/bin/env node
/**
 * Native stack manager — the Docker-free equivalent of `infra/docker-compose.yml`.
 *
 * `npm run stack:up` writes `.stack/nginx.conf` and starts nginx on :8080 in front of
 * locally running API replicas (8787/8788) and SSE gateway replicas (8789/8790).
 * Redis is checked and reported; start it yourself (`brew services start redis` or
 * `redis-server`) if it is not already listening.
 *
 *   npm run stack:up      # generate config + start nginx
 *   npm run stack:down    # stop nginx started by this script
 *   npm run stack:status  # print upstream targets and process state
 */
...
const recordingsDir =
  process.env.RECORDING_LOCAL_DIR ??
...
const redisOk = () =>
  spawnSync('redis-cli', ['ping'], { stdio: 'pipe' }).stdout?.toString().trim() === 'PONG';
...
const startNginx = () => {
  ...
  if (nginxRunning()) {
    ...
    if (reload.status !== 0) process.exit(reload.status ?? 1);
...
const status = () => {
  ...
  console.log(
    `  nginx: ${nginxRunning() ? `up (pid ${readFileSync(pidFile, 'utf8').trim()})` : 'down'}`,
  );
  ...
  if (existsSync(nginxConfPath)) console.log(`  config: ${nginxConfPath}`);
};

const up = () => {
  if (!redisOk()) {
    console.warn(
      'stack: WARNING redis is not reachable at redis-cli default — start Redis before the API',
    );
  }
  startNginx();
  status();
