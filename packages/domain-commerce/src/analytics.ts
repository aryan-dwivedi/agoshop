    ...
    const rows = await db
        ...
        .where(
            and(
                ...
                eq(analyticsEvents.type, 'product_pinned'),
            ),
        )
...
const reactionTotal = async (sessionId: string): Promise<number> => {
    ...
    return countOf(
        db
            ...
            .from(analyticsEvents)
            .where(
                and(eq(analyticsEvents.sessionId, sessionId), eq(analyticsEvents.type, 'reaction')),
            ),
    );
};
export const sessionAnalytics = async (sessionId: string): Promise<SessionAnalyticsDto> => {
    ...
    const [
        ...
        countOf(
            db
                ...
                .where(
                    and(
                        eq(analyticsEvents.sessionId, sessionId),
                        eq(analyticsEvents.type, 'session_join'),
                    ),
                ),
        ),
        countOf(
            db
                .select({
                    value: sql`coalesce(sum((${analyticsEvents.payload} ->> 'viewers')::int), 0)`,
                })
                .from(analyticsEvents)
                .where(
                    and(
                        eq(analyticsEvents.sessionId, sessionId),
                        eq(analyticsEvents.type, 'viewer_sample'),
                    ),
                ),
        ),
        countOf(
            db
                ...
                .from(chatMessages)
                .where(
                    and(eq(chatMessages.sessionId, sessionId), eq(chatMessages.status, 'visible')),
                ),
        ),
        ...
        db
            ...
            .where(
                and(
                    eq(analyticsEvents.sessionId, sessionId),
                    eq(analyticsEvents.type, 'add_to_cart'),
                ),
            ),
    ...
    for (const window of windows) {
        const minutes = Math.max(
            0,
            Math.round((window.to.getTime() - window.from.getTime()) / 60000),
        );
    ...
    return {
        ...
        discountByCode,
        viewerSeries: viewerSeriesRows.map((r) => ({
            minute: r.minute,
