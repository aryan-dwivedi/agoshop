...
/**
 * The evaluator is the only place discount semantics exist, so these assertions are
 * the contract: candidate classes, deterministic tie-breaks, the total cap, and the
 * suppression reasons the cart and the AI both surface.
 */
...
const ctx = (over: Partial<LineContext> = {}): LineContext => ({
  ...
  quantity: 1,
  orderSubtotalMinorUnits: 100_000,
  userSegments: [],
...
describe('live-session eligibility', () => {
  it('applies the live rule only when the resolver says the line is live-eligible', () => {
    ...
    expect(eligible.discountMinorUnits).toBe(20_000);

    // The session ended: liveEligible is the ONLY input that can turn this on.
    ...
    expect(ended.netMinorUnits).toBe(100_000);
  });
...
describe('candidate classes: non-stackable means it combines with nothing', () => {
  ...
  it('flipping LIVE20.stackable to true is exactly what permits the combination', () => {
    ...
    expect(result.discountMinorUnits).toBe(25_000);
    expect(result.suppressed).toEqual([]);
  });
...
describe('kinds and clamping', () => {
  it('handles a flat rule without inventing a percentage', () => {
    const flat = promo({ code: 'FLAT500', kind: 'flat', value: 50_000, priority: 5 });
    const result = evaluatePromotions([flat], ctx(), 50);
    expect(result.discountMinorUnits).toBe(50_000);
  });

  it('clamps a flat rule larger than the line to the line gross, then to the cap', () => {
    const flat = promo({ code: 'FLATBIG', kind: 'flat', value: 999_999 });
    const result = evaluatePromotions([flat], ctx(), 50);
    expect(result.discountMinorUnits).toBe(50_000);
    expect(result.suppressed).toEqual([
  ...
  it('multiplies by quantity', () => {
    ...
    expect(result.grossMinorUnits).toBe(300_000);
    expect(result.discountMinorUnits).toBe(60_000);
  });
});
...
describe('the total cap', () => {
  it('trims the lowest-priority stackable contribution first and reports it capped', () => {
    ...
    expect(result.discountMinorUnits).toBeLessThanOrEqual(50_000);
    expect(result.applied.map((a) => a.code)).toEqual(['BIG']);
    // LOW is dropped before MID, because trimming starts at the lowest priority.
  ...
  it('clamps a lone non-stackable winner when there is nothing left to trim', () => {
    ...
    expect(result.discountMinorUnits).toBe(50_000);
    expect(result.applied).toEqual([{ code: 'HUGE', label: 'HUGE label', minorUnits: 50_000 }]);
...
describe('conditions', () => {
  ...
  it('honours category, line-value and order-value conditions', () => {
    const beauty = promo({
      code: 'BEAUTY15',
      value: 15,
      conditions: { categorySlugs: ['cosmetics'], minLineMinorUnits: 99_900 },
    });
    ...
    expect(
      evaluatePromotions(
        ...
        ctx({
          ...
          orderSubtotalMinorUnits: 50_000,
        }),
        50,
      ).applied,
    ).toEqual([]);
