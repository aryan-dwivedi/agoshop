...
/**
 * The AI's tool surface. One definition drives three things that must never drift:
 * the JSON schema sent to the model, the zod validator the executor runs on the
 * model's arguments, and the read-only/mutating classification that decides whether
...
/**
 * Mutating tools execute serially in model order and are deduplicated through
 * `aiToolCalls`; read-only tools may run in parallel.
 */
...
export type ToolResult = Record<string, unknown> | { error: { code: string; message: string } };
