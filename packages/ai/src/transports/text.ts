...
import { aiConversations, aiMessages } from '@shop/db/schema.js';
...
import { LANGUAGE_AUTO, detectLanguage, resolveSpokenLanguage } from '@shop/shared';
...
export const textTransport: VoiceTransport = {
    ...
    async sendUserTurn(
        ...
        return {
            reply: result.text,
            language,
            toolCalls: result.executed.map((e) => ({
                name: e.name,
                outcome: e.outcome,
            })),
