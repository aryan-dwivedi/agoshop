...
import {
    ...
    listOrdersWithFulfilment,
} from '@shop/domain-commerce/fulfilment.js';

type FulfilmentToolInvocation = Extract<
    ToolInvocation,
    {
...
export const runFulfilmentTool = async (
    conversation: ConversationRecord,
    call: FulfilmentToolInvocation,
): Promise<Record<string, unknown>> => {
    switch (call.name) {
        case 'list_my_orders': {
            const orders = await listOrdersWithFulfilment(
                conversation.userId,
                call.args.limit ?? 5,
            );
            return {
        ...
        case 'get_order_status': {
            if (call.args.order_id) {
                const order = await getOrderFulfilmentStatus(
                    conversation.userId,
                    call.args.order_id,
                );
