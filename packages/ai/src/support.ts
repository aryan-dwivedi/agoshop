...
import { aiMessages, supportTickets } from '@shop/db/schema.js';
import { conflict, forbidden, notFound } from '@shop/platform/lib/errors.js';
import { publishToUser } from '@shop/platform/lib/sse.js';
import { EVENTS } from '@shop/shared';

import { loadConversation, stopConversation } from './conversations.js';
...
export const escalateToHuman = async (input: {
    ...
    await stopConversation(input.conversationId, {
        status: 'stopped',
        reason: 'escalated',
    });
    ...
    const [claimed] = await db
        ...
        .where(
            and(
                ...
                or(
                    eq(supportTickets.status, 'queued'),
                    eq(supportTickets.assignedAgentId, agentUserId),
                ),
            ),
        )
        .returning();
...
export const getTicketForShopper = async (
    ...
    const [row] = await db
        ...
        .where(
            and(
                eq(supportTickets.conversationId, conversationId),
