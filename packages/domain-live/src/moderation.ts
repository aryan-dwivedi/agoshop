...
import { publishToShards } from '@shop/agora/signaling.js';
...
import { moderationActionsTotal } from '@shop/platform/lib/metrics.js';
...
export const applyModeration = async (a: {
    ...
    if (a.action === 'delete_message') {
        ...
        const deleted = await db
            ...
            .set({ status: 'deleted' })
            .where(
                and(eq(chatMessages.id, a.targetMessageId), eq(chatMessages.sessionId, session.id)),
            )
