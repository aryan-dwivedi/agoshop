...
import { liveSessionProducts, liveSessions, products, sellers } from '@shop/db/schema.js';
...
const resolveSeller = async (
    ...
    if (!owned) throw badRequest('no_seller_profile', 'this account owns no seller profile');
    return owned.id;
};
export const roomRule = (percent: number | null | undefined): number | null =>
...
export const createSession = async (actor: SessionActor, input: CreateSessionInput) => {
    ...
    if (title.length === 0) throw badRequest('title_required');
    let slug = input.slug?.trim().toLowerCase() ?? '';
    if (slug.length > 0) {
        if (!SLUG_PATTERN.test(slug)) throw badRequest('invalid_slug');
    ...
    if (!row) throw conflict('session_not_created');
    const dto = await getSessionById(row.id);
    if (!dto) throw conflict('session_not_created');
    return dto;
};
...
export const setSessionProducts = async (
    sessionId: string,
    items: {
        productId: string;
        sortOrder?: number;
    ...
    if (session.status === 'ended') throw conflict('session_ended');
    if (items.length === 0) throw badRequest('no_products');
    if (items.filter((i) => i.isFeatured).length > 1) throw badRequest('multiple_featured');
    const ids = items.map((i) => i.productId);
    const known = await db
        .select({ id: products.id })
        .from(products)
        .where(inArray(products.id, ids));
    if (known.length !== new Set(ids).size) throw badRequest('unknown_product');
    ...
    if (!ordered.some((i) => i.isFeatured) && ordered[0]) ordered[0].isFeatured = true;
    await db.transaction(async (tx) => {
        await tx.delete(liveSessionProducts).where(eq(liveSessionProducts.sessionId, sessionId));
        await tx.insert(liveSessionProducts).values(
            ordered.map((item) => ({
                sessionId,
                productId: item.productId,
