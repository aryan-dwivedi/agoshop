...
import { liveSessionProducts, liveSessions } from '@shop/db/schema.js';
...
export const pinProduct = async (sessionId: string, productId: string | null) => {
    if (productId !== null) {
        const [attached] = await db
            ...
            .where(
                and(
                    eq(liveSessionProducts.sessionId, sessionId),
                    eq(liveSessionProducts.productId, productId),
                ),
            );
        if (!attached) throw badRequest('product_not_in_session');
    }
    ...
    await publishToSession(sessionId, EVENTS.sessionProductPinned, {
        sessionId,
        productId,
    });
