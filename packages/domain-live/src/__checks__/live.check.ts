...
import { chatMessages, liveSessions, products, sellers, users } from '@shop/db/schema.js';
    ...
    try {
        ...
        const starts = await Promise.allSettled(
            Array.from({ length: 10 }, () =>
                startSession(sessionId, { consentAcknowledged: true }),
            ),
        );
        ...
        const [afterTier] = await db
            ...
            .where(eq(liveSessions.id, sessionId));
        check(
            'db delivery tier is cdn',
            afterTier?.deliveryTier === 'cdn',
            afterTier?.deliveryTier,
        );
        check(
        ...
        const ends = await Promise.allSettled(
            Array.from({ length: 10 }, () => endSession(sessionId)),
        );
