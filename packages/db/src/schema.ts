...
/**
 * Money is integer paise (INR); every money column is suffixed `MinorUnits`.
 * Requires PostgreSQL >= 15 for `UNIQUE NULLS NOT DISTINCT` on cart lines.
 */
...
export const users = pgTable('users', {
  ...
  /** Platform-level ban. Session-scoped chat bans live in chatModeration. */
...
export const productVariants = pgTable(
  'product_variants',
  {
    ...
    /** The authoritative price at every evaluation. */
    priceMinorUnits: integer('price_minor_units').notNull(),
    /**
     * Manufacturer's list price, the tier the shop price is marked down FROM.
     * Nullable: not every variant is marked down, and a seller who has not published
     * an MRP must not be given a fabricated one.
     */
...
export const cartItems = pgTable(
  'cart_items',
  {
    ...
    /** Display snapshot only — never the authoritative checkout price. */
  ...
  (t) => [
    // NULLS NOT DISTINCT so repeated browse adds of one variant coalesce.
    unique('cart_items_line_unique').on(t.cartId, t.variantId, t.liveSessionId).nullsNotDistinct(),
  ],
);
...
export const orders = pgTable(
  'orders',
  {
    ...
    /** Pending until capture completes; only paid orders fulfil. */
    ...
    appliedPromotions: jsonb('applied_promotions')
      .$type<{ code: string; label: string; minorUnits: number }[]>()
      .notNull()
      .default([]),
...
export const orderItems = pgTable(
  'order_items',
  {
    ...
    appliedPromotionCodes: jsonb('applied_promotion_codes').$type<string[]>().notNull().default([]),
    /** Live attribution is per line: one cart can mix sessions and browse lines. */
    liveSessionId: uuid('live_session_id').references(() => liveSessions.id, {
      onDelete: 'set null',
    }),
  },
...
export const liveSessions = pgTable('live_sessions', {
  ...
  /**
   * A second publisher invited onto the stage. Deliberately NOT a second owner: it
   * authorizes RTC publishing only, so lifecycle actions (start, end, pin, polls,
   * moderation) stay with `hostUserId` and a co-host cannot end someone else's show.
   */
  coHostUserId: uuid('co_host_user_id').references(() => users.id, { onDelete: 'set null' }),
  ...
  /**
   * A seller-uploaded video that IS the broadcast, rather than a camera. Combined with
   * `autoStart` it makes a premiere: the session flips live on schedule with nobody at
   * a console, and every viewer is placed at the same wall-clock position in the file
   * (see `syncedPosition`), so it behaves like a live stream rather than a download.
   */
  sourceVideoUrl: text('source_video_url'),
  /**
   * Whether the background process may perform the scheduled -> live transition once
   * `scheduledFor` passes. Opt-in: a session a seller intends to host from a camera
   * must never go live to an empty channel because a clock ticked over.
   */
  autoStart: boolean('auto_start').notNull().default(false),
  /**
   * This room's own live-only markdown, in whole percent, editable by the host mid-show.
   * Stored as a rule input, never as a price: the live tier is always what the promotions
   * evaluator returns, so a mid-show change reprices open carts and is re-proved at
   * checkout instead of being frozen into a row somewhere. `null` means this session
   * adds no rule of its own.
   */
  discountPercent: integer('discount_percent'),
  /** Frozen at the scheduled -> live transition; never recomputed. */
  ...
  /** Fixed numeric uid OBS publishes as via Media Gateway ingress. */
...
export const sessionTranscripts = pgTable(
  'session_transcripts',
  {
    ...
    translatedText: jsonb('translated_text').$type<Record<string, string>>().notNull().default({}),
...
export const chatMessages = pgTable(
  'chat_messages',
  {
    ...
    /** Client-supplied id so a retried send replays instead of duplicating. */
    clientMessageId: text('client_message_id').notNull(),
    text: text('text').notNull(),
    /** Optional host-shared product card attached to this chat message. */
...
/**
 * Tool-call idempotency. Keyed on inputs a retry reproduces — a retried Agora
 * callback re-runs the model and mints a NEW toolCallId, so keying on that id
 * would dedupe nothing. `toolCallId` is kept for tracing only.
 */
...
export const sessionRelations = relations(liveSessions, ({ one, many }) => ({
  seller: one(sellers, { fields: [liveSessions.sellerId], references: [sellers.id] }),
